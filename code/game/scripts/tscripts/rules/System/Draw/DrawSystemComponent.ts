...
export class DrawSystemComponent extends ET.SingletonComponent {
    ...
    public addEvent() {
        ...
        EventHelper.addProtocolEvent(this, DrawConfig.EProtocol.StartCardSelected, (event: CLIENT_DATA<DrawConfig.I.ICardSelected>) => {
            [event.state, event.message] = GPlayerSystem.GetInstance().GetPlayer(event.PlayerID).DrawComp().OnStartCardSelected(1, event.data.itemName);
        });
        /**选卡 */
        EventHelper.addProtocolEvent(this, DrawConfig.EProtocol.CardSelected, (event: CLIENT_DATA<DrawConfig.I.ICardSelected>) => {
            [event.state, event.message] = GPlayerSystem.GetInstance().GetPlayer(event.PlayerID).DrawComp().OnSelectCard(event.data.index, event.data.itemName, event.data.b2Public);
        });
    }

...
export module BattleHelper {
     ...
     * @param iAttackState 攻击特效参数
      */
    export function Attack(hCaster: BaseNpc_Plus, hTarget: BaseNpc_Plus, iAttackState: enum_ATTACK_STATE = 0) {
        if (globalData.RECORD_SYSTEM_DUMMY.ATTACK_SYSTEM == null) {
            globalData.RECORD_SYSTEM_DUMMY.ATTACK_SYSTEM = {}
        }
        let iNextRecord = GetNextRecord()
        globalData.RECORD_SYSTEM_DUMMY.ATTACK_SYSTEM['' + iNextRecord] = {
    ...
    export function GoApplyDamage(tDamageTable: DamageOptions) {
        ...
        if (globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM == null) {
            globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM = {}
        }
        let iNextRecord = GetNextRecord()
        globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM['' + iNextRecord] = iEOMFlag
    ...
    export function DamageFilter(iRecord: string | number, ...args: enum_EOM_DAMAGE_FLAGS[]) {
        let bool = false
        if (globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM) {
    ...
    export function GetNextRecord() {
        if (globalData.RECORD_SYSTEM_DUMMY.iLastRecord == null) {
            globalData.RECORD_SYSTEM_DUMMY.iLastRecord = 0
        }
        if (globalData.RECORD_SYSTEM_DUMMY.iLastRecord && globalData.RECORD_SYSTEM_DUMMY.iLastRecord >= 255) {
            globalData.RECORD_SYSTEM_DUMMY.iLastRecord = globalData.RECORD_SYSTEM_DUMMY.iLastRecord - 256
        }
        return (globalData.RECORD_SYSTEM_DUMMY.iLastRecord + 1);
    }

    export function GetDamageFlag(iRecord: string | number) {
        let iEOMFlag = enum_EOM_DAMAGE_FLAGS.EOM_DAMAGE_FLAG_NONE
        if (globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM != null) {
            return globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM['' + iRecord] || iEOMFlag
        }
        return iEOMFlag
    }
    export function GetAttackOption(iRecord: string | number) {
        if (globalData.RECORD_SYSTEM_DUMMY.ATTACK_SYSTEM != null) {
            return globalData.RECORD_SYSTEM_DUMMY.ATTACK_SYSTEM['' + iRecord]
        }
    }
    ...
    export function _BeforeTransformedDamage(iRecord: string | number) {
        iRecord = '' + iRecord;
        if (globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM == null) { globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM = {} }
        if (globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM[iRecord] == null) { globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM[iRecord] = enum_EOM_DAMAGE_FLAGS.EOM_DAMAGE_FLAG_NONE }
        let isIn = GameFunc.IncludeArgs(globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM[iRecord], enum_EOM_DAMAGE_FLAGS.EOM_DAMAGE_FLAG_BEFORE_TRANSFORMED_DAMAGE)[0]
        if (!isIn) {
            globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM[iRecord] = globalData.RECORD_SYSTEM_DUMMY.DAMAGE_SYSTEM[iRecord] + enum_EOM_DAMAGE_FLAGS.EOM_DAMAGE_FLAG_BEFORE_TRANSFORMED_DAMAGE
        }
    }

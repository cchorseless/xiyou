...
export module TimerHelper {
    ...
    export class TimeWorker {
        ...
        static Start() {
            ...
            if (globalData.TimerEntity != null) { return }
        ...
        private static startTimeThink() {
            if (globalData.TimerEntity == null) { return }
            if (GameRules.State_Get() >= DOTA_GameState.DOTA_GAMERULES_STATE_POST_GAME) { return };
            //    固定時間间隔
            globalData.TimerEntity.SetContextThink(TimeWorker.__TIME_THINK__, () => {
                for (let timeid in globalData.allTimers) {
                    ...
                    if (now >= _timeInfo.finishtime) {
                        // -- Run the callback
                        let [status, nextCall] = xpcall(_timeInfo.handler, (msg: any) => {
        ...
        private static startFrameThink() {
            ...
            if (GameRules.State_Get() >= DOTA_GameState.DOTA_GAMERULES_STATE_POST_GAME) { return };
            //    固定帧率
            globalData.TimerEntity.SetContextThink(TimeWorker.__FRAME_THINK__, () => {
    ...
    export function addTimer(delay: number, cb: (this: any) => void | number, context: any = null, useGameTime = true) {
        ...
        let finishtime;
        if (useGameTime) { finishtime = GameRules.GetGameTime() + delay }
        else {
    ...
    export function removeTimer(timeid: string) {
        if (globalData.allTimers[timeid]) { delete globalData.allTimers[timeid] };
        if (globalData.allFrameTimers[timeid]) { delete globalData.allFrameTimers[timeid] };
    }

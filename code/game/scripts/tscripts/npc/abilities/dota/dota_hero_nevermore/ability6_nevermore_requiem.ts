...
export class ability6_nevermore_requiem extends BaseAbility_Plus {
    ...
    OnAbilityPhaseStart() {
        let hCaster = this.GetCasterPlus()
        if (!hCaster.HasTalent("special_bonus_unique_nevermore_custom")) {
            this.ParticleModifier = modifier_nevermore_6_particle_nevermore_wings.apply(hCaster, hCaster, this, { duration: BaseModifier_Plus.LOCAL_PARTICLE_MODIFIER_DURATION }) as BaseModifier_Plus

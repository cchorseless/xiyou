...
export class modifier_enchantress_3 extends BaseModifier_Plus {
    ...
    OnIntervalThink() {
        if (IsServer()) {
            ...
            if (GameFunc.IsValid((hAbility).hThinker)) {
                return
            }
        ...
        if (IsServer()) {
            ...
            if (null != tTargets[0]) {
                ...
                for (let hTarget of (tTargets)) {
                    if (!this.tPoints.indexOf(hTarget)) {
                        ...
                        for (let i = 0; i < this.tPoints.length; i++) {
                            ...
                            if (hUnit == hThinker || !GameFunc.IsValid(hUnit) || ((hUnit.GetAbsOrigin() - hThinker.GetAbsOrigin()) as Vector).Length2D() > this.radius) {
                ...
                for (let i = 0; i < this.tPoints.length; i++) {
                    ...
                    if (hUnit != hThinker) {
                        if (!GameFunc.IsValid(hUnit) || ((hUnit.GetAbsOrigin() - hThinker.GetAbsOrigin()) as Vector).Length2D() > this.radius) {

...
import { ability6_shadow_shaman_mass_serpent_ward } from "./ability6_shadow_shaman_mass_serpent_ward";
...
export class ability1_shadow_shaman_ether_shock extends BaseAbility_Plus {
    ...
    EtherShockScepter(hTarget: BaseNpc_Plus) {
        let hCaster = this.GetCasterPlus()
    ...
    EtherShock(hTarget: BaseNpc_Plus, hSource: BaseNpc_Plus) {
        let hCaster = this.GetCasterPlus()
...
export class modifier_shadow_shaman_1_totem extends BaseModifier_Plus {
    ...
    OnDestroy() {
        ...
        if (IsServer()) {
            if (GameFunc.IsValid(this.GetCasterPlus())) {
                ...
                if (GameFunc.IsValid(hAblt) && hAblt.RemoveTotem != null) {
    ...
    OnIntervalThink() {
        if (IsServer()) {
            ...
            if (!GameFunc.IsValid(hCaster) || !hCaster.GetIntellect) {

...
export class ability3_storm_spirit_overload extends BaseAbility_Plus {
    ...
    OnProjectileHit_ExtraData(hTarget: BaseNpc_Plus, vLocation: Vector, ExtraData: any) {
        ...
        let hAttacker = EntIndexToHScript(ExtraData.attacker_index) as BaseNpc_Plus
        let count = ExtraData.count
        if (GameFunc.IsValid(hTarget) && GameFunc.IsValid(hAttacker) && hCaster.HasShard()) {
...
export class modifier_storm_spirit_3 extends BaseModifier_Plus {
    ...
    OnAbilityExecuted(params: ModifierTable) {
        ...
        if (hParent == params.unit && !hParent.PassivesDisabled() && !hParent.IsIllusion()) {
            ...
            if (GameFunc.IsValid(hAbility) && !hAbility.IsItem() && hAbility.ProcsMagicStick()) {

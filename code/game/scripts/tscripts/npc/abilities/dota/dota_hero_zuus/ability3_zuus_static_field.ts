...
export class ability3_zuus_static_field extends BaseAbility_Plus {
    ...
    TriggerStaticField(hTarget: BaseNpc_Plus) {
        ...
        if (modifier_zuus_3_scepter.exist(hCaster)
            ...
            let fMaxHealth = 0
            for (let hUnit of (hUnits as BaseNpc_Plus[])) {
    ...
    GetTotalDamageOutgoing_Percentage(params: ModifierAttackEvent) {
        if (IsServer()) {
            ...
            if (GameFunc.IsValid(hAbility) &&
                ...
                (this.GetAbilityPlus() as ability3_zuus_static_field).TriggerStaticField(params.target as BaseNpc_Plus)
            }
        }
    }
}

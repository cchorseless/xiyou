...
export class ability6_obsidian_destroyer_sanity_eclipse extends BaseAbility_Plus {
    ...
    OnSpellStart() {
        ...
        //  if ( GameFunc.IsValid(hAbility2) && hAbility2.tUnit!=null ) {
        //  	for (let hUnit of ( hAbility2.tUnit )) {

        //  		if (  GameFunc.IsValid(hUnit) &&.IsAlive() && hUnit.IsPositionInRange(position, radius) && hUnit modifier_obsidian_destroyer_2_debuff.exist(  hUnit ) ) {
    ...
    HitTarget(target: BaseNpc_Plus) {
        ...
        // 每个目标获得4点能量精华
        let hModifier = modifier_obsidian_destroyer_3.findIn(caster) as BaseModifier_Plus;

...
export class ability2_dawnbreaker_celestial_hammer extends BaseAbility_Plus {
    ...
    CastFilterResultLocation(v: Vector) {
        let caster = this.GetCasterPlus() as BaseNpc_Plus & { no_hammer: boolean }
        //  没有锤子无法释放
    ...
    OnProjectileHitHandle(hTarget: BaseNpc_Plus, vLocation: Vector, projectileHandle: any) {
        let hCaster = this.GetCasterPlus()
        if (IsServer()) {
...
export class modifier_dawnbreaker_2_buff_attack extends BaseModifier_Plus {
    ...
    OnCreated(params: ModifierTable) {
        super.OnCreated(params);
        let parent = this.GetParentPlus() as BaseNpc_Plus & { no_hammer: boolean }
        //  标记没有锤子的状态

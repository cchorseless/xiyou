...
export class ability2_crystal_maiden_frostbite extends BaseAbility_Plus {
    ...
    Frostbite(target: BaseNpc_Plus, bonusDamagePercentage: number = 0) {
...
export class modifier_crystal_maiden_2_debuff extends BaseModifier_Plus {
    ...
    OnDestroy() {
        ...
        if (IsServer()) {
            ...
            if (GameFunc.IsValid(this.modifier_truesight as BaseModifier_Plus)) {
                this.modifier_truesight.Destroy()
            }
        }
    }
    @registerProp(GameEnum.Property.Enum_MODIFIER_PROPERTY.INCOMING_SPELL_DAMAGE_PERCENTAGE)

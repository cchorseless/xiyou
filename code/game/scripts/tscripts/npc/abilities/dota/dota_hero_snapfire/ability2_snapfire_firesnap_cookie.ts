...
export class ability2_snapfire_firesnap_cookie extends BaseAbility_Plus {
    ...
    OnSpellStart() {
        let hCaster = this.GetCasterPlus() as BaseNpc_Plus & { tMarkUnit: BaseNpc_Plus[] }
    ...
    CastSpellStart(hTarget: BaseNpc_Plus) {
...
export class modifier_snapfire_2_jump extends BaseModifierMotionBoth_Plus {
    ...
    UpdateHorizontalMotion(me: BaseNpc_Plus, dt: number): void {
        if (IsServer()) {
            if (this.vV != null) {
    ...
    JumpFinish() {
        ...
        let tTargets = FindUnitsInRadius(hCaster.GetTeamNumber(),
            ...
            DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NONE, FindOrder.FIND_CLOSEST, false) as BaseNpc_Plus[]

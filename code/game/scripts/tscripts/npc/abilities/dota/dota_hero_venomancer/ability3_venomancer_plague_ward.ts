...
export class modifier_venomancer_3 extends BaseModifier_Plus {
    ...
    AttackLanded(params: ModifierAttackEvent) {
        ...
        if (!GameFunc.IsValid(hTarget) || hTarget.GetClassname() == "dota_item_drop" || params.attacker != hParent || BattleHelper.AttackFilter(params.record, BattleHelper.enum_ATTACK_STATE.ATTACK_STATE_NOT_USECASTATTACKORB, BattleHelper.enum_ATTACK_STATE.ATTACK_STATE_NOT_PROCESSPROCS, BattleHelper.enum_ATTACK_STATE.ATTACK_STATE_FAKEATTACK) || !GameFunc.IsValid(hAbility)) {
            return
        }
        ...
        if (hCaster.HasShard()) {
            ...
            if (GameFunc.IsValid(hAbility1) && hAbility1.GetLevel() > 0) {
                if (GameFunc.mathUtil.PRD(this.chance, hCaster, "venomancer_shard")) {

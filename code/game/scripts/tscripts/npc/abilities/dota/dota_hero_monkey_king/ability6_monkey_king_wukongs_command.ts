...
export class ability6_monkey_king_wukongs_command extends BaseAbility_Plus {
    ...
    OnSpellStart() {
        ...
        this.hDummy = modifier_monkey_king_6_thinker.applyThinker(vPosition, hCaster, this, { radius: max_radius }, hCaster.GetTeamNumber(), false) as BaseNpc_Plus
...
export class modifier_monkey_king_6_scepter_active extends BaseModifier_Plus {
    ...
    On_AttackLanded(params: ModifierAttackEvent) {
        if (params.attacker == this.GetParentPlus()) {
            ...
            let vPosition = hCaster.GetAbsOrigin()
            hCaster.SetAbsOrigin(this.GetParentPlus().GetAbsOrigin())
            BattleHelper.Attack(hCaster, params.target as BaseNpc_Plus, BattleHelper.enum_ATTACK_STATE.ATTACK_STATE_SKIPCOOLDOWN + BattleHelper.enum_ATTACK_STATE.ATTACK_STATE_IGNOREINVIS + BattleHelper.enum_ATTACK_STATE.ATTACK_STATE_NOT_USEPROJECTILE)
            hCaster.SetAbsOrigin(vPosition)

    ...
    OnIntervalThink() {
        if (IsServer()) {
            if (!GameFunc.IsValid(this.modifier) || this.level == null || this.level != this.GetAbilityPlus().GetLevel()) {
                ...
                this.modifier = modifier_t7_enlightenment_aura_hidden.apply((this.GetParentPlus(), this.GetParentPlus(), this.GetAbilityPlus(), null)) as BaseModifier_Plus
            }
        }
    }
    OnDestroy() {

...
import { modifier_ember_spirit_1 } from "./ember_spirit_1";
/** dota原技能数据 */
...
@registerAbility()
export class ember_spirit_2 extends BaseAbility_Plus {
    /**对应dota内的名字 */
...
export class modifier_ember_spirit_2_invulnerability extends BaseModifier_Plus {
    modifier_no_health_bar: BaseModifier_Plus;
    IsHidden() {
        return true
    }
    IsDebuff() {
    ...
    OnCreated(params: ModifierTable) {
        ...
        if (IsServer()) {
            ...
            this.modifier_no_health_bar = modifier_no_health_bar.apply(this.GetParentPlus(), this.GetParentPlus(), this.GetAbilityPlus(), null) as BaseModifier_Plus
        }
    }
    OnDestroy() {
        super.OnDestroy();

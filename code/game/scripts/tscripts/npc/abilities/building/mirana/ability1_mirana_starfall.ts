...
export class modifier_mirana_1_thinker extends modifier_particle_thinker {
    ...
    OnRemoved() {
        ...
        if (IsServer()) {
            ...
            for (let iTargetEntIndex of (this.tTargetEntIndex)) {
                let hTarget = EntIndexToHScript(iTargetEntIndex) as BaseNpc_Plus
                if (GameFunc.IsValid(hTarget) && hTarget.IsAlive()) {
                    let iStackCount = 0
                    if (this.bIsSecondary) {
                        let hModifier = modifier_mirana_1_counter.findIn(hTarget) as BaseModifier_Plus;
            ...
            if (!this.bIsSecondary) {
                ...
                let hModifier = modifier_mirana_1_thinker.apply(hParent, hCaster, hAbility, { duration: ability1_mirana_starfall.delay, is_secondary: 1, hashtable_index: HashTableHelper.GetHashtableIndex(tTargetEntIndex) }) as BaseModifier_Plus
                if (GameFunc.IsValid(hModifier)) {
    ...
    Init_old() {
        if (IsServer()) {
            if (!this.initialized && this.GetAbilityPlus().GetLevel() != 0) {
                ...
                this.GetAbilityPlus().addFrameTimer(1, () => {
                    if (this.IsNull()) {
                        return
                    }

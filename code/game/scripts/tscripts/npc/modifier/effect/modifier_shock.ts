...
export class modifier_shock extends BaseModifier_Plus {
    ...
    OnHurted(params: ModifierInstanceEvent) {
        ...
        let hAbility = params.inflictor as BaseAbility_Plus;
     ...
     * @param iShockStack
     */
    static Shock(target: BaseNpc_Plus, hCaster: BaseNpc_Plus, hAbility: BaseAbility_Plus, iCount: number) {
        if (!IsServer()) { return };
        if (iCount > 0) {
    ...
    static ShockActive(target: BaseNpc_Plus, hCaster: BaseNpc_Plus, hAbility: BaseAbility_Plus, fPercent: number, bIgnoreCooldown = true) {

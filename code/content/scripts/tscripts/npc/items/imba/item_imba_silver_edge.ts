...
BeDestroy():void {
    if (IsServer()) {
        if (!this.GetParentPlus().findBuff<modifier_silver_edge_invis_flying_disabled>("modifier_silver_edge_invis_flying_disabled")) {
    ...
    if (IsServer()) {
        if (params.attacker == this.GetParentPlus()) {
            ...
            if (this.GetParentPlus().IsRangedAttacker()) {
                ...
                let direction = (CalculateDirection(params.target, this.GetParentPlus()));
            ...
            let enemies = FindUnitsInCone(this.GetParentPlus().GetTeamNumber(), CalculateDirection(params.target, this.GetParentPlus()), this.GetParentPlus().GetAbsOrigin(), cleave_radius_start, cleave_radius_end, cleave_distance, undefined, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_CREEP, 0, FindOrder.FIND_ANY_ORDER, false);
    ...
    if (IsServer()) {
        if (params.unit == this.GetParentPlus()) {
            ...
            if (params.attacker.IsHeroDamage(params.damage)) {
                parent.AddNewModifier(parent,this, "modifier_item_imba_silver_edge_invis_flying_disabled", {
...
CC_OnAttack(keys:ModifierAttackEvent):void {
    ...
    let parent = this.GetParentPlus();
    if (keys.attacker == parent && item && !parent.IsIllusion() && parent.FindAllModifiersByName(this.GetName())[1] == this) {
        if (!parent.IsRangedAttacker()) {
            if (this.echo_ready == true && !keys.no_attack_cooldown) {
                this.echo_ready = false;
                this.StartIntervalThink(GetItemKV("item_imba_echo_sabre", "AbilityCooldown"));
        ...
        if (parent.HasModifier("modifier_imba_echo_rapier_haste") && (!parent.HasAbility("imba_slark_essence_shift") || parent.findAbliityPlus<imba_slark_essence_shift>("imba_slark_essence_shift").GetCooldownTime() < parent.FindAbilityByName("imba_slark_essence_shift").GetEffectiveCooldown(parent.FindAbilityByName("imba_slark_essence_shift").GetLevel()))) {

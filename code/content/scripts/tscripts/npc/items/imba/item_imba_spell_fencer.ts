...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    if (IsServer()) {
        ...
        ApplyDamage({
            ...
            damage_flag: DOTADamageFlag_t.DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS
        });
    }
}
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    if (IsServer()) {
        ...
        for (const [_, sword_modifier] of ipairs(priority_sword_modifiers)) {
...
BeCreated(p_0:any,):void {
    ...
    if (IsServer()) {
        ...
        for (const [_, modifier] of ipairs(higher_tier_modifiers)) {

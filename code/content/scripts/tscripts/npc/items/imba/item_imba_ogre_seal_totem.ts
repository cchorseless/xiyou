...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let vToTarget = this.GetCursorPosition() - this.GetCasterPlus().GetOrigin();
        vToTarget = vToTarget.Normalized();
        let vLocation = this.GetCasterPlus().GetOrigin() + vToTarget * 25;
        let kv = {
            vLocX: vLocation.x,
            vLocY: vLocation.y,
            vLocZ: vLocation.z

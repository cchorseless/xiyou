...
BeCreated(p_0:any,):void {
    ...
    if (hItem) {
        ...
        let main_hero = this.GetParentPlus().GetOwnerPlus().GetAssignedHero();
        let main_hero_modifier = main_hero.FindModifierByName(this.GetName());
...
BeDestroy():void {
    if (IsServer()) {
        let gem = CreateItem("item_imba_gem", undefined, undefined);
        gem.SetOwner(undefined);
        CreateItemOnPositionSync(this.GetParentPlus().GetAbsOrigin(), gem);
        gem.dummy_unit = BaseNpc_Plus.CreateUnitByName("npc_dummy_unit_perma", this.GetParentPlus().GetAbsOrigin(), true, undefined, undefined, this.GetCasterPlus().GetTeam());

...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    if (this.GetItemPlus() && this.GetItemPlus().GetName() == "item_imba_manta_2" && keys.unit == this.GetParentPlus() && this.GetParentPlus().IsRealHero() && keys.attacker != this.GetParentPlus() && (!this.GetParentPlus().IsReincarnating || (this.GetParentPlus().IsReincarnating && !this.GetParentPlus().IsReincarnating())) && (keys.attacker.IsRealHero() || keys.attacker.IsClone() || keys.attacker.IsTempestDouble() || keys.attacker.IsIllusion())) {
        ...
        this.death_illusions = CreateIllusions(this.GetCasterPlus(), keys.attacker.GetOwnerPlus().GetAssignedHero(), {
        ...
        }, 1, this.distance_multiplier, false, true);
        for (const [_, illusion] of ipairs(this.death_illusions)) {
...
public incoming_damage : number; 
public distance_multiplier : number; 
public created_manta_illusions : any; 
IsHidden():boolean {
    return true;
}
IsPurgable():boolean {
...
BeDestroy():void {
    ...
    if (this.GetItemPlus().manta_illusions) {
        for (const [_, illusion] of ipairs(this.GetItemPlus().manta_illusions)) {
            if (illusion && !illusion.IsNull()) {
                illusion.ForceKill(false);
            }
        }
    }
    for (const [_, mod] of ipairs(this.GetCasterPlus().FindAllModifiersByName("modifier_item_manta_passive"))) {
        if (mod.GetItemPlus() && mod.GetItemPlus() != this.GetItemPlus() && mod.GetItemPlus().manta_illusions) {
            for (const [_, illusion] of ipairs(mod.GetItemPlus().manta_illusions)) {
                if (illusion && !illusion.IsNull()) {
                    illusion.ForceKill(false);
                }
            }
    ...
    this.GetItemPlus().manta_illusions = {}
    ...
    this.created_manta_illusions = CreateIllusions(this.GetCasterPlus(), this.GetParentPlus().GetOwnerPlus().GetAssignedHero(), {
        outgoing_damage: this.outgoing_damage,
        incoming_damage: this.incoming_damage,
        ...
        outgoing_damage_roshan: undefined,
        duration: this.GetItemPlus().GetSpecialValueFor("tooltip_illusion_duration")
    }, this.GetItemPlus().GetSpecialValueFor("images_count"), this.distance_multiplier, true, true);
    if (this.GetParentPlus().HasModifier("modifier_item_imba_yasha_active")) {
        this.GetParentPlus().findBuff<modifier_item_imba_yasha_active>("modifier_item_imba_yasha_active").Destroy();
    }
    ...
    for (const [_, unit] of ipairs(this.created_manta_illusions)) {
        ...
        if (unit.IsIllusion()) {
            table.insert(this.GetItemPlus().manta_illusions, unit);
        }
    }
}

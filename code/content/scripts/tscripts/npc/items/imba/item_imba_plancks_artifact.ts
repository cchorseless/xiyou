...
set_respawn_time(reset) {
    if (reset != undefined) {
        this.GetCasterPlus().plancks_artifact_respawn_reduction = undefined;
        print("PLANCK: resetting respawn time");
    } else {
        this.GetCasterPlus().plancks_artifact_respawn_reduction = this.GetSpecialValueFor("respawn_time_reduction") * this.GetCurrentCharges();
        print("PLANCK: setting respawn reduction time to " + tostring(this.GetCasterPlus().plancks_artifact_respawn_reduction));
    }
}
OnSpellStart():void {
    if (this.GetCasterPlus().HasModifier("modifier_imba_reincarnation")) {
        this.GetCasterPlus().Kill(this.GetCasterPlus());
    } else {
        TrueKill(this.GetCasterPlus(), this.GetCasterPlus(), );
    }
    ...
    for (const [_, unit] of ipairs(units_to_damage)) {
        ApplyDamage({
            victim: unit,
...
BeCreated(p_0:any,):void {
    ...
    if (IsServer()) {
        this.GetItemPlus().set_respawn_time();
    }
}
BeDestroy():void {
    if (IsServer()) {
        this.GetItemPlus().set_respawn_time(true);
    }
}

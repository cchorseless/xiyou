...
export class modifier_item_imba_shadow_blade_invis extends BaseModifier_Plus {
    ...
    BeDestroy(): void {
        if (IsServer()) {
            if (!this.GetParentPlus().findBuff("modifier_shadow_blade_invis_flying_disabled")) {
        ...
        if (IsServer()) {
            if (params.unit == this.GetParentPlus()) {
                ...
                if (GFuncEntity.IsHeroDamage(params.attacker, params.damage)) {
                    parent.AddNewModifier(parent, this.GetItemPlus(), "modifier_item_imba_shadow_blade_invis_flying_disabled", {

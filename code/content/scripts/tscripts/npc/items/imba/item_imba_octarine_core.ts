...
OnSpellStart():void {
    ...
    if (this.GetCasterPlus().findBuffStack("modifier_imba_octarine_core_basic", this.GetCasterPlus()) == 1) {
        for (const [_, mod] of ipairs(this.GetParentPlus().FindAllModifiersByName(this.GetIntrinsicModifierName()))) {
            mod.SetStackCount(2);
        }
    } else {
        for (const [_, mod] of ipairs(this.GetParentPlus().FindAllModifiersByName(this.GetIntrinsicModifierName()))) {
...
CC_OnTakeDamage(keys:ModifierInstanceEvent):void {
    if (keys.attacker == this.GetParentPlus() && !keys.unit.IsBuilding() && !keys.unit.IsOther()) {
        if (this.GetParentPlus().FindAllModifiersByName(this.GetName())[1] == this && keys.damage_category == DamageCategory_t.DOTA_DAMAGE_CATEGORY_SPELL && keys.inflictor && bit.band(keys.damage_flags, DOTADamageFlag_t.DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL) != DOTADamageFlag_t.DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL) {
            ...
            if (keys.unit.IsIllusion()) {
                if (keys.damage_type == DAMAGE_TYPES.DAMAGE_TYPE_PHYSICAL && keys.unit.GetPhysicalArmorValue && GetReductionFromArmor) {
            ...
            if (keys.unit.IsCreep()) {
                keys.attacker.Heal(math.max(keys.damage, 0) * this.GetItemPlus().GetSpecialValueFor("creep_lifesteal") * 0.01, keys.attacker);
            } else {
                keys.attacker.Heal(math.max(keys.damage, 0) * this.GetItemPlus().GetSpecialValueFor("hero_lifesteal") * 0.01, keys.attacker);
            }
        }
    }
}
...
CC_OnSpentMana(keys:ModifierAbilityEvent):void {
    if (this.GetItemPlus() && keys.unit == this.GetParentPlus() && keys.unit.FindAllModifiersByName(this.GetName())[1] == this && this.GetStackCount() == 2 && this.GetItemPlus().IsCooldownReady() && keys.cost > 0) {

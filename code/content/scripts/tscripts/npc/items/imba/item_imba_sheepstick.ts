...
export class modifier_item_imba_sheepstick_debuff extends BaseModifier_Plus {
    ...
    Init(p_0: any,): void {
        if (IsServer()) {
            ...
            if (this.GetCasterPlus().TempData().sheepstick_model) {
                if (this.GetCasterPlus().TempData().sheepstick_model == "models/props_gameplay/pig_blue.vmdl") {
                    this.SetStackCount(1);
                } else if (this.GetCasterPlus().TempData().sheepstick_model == "models/props_gameplay/roquelaire/roquelaire.vmdl") {

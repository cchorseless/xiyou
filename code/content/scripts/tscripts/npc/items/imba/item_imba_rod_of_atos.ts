...
OnSpellStart():void {
    ...
    for (let shot = 0; shot <= this.curtain_fire_shot_count + (GameFunc.GetCount(enemies) * this.curtain_fire_shot_per_hero_count); shot += 1) {
        this.AddTimer(shot * this.curtain_fire_delay, () => {
            ...
            let random_target_location = GetGroundPosition(target_location + RandomVector(this.curtain_fire_radius_second * 0.5), undefined);
            let random_fire_direction = random_target_location - random_spawn_location;
            random_fire_direction.z = 0;
            let linear_projectile = {
                Ability: this,
                EffectName: "particles/items2_fx/rod_of_atos_attack_" + string.lower(this.caster.GetTeamRealName()) + ".vpcf",
                ...
                bDeleteOnHit: true,
                vVelocity: random_fire_direction.Normalized() * this.curtain_fire_speed,
                bProvidesVision: false
            }
...
BeCreated(p_0:any,):void {
    ...
    if (this.GetItemPlus() && this.GetItemPlus().GetLevel() >= 2 && !this.GetItemPlus().initialized) {
        this.GetItemPlus().SetCurrentCharges(this.GetItemPlus().GetSpecialValueFor("initial_charges"));
        this.GetItemPlus().initialized = true;
    }
}

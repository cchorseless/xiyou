...
OnIntervalThink():void {
    for (const [_, modifier] of ipairs(this.GetParentPlus().FindAllModifiersByName("modifier_imba_radiance_basic"))) {
    ...
    if (IsServer()) {
        ...
        if (stacks > 0) {
            ...
            if (!modifier_afterburn) {
                modifier_afterburn = this.GetParentPlus().AddNewModifier(this.GetItemPlus().GetCasterPlus(), this.GetItemPlus(), "modifier_imba_radiance_afterburn", {});
            }
            if (modifier_afterburn) {
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        if (caster.IsRealHero()) {
            ...
            for (const [_, hero] of ipairs(real_hero_finder)) {

...
CC_GetModifierProcAttack_BonusDamage_Physical(keys:ModifierAttackEvent):number {
    if (keys.attacker == this.parent && keys.target.GetTeamNumber() != this.parent.GetTeamNumber() && this.GetStackCount() == 2 && GFuncRandom.PRD(this.agi_armor_ignore_chance_pct, )) {
        let damage = CalculateDamageIgnoringArmor(keys.target.GetPhysicalArmorBaseValue(), keys.damage);
        SendOverheadEventMessage(undefined, DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_DAMAGE, keys.target, damage, undefined);
        return damage;
    }
}

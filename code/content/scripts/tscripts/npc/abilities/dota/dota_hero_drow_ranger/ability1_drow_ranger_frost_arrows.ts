...
export class modifier_drow_ranger_1 extends BaseModifier_Plus {
    ...
    GetProcAttack_BonusDamage_Physical(params: ModifierAttackEvent) {
        if (!params.attacker.IsIllusion()) {
            if (this.records.indexOf(params.record) != -1) {
                modifier_drow_ranger_1_slow.apply(params.target, params.attacker, this.GetAbilityPlus(), { duration: this.frost_arrows_duration * (params.target as IBaseNpc_Plus).GetStatusResistanceFactor(params.attacker) })
                modifier_drow_ranger_1_debuff.apply(params.target, params.attacker, this.GetAbilityPlus())
                return this.agility_multiplier * (params.attacker as IBaseNpc_Plus).GetAgility()
            }
        }
    }
}
        ...
        if (hTargets[0] != null) {
            for (let hTarget of (hTargets)) {
                if (hCaster.HasTalent("special_bonus_unique_drow_ranger_custom_1")) {
                    modifier_drow_ranger_1_root.apply(hTarget, hCaster, hAbility, { duration: duration * (hTarget as IBaseNpc_Plus).GetStatusResistanceFactor(hCaster) })
                }

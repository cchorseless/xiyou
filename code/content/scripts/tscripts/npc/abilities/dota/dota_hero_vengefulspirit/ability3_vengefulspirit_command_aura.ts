...
export class modifier_vengefulspirit_3 extends BaseModifier_Plus {
    ...
    OnTakeDamage(params: IModifierTable) {
        if (IsServer()) {
            ...
            if (IsValid(hAbility) && hAbility.IsItem != null && IsValid(params.unit) && hAbility != this.GetAbilityPlus() && !hAbility.IsItem() && !hAbility.IsToggle() && hAbility.ProcsMagicStick()) {
...
export class modifier_vengefulspirit_3_aura extends BaseModifier_Plus {
    ...
    GetCastRangeBonusStacking(params: ModifierAbilityEvent) {
        ...
        else if (!IsValid(params.ability)) {
            if (IsValid(hCaster)) {
                return this.bonus_cast_range + hCaster.GetTalentValue("special_bonus_unique_vengefulspirit_custom_4")
            }
            return this.bonus_cast_range
...
export class modifier_vengefulspirit_3_attributes extends BaseModifier_Plus {
    ...
    Init(params: IModifierTable) {
        let hCaster = this.GetCasterPlus()
        this.primary_attributes = this.GetSpecialValueFor("primary_attributes") + (IsValid(hCaster) && hCaster.GetTalentValue("special_bonus_unique_vengefulspirit_custom_7") || 0)
        if (IsServer()) {

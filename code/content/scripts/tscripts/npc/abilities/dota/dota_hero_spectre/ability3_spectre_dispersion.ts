...
export class ability3_spectre_dispersion extends BaseAbility_Plus {
    ...
    CreateGhost(hTarget: IBaseNpc_Plus, fDelay: number = 0) {
        let hCaster = this.GetCasterPlus()
        let hHero = PlayerResource.GetSelectedHeroEntity(hCaster.GetPlayerOwnerID()) as unknown as IBaseNpc_Plus
        ...
        if (fDelay == null || fDelay <= 0) {
            ...
            modifier_spectre_3_ghost.apply(hIllusion, hCaster, this, { duration: illusion_duration, hTargetIndex: IsValid(hTarget) && hTarget.GetEntityIndex() || null })
...
export class modifier_spectre_3_buff extends BaseModifier_Plus {
    ...
    Init(params: IModifierTable) {
        ...
        if (IsServer()) {
            ...
            this.hTarget = EntIndexToHScript(params.target_index) as IBaseNpc_Plus
        }
    }

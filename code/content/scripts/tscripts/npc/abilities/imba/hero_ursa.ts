...
OnSpellStart():void {
    if (!this.GetCasterPlus().HasModifier("modifier_imba_earthshock_movement")) {
        ...
        let direction_vector = this.GetCasterPlus().GetForwardVector() * this.GetSpecialValueFor("hop_distance");
    ...
    if (IsServer()) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            if (!enemy.IsMagicImmune()) {
                let distance = (enemy.GetAbsOrigin() - caster.GetAbsOrigin()).Length2D();
...
BeCreated(p_0:any,):void {
    ...
    this.distance = (this.enemy.GetAbsOrigin() - this.caster.GetAbsOrigin()).Length2D();
...
BeDestroy():void {
    ...
    if (this.GetAbilityPlus() && this.GetAbilityPlus().ApplyEarthShock && this.GetRemainingTime() <= 0) {
        this.GetAbilityPlus().ApplyEarthShock();
    }
}
UpdateHorizontalMotion(me:CDOTA_BaseNPC, dt:number):void {
    ...
    me.SetAbsOrigin(me.GetAbsOrigin() + this.velocity * dt);
}
...
CC_OnUnitMoved(p_0:ModifierUnitEvent,):void {
    if (IsServer()) {
        if (this.caster.last_position == undefined) {
            this.caster.last_position = this.caster.GetAbsOrigin();
        } else {
            if (this.caster.GetAbsOrigin() - this.caster.last_position == Vector(0, 0, 0)) {
                return undefined;
            } else {
                this.caster.last_position = this.caster.GetAbsOrigin();
            }
...
OnSpellStart():void {
    ...
    if (ability.territorial_aura_modifier && !ability.territorial_aura_modifier.IsNull()) {
        if (ability.territorial_aura_modifier.ForceKill) {
            ability.territorial_aura_modifier.ForceKill(false);
        }
        if (ability.territorial_aura_modifier.RemoveSelf) {
    ...
    ability.territorial_aura_modifier.AddRangeIndicator(caster, ability, "vision_range", undefined, 200, 160, 100, true, true, false);
}
OnOwnerSpawned():void {
    if (!IsServer()) {
        return;

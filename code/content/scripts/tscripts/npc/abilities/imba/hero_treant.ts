...
OnSpellStart():void {
    ...
    if (cursor_position == cast_position) {
        cursor_position = cursor_position + this.GetCasterPlus().GetForwardVector();
    }
    ...
    for (let thicket = 1; thicket <= math.floor((this.GetCastRange(cursor_position, this.GetCasterPlus()) - 100) / this.GetSpecialValueFor("vine_spawn_interval")); thicket += 1) {
        this.GetCasterPlus().SetContextThink(DoUniqueString("grasp_thinker"), () => {
            ...
            }, GetGroundPosition(cast_position + (cursor_position - cast_position).Normalized() * (100 + (this.GetSpecialValueFor("vine_spawn_interval") * (thicket - 1))), undefined), this.GetCasterPlus().GetTeamNumber(), false);
...
BeCreated(keys:any):void {
    ...
    if (!this.GetAbilityPlus().thinker_tracker[this.unique_string] && GridNav.IsNearbyTree(this.GetParentPlus().GetAbsOrigin(), this.latch_range, false)) {
        this.GetAbilityPlus().thinker_tracker[this.unique_string] = true;
        this.bTouchingTree = true;
        for (const [_, ent] of ipairs(Entities.FindAllByName("npc_dota_thinker"))) {
            if (ent.HasModifier(this.GetName()) && ent.FindModifierByName(this.GetName()).unique_string == this.unique_string && ent.FindModifierByName(this.GetName()).bramble_particle) {
                ParticleManager.SetParticleControl(ent.FindModifierByName(this.GetName()).bramble_particle, 1, Vector(1, 0, 0));
                ent.FindModifierByName(this.GetName()).bTouchingTree = true;
            }
        }
        ParticleManager.SetParticleControl(this.bramble_particle, 1, Vector(1, 0, 0));
    } else if (this.GetAbilityPlus().thinker_tracker[this.unique_string]) {
        ParticleManager.SetParticleControl(this.bramble_particle, 1, Vector(1, 0, 0));
        this.bTouchingTree = true;
    }
}
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, ExtraData:any):boolean|void {
    target.Heal(this.GetTalentSpecialValueFor("leech_damage") * this.GetSpecialValueFor("damage_interval"), this.GetCasterPlus());
...
OnIntervalThink():void {
    this.GetParentPlus().Heal(this.heal_per_tick, this.GetCasterPlus());
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    if (this.GetAbilityPlus() && keys.target == this.GetParentPlus() && keys.damage > 0 && (keys.attacker.GetPlayerOwnerID() || keys.attacker.IsRoshan())) {
...
OnSpellStart():void {
    ...
    if (this.GetCasterPlus().HasScepter()) {
        ...
        for (const [_, ent] of ipairs(Entities.FindAllByName("npc_dota_treant_eyes"))) {

...
OnSpellStart():void {
    ...
    let distance = (caster.GetAbsOrigin() - target_point).Length2D();
...
OnProjectileHit(target:CDOTA_BaseNPC|undefined, location:Vector):boolean|void {
    ...
    let push_distance = (target.GetAbsOrigin() - target_point).Length2D();
    ...
    let bump_point = caster.GetAbsOrigin() + direction * (distance + 150);
...
OnSpellStart():void {
    ...
    EmitSoundOn(sound_cast, caster);
    if (USE_MEME_SOUNDS && RollPercentage(MEME_SOUNDS_CHANCE)) {
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            ...
            let distance = (enemy.GetAbsOrigin() - this.caster.GetAbsOrigin()).Length2D();
            if (distance > this.pull_prevent_radius) {
                let direction = (enemy.GetAbsOrigin() - this.caster.GetAbsOrigin()).Normalized();
                let pull_location = this.caster.GetAbsOrigin() + direction * (distance - this.pull_per_think);
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    if (IsServer()) {
        ...
        if (attacker == this.caster) {
            ApplyCausticFinale(attacker, target);
        }
    }
}
    ...
    if (IsServer()) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            ...
            if ((distance - this.pull_speed) > 50) {
                let pull_point = this.caster.GetAbsOrigin() + direction * (distance - this.pull_speed);
...
OnSpellStart():void {
    ...
    if (USE_MEME_SOUNDS && RollPercentage(MEME_SOUNDS_CHANCE) && this.sand_storm == undefined) {
...
OnIntervalThink():void {
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        if (!IsNearFountain(enemy.GetAbsOrigin(), 1200)) {
            let direction = (this.parent.GetAbsOrigin() - enemy.GetAbsOrigin()).Normalized();
            let pull_location = enemy.GetAbsOrigin() + (direction * this.pull_speed * frameTime);
            enemy.SetAbsOrigin(pull_location);
            ResolveNPCPositions(enemy.GetAbsOrigin(), enemy.GetHullRadius());
        }
    }
    ...
    if ((this.caster.GetAbsOrigin() - this.parent.GetAbsOrigin()).Length() <= this.sand_storm_radius) {

...
CreateLaser(start_pos, laser_direction) {
    if (IsServer()) {
        let caster = this.GetCasterPlus();
        let travel_speed = this.GetSpecialValueFor("travel_speed");
        let radius = this.GetSpecialValueFor("radius");
        let distance = this.GetCastRange(caster.GetAbsOrigin(), caster) + GetCastRangeIncrease(caster);
...
OnProjectileThink_ExtraData(location:Vector, extradata:any):void {
    if (IsServer()) {
        ...
        let projectile_distance = (location - start_pos).Length2D();
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let range = this.GetCastRange(caster_loc, caster) + GetCastRangeIncrease(caster);
...
OnProjectileThink_ExtraData(location:Vector, ExtraData:any):void {
    ...
    if (ExtraData.mini_missile_count && ExtraData.mini_missile_count > 0) {
        let distance = (location - Vector(ExtraData.cast_origin_x, ExtraData.cast_origin_y, 0)).Length2D();
        ...
        if ((math.mod(math.floor(distance), interval)) < math.floor(interval * FrameTime()) && (math.floor(distance - interval * FrameTime())) > math.floor(interval * FrameTime())) {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let caster_loc = caster.GetAbsOrigin();
        let direction = (target_loc - caster_loc).Normalized();
        let ability = this;
        let damage = this.GetSpecialValueFor("damage");
        ...
        let spawn_line_direction = RotateVector2D(direction, 90, true);
...
OnProjectileThink_ExtraData(location:Vector, ExtraData:any):void {
    if (IsServer()) {
        ...
        if (ExtraData.flame_duration) {
            this[ExtraData.index].counter = this[ExtraData.index].counter || 0;
            if (this[ExtraData.index].counter == 0) {
            ...
            this[ExtraData.index].counter = this[ExtraData.index].counter + 1;
            if (this[ExtraData.index].counter > 9) {
                this[ExtraData.index].counter = 0;
            }
        }
        if (ExtraData.railgun_damage) {
            ...
            if (this[ExtraData.index].counter == 0) {
                ...
                if (this[ExtraData.index].lightningBolt) {
                    ParticleManager.ReleaseParticleIndex(this[ExtraData.index].lightningBolt);
                    this[ExtraData.index].lightningBolt = undefined;
                }
                this[ExtraData.index].lightningBolt = ResHelper.CreateParticleEx("particles/units/heroes/hero_zuus/zuus_arc_lightning_.vpcf", ParticleAttachment_t.PATTACH_WORLDORIGIN, caster);
                ParticleManager.SetParticleControl(this[ExtraData.index].lightningBolt, 0, GetGroundPosition(location, caster));
                ParticleManager.SetParticleControl(this[ExtraData.index].lightningBolt, 1, GetGroundPosition(location, caster) + (Vector(ExtraData.direction_x, ExtraData.direction_y, 0) * ExtraData.railgun_range));
            }
            ParticleManager.SetParticleControl(this[ExtraData.index].lightningBolt, 0, GetGroundPosition(location, caster));
            this[ExtraData.index].counter = this[ExtraData.index].counter + 1;
            if (this[ExtraData.index].counter > 29) {
                this[ExtraData.index].counter = 0;
            }
        }
    }
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        let location = parent.GetAbsOrigin() + (direction * 1000);
        parent.Stop();

...
OnSpellStart():void {
    ...
    if (target.GetTeamNumber() != caster.GetTeamNumber()) {
        EmitSoundOn(responses_enemy[RandomInt(1, GameFunc.GetCount(responses_enemy))], caster);
    } else if (target == caster) {
        EmitSoundOn(responses_self[RandomInt(1, GameFunc.GetCount(responses_self))], caster);
    } else {
        EmitSoundOn(responses_friendly[RandomInt(1, GameFunc.GetCount(responses_friendly))], caster);
    }
...
BeCreated(p_0:any,):void {
    ...
    if (IsServer()) {
        ...
        ParticleManager.SetParticleControl(particle_disruption_fx, 0, this.parent.GetAbsOrigin() + Vector(0, 0, 100));
...
BeDestroy():void {
    ...
    if (IsServer()) {
        ...
        let illusions = CreateIllusions(this.caster, this.parent, {
        ...
        }, this.illusion_count, this.parent.GetHullRadius(), false, true);
...
BeCreated(params:any):void {
    ...
    if (params.soul_target) {
        this.target = EntIndexToHScript(params.soul_target);
...
OnSpellStart():void {
    ...
    if (RollPercentage(75)) {
        if (GameFunc.GetCount(enemies) > 0) {
            EmitSoundOn(responses_catch[RandomInt(1, GameFunc.GetCount(responses_catch))], caster);
        } else {
            EmitSoundOn(responses_miss[RandomInt(1, GameFunc.GetCount(responses_miss))], caster);
        }
    }
    if (GameFunc.GetCount(enemies) <= 0) {
        return;
...
BeCreated(params:any):void {
    ...
    this.AddTimer(FrameTime(), () => {
        this.parent.Heal(this.allied_heal, this.caster);
    });
}
/** DeclareFunctions():modifierfunction[] {
    let decFuncs = {
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    ...
    if (unit == this.parent && this.parent.IsRealHero()) {
        if (this.caster.HasAbility(this.shadow_poison_ability)) {
            let shadow_poison_ability_handle = this.caster.FindAbilityByName(this.shadow_poison_ability);
...
BeDestroy():void {
    ...
    if (!this.soul_taken) {
        ...
        this.parent.Heal(health_restore, this.caster);
    }
}
}
@registerAbility()
export class imba_shadow_demon_shadow_poison extends BaseAbility_Plus {
public shadow_poison_projectileID : any; 
GetCooldown(level:number):number {
...
FireShadowPoisonProjectile(origin_point, target_point:IBaseNpc_Plus, grudges) {
    let caster = this.GetCasterPlus();
    let ability = this;
    ...
    let direction = (target_point - origin_point).Normalized();
    ...
    table.insert(this.shadow_poison_projectileID, projectileID);
    this.shadow_poison_projectileID[projectileID] = {
        targets: 0,
        isGrudge: grudges
    }
}
OnProjectileHitHandle(target:IBaseNpc_Plus, location, projectileID) {
    if (!target) {
        table.remove(this.shadow_poison_projectileID, projectileID);
        return;
    }
...
OnOwnerSpawned():void {
    if (this.GetCasterPlus().HasTalent("special_bonus_imba_shadow_demon_shadow_poison_damage") && !this.GetCasterPlus().HasModifier("modifier_special_bonus_imba_shadow_demon_shadow_poison_damage")) {
        this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(), this.GetCasterPlus().findAbliityPlus("special_bonus_imba_shadow_demon_shadow_poison_damage"), "modifier_special_bonus_imba_shadow_demon_shadow_poison_damage", {});
    }
}
OnOwnerSpawned():void {
...
public particle_4stacks : number; 
...
public particle_4stacks_fx : number; 
...
BeDestroy():void {
    ...
    if (!this.parent.IsIllusion() && this.parent.IsAlive()) {
        ...
        damage = this.CalculateShadowPoisonDamage();
        let damageTable = {
            victim: this.parent,
            attacker: this.caster,
            damage: damage,
...
OnSpellStart():void {
    ...
    let illusions = FindUnitsInRadius(caster.GetTeamNumber(), caster.GetAbsOrigin(), undefined, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO, DOTADamageFlag_t.DOTA_DAMAGE_FLAG_NONE, FindOrder.FIND_ANY_ORDER, false);
    for (const [_, illusion] of ipairs(illusions)) {
...
OnSpellStart():void {
    ...
    EmitSoundOn(cast_sound, caster);
    if (RollPercentage(75)) {
        EmitSoundOn(cast_responses[RandomInt(1, GameFunc.GetCount(cast_responses))], caster);
    }
...
BeCreated(p_0:any,):void {
    ...
    let direction = (this.parent.GetAbsOrigin() - this.caster.GetAbsOrigin()).Normalized();
...
BeCreated(p_0:any,):void {
    ...
    if (!this.GetCasterPlus().HasModifier("modifier_generic_charges") && this.GetCasterPlus().HasAbility("imba_shadow_demon_disruption")) {
        ...
        let modifier_charge_handler = this.GetCasterPlus().findBuff<modifier_generic_charges>("modifier_generic_charges");

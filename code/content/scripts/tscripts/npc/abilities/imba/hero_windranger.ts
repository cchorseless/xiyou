...
SearchForShackleTarget(target:IBaseNpc_Plus, target_angle:IBaseNpc_Plus, ignore_list, target_count:IBaseNpc_Plus) {
    let shackleTarget = undefined;
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        if (enemy != target && !ignore_list[enemy] && math.abs(AngleDiff(target_angle, VectorToAngles(enemy.GetAbsOrigin() - target.GetAbsOrigin()).y)) <= this.GetSpecialValueFor("shackle_angle")) {
            shackleTarget = enemy;
    ...
    if (!shackleTarget && target.GetName() != "npc_dota_thinker") {
        ...
        for (const [_, tree] of ipairs(trees)) {
            if (!ignore_list[enemy] && math.abs(AngleDiff(target_angle, VectorToAngles(tree.GetAbsOrigin() - target.GetAbsOrigin()).y)) <= this.GetSpecialValueFor("shackle_angle")) {
                shackleTarget = tree;
                if (target.AddNewModifier) {
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, ExtraData:any):boolean|void {
    if (!ExtraData.bSplinterSister || ExtraData.bSplinterSister != 1) {
        ...
        for (let targets = 0; targets <= this.GetSpecialValueFor("shackle_count"); targets += 1) {
            if (next_target) {
                next_target = this.SearchForShackleTarget(next_target, VectorToAngles(next_target.GetAbsOrigin() - Vector(ExtraData.location_x, ExtraData.location_y, ExtraData.location_z)).y, shackled_targets, targets);
                if (next_target) {
...
FirePowershot(channel_pct, overstretch_bonus) {
    ...
    ProjectileManager.CreateLinearProjectile({
        ...
        vVelocity: (this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin()).Normalized() * this.GetSpecialValueFor("arrow_speed") * Vector(1, 1, 0),
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, data:any):boolean|void {
    if (target && data.dummy_index && EntIndexToHScript(data.dummy_index) && !EntIndexToHScript(data.dummy_index).IsNull() && EntIndexToHScript(data.dummy_index).units_hit) {
        EmitSoundOnLocationWithCaster(location, "Hero_Windrunner.PowershotDamage", this.GetCasterPlus());
        let damage = this.GetTalentSpecialValueFor("powershot_damage") * data.channel_pct * 0.01 * ((100 - this.GetSpecialValueFor("damage_reduction")) * 0.01) ^ EntIndexToHScript(data.dummy_index).units_hit;
        ...
        if (data.channel_pct >= this.GetSpecialValueFor("godshot_min") && data.channel_pct <= this.GetSpecialValueFor("godshot_max")) {
            ...
            damage = this.GetTalentSpecialValueFor("powershot_damage") * this.GetSpecialValueFor("scattershot_damage_pct") * 0.01 * ((100 - this.GetSpecialValueFor("damage_reduction")) * 0.01) ^ EntIndexToHScript(data.dummy_index).units_hit;
        }
        ApplyDamage({
            victim: target,
            damage: damage,
...
OnIntervalThink():void {
    if (this.GetAbilityPlus()) {
        ...
        this.GetAbilityPlus().FirePowershot(this.channel_pct);
    }
}
}
@registerModifier()
...
BeDestroy():void {
    ...
    this.GetParentPlus().SetCursorPosition(this.GetParentPlus().GetAbsOrigin() + this.GetParentPlus().GetForwardVector());
    this.GetAbilityPlus().FirePowershot(1, this.GetStackCount() * this.overstretch_bonus_range_per_second);
}
CheckState():Partial<Record<modifierstate,boolean>> {
    return {
...
CC_OnAbilityFullyCast(keys:ModifierAbilityEvent):void {
    if (keys.unit == this.GetParentPlus() && keys.ability.GetName() == "windrunner_focusfire") {
        this.ability = keys.ability;
        this.target = keys.ability.GetCursorTarget();
    }
}
@registerEvent(Enum_MODIFIER_EVENT.ON_ATTACK_LANDED)
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    if (keys.attacker == this.GetParentPlus() && this.GetParentPlus().HasModifier("modifier_windrunner_focusfire") && this.target && !this.target.IsNull() && this.target.IsAlive() && this.target == keys.target && GFuncRandom.PRD(this.ability.GetSpecialValueFor("ministun_chance"), )) {
...
OnIntervalThink():void {
    if (this.GetParentPlus().AttackReady() && this.target && !this.target.IsNull() && this.target.IsAlive() && (this.target.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D() <= this.GetParentPlus().Script_GetAttackRange() && this.bFocusing) {

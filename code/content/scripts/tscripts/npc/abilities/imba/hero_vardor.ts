...
PierceTargetLocation(target_point:IBaseNpc_Plus, is_charge_yari) {
    let caster = this.GetCasterPlus();
    let ability = this;
    ...
    let spear_duration = ability.GetSpecialValueFor("spear_duration");
    let dummy = BaseNpc_Plus.CreateUnitByName("npc_dota_vardor_spear_dummy", target_point, false, caster, caster, caster.GetTeamNumber());
...
public yariHolder : any; 
public yariManager : any; 
IsHidden():boolean {
    return false;
}
IsPurgable():boolean {
...
BeCreated(p_0:any,):void {
    if (IsServer()) {
        ...
        let yariHolder = BaseNpc_Plus.CreateUnitByName("spearholder_unit", this.caster.GetAbsOrigin(), false, this.caster, this.caster, this.caster.GetTeam());
...
OnStackCountChanged(previous_stacks:number):void {
    ...
    if (this.GetStackCount() == 0) {
        ...
        let ret = 1;
        if (caster.oppressReturn) {
            if (caster.oppressReturn == true) {
                ret = 0;
            }
        }
...
BeCreated(event:any):void {
    if (IsServer()) {
        this.target = EntIndexToHScript(event.EntIndex);
        this.StartIntervalThink(0.005);
        this.particles = {}
...
ChangeCount(newCount, reapply) {
    ...
    let ret = 1;
    if (this.target.oppressReturn) {
        if (this.target.oppressReturn == true) {
            ret = 0;
        }
    }
    let startIndex = 2;
...
OnIntervalThink():void {
    if (IsServer()) {
        if (this.target) {
            ...
            let distance = (oldLoc - targetLoc).Length2D();
            if (distance > 600) {
                this.Hide(true);
            }
...
CastFilterResultLocation(location:Vector):UnitFilterResult {
    ...
    let caster = this.GetCasterPlus();
    let ability = this;
    let cast_range = this.GetCastRange(location, target);
...
OnSpellStart():void {
    ...
    if (!target) {
        ...
        DisplayError(this.GetCasterPlus().GetPlayerID(), "#dota_hud_error_vardor_no_yaris_in_range");
        return;
    }
...
OnProjectileHit(target:CDOTA_BaseNPC|undefined, location:Vector):boolean|void {
    ...
    let yariModifier = caster.FindModifierByName(modifier_charges);
    if (yariModifier) {
...
BeCreated(p_0:any,):void {
    ...
    if (yariModifier) {
        this.GetParentPlus().oppressReturn = true;
        if (yariModifier.yariManager) {

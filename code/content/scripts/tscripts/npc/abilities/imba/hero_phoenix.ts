...
OnSpellStart():void {
    ...
    let _direction = (target_point - caster.GetAbsOrigin()).Normalized();
    ...
    caster.SetContextThink(DoUniqueString("updateIcarusDive"), () => {
        ...
        if (imba_phoenix_check_for_canceled(caster)) {
        ...
        let pos = ellipseCenter + rightDir * x + forwardDir * y;
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            if (enemy != caster) {
                ...
                if (caster.HasTalent("special_bonus_imba_phoenix_2") && caster.GetTeamNumber() != enemy.GetTeamNumber()) {
                    let item = CreateItem("item_imba_dummy", caster, caster);
    ...
    if (caster.HasModifier("modifier_imba_phoenix_burning_wings_buff")) {
        caster.Heal(this.healthCost, caster);
...
BeDestroy():void {
    ...
    let caster = this.GetCasterPlus();
    let pfx = caster.fire_spirits_pfx;
    if (pfx) {
        ParticleManager.DestroyParticle(pfx, false);
        ParticleManager.ReleaseParticleIndex(pfx);
    }
...
OnSpellStart():void {
    ...
    if (!caster.HasTalent("special_bonus_imba_phoenix_7")) {
        ...
        for (let i = 1; i <= caster.fire_spirits_numSpirits; i += 1) {
            let radius = 0;
            if (i <= currentStack) {
                radius = 1;
            }
    ...
    let DummyUnit = BaseNpc_Plus.CreateUnitByName("npc_dummy_unit", point, false, caster, caster.GetOwnerPlus(), caster.GetTeamNumber());
...
OnProjectileThink(vLocation:Vector):void {
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        if (enemy.GetTeamNumber() != caster.GetTeamNumber()) {
            ...
            enemy.AddNewModifier(caster, ability, "modifier_imba_phoenix_burning_wings_ally_buff", {
                duration: 0.2
            });
        }
    }
...
OnProjectileHit(hTarget:CDOTA_BaseNPC|undefined, vLocation:Vector):boolean|void {
    ...
    let DummyUnit = BaseNpc_Plus.CreateUnitByName("npc_dummy_unit", location, false, caster, caster.GetOwnerPlus(), caster.GetTeamNumber());
    DummyUnit.AddNewModifier(caster, ability, "modifier_kill", {
        duration: 0.1
    });
...
OnIntervalThink():void {
    ...
    let heal_amp = 1 + (caster.GetSpellAmplification(false) * 0.01);
    dmg = dmg * heal_amp;
    this.GetParentPlus().Heal(dmg * this.GetStackCount(), caster);
...
OnSpellStart():void {
    ...
    let casterOrigin = caster.GetAbsOrigin();
    caster.AddNewModifier(caster, ability, modifierCasterName, {
    ...
    caster.SetContextThink(DoUniqueString("updateSunRay"), () => {
        if (this.GetCasterPlus().HasModifier("modifier_mars_arena_of_blood_leash") && this.GetCasterPlus().findBuff<modifier_mars_arena_of_blood_leash>("modifier_mars_arena_of_blood_leash").GetAuraOwner() && (this.GetCasterPlus().GetAbsOrigin() - this.GetCasterPlus().FindModifierByName("modifier_mars_arena_of_blood_leash").GetAuraOwner().GetAbsOrigin()).Length2D() >= this.GetCasterPlus().FindModifierByName("modifier_mars_arena_of_blood_leash").GetSpecialValueFor("radius") - this.GetCasterPlus().FindModifierByName("modifier_mars_arena_of_blood_leash").GetSpecialValueFor("width")) {
            this.GetCasterPlus().RemoveModifierByName("modifier_imba_phoenix_sun_ray_caster_dummy");
        }
        ParticleManager.SetParticleControl(pfx, 0, caster.GetAttachmentOrigin(attach_point));
        if ((imba_phoenix_check_for_canceled(caster) && ((!this.GetCasterPlus().HasScepter()) || (this.GetCasterPlus().HasScepter() && !this.GetCasterPlus().HasModifier("modifier_imba_phoenix_supernova_caster_dummy")))) || caster.IsSilenced() || caster.HasModifier("modifier_legion_commander_duel") || caster.HasModifier("modifier_lone_druid_savage_roar")) {
        ...
        if (caster.sun_ray_is_moving && !GameRules.IsGamePaused()) {
        ...
        let endcapPos = casterOrigin + casterForward * pathLength;
        ...
        let units = FindUnitsInLine(caster.GetTeamNumber(), caster.GetAbsOrigin() + caster.GetForwardVector() * 32, endcapPos, undefined, ability.GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NONE);
        for (const [_, unit] of ipairs(units)) {
            unit.AddNewModifier(caster, ability, "modifier_imba_phoenix_sun_ray_dummy_buff", {
...
BeDestroy():void {
    ...
    if (caster.HasTalent("special_bonus_imba_phoenix_4")) {
        let endcapPos = caster.GetAbsOrigin() + caster.GetForwardVector() * ability.GetSpecialValueFor("beam_range");
        let units = FindUnitsInLine(caster.GetTeamNumber(), caster.GetAbsOrigin() + caster.GetForwardVector() * 32, endcapPos, undefined, ability.GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NONE);
        for (const [_, unit] of ipairs(units)) {
            if (unit != caster) {
...
OnIntervalThink():void {
    ...
    if (taker != this.GetCasterPlus()) {
        taker.Heal(total_heal, this.GetCasterPlus());
...
OnSpellStart():void {
    ...
    caster.AddNoDraw();
    let egg = BaseNpc_Plus.CreateUnitByName("npc_dota_phoenix_sun", ground_location, false, caster, caster.GetOwnerPlus(), caster.GetTeamNumber());
    ...
    caster.ally = this.GetCursorTarget();
    if (caster.ally == caster) {
        caster.ally = undefined;
    } else {
        let ally = caster.ally;
        if (!caster.HasTalent("special_bonus_imba_phoenix_6")) {
            ...
            ally.AddNoDraw();
            let _direction = (ally.GetAbsOrigin() - caster.GetAbsOrigin()).Normalized();
            caster.SetForwardVector(_direction);
            let loaction = caster.GetForwardVector() * 192 + caster.GetAbsOrigin();
            let egg2 = BaseNpc_Plus.CreateUnitByName("npc_dota_phoenix_sun", loaction, false, ally, ally.GetOwnerPlus(), ally.GetTeamNumber());
...
BeDestroy():void {
    ...
    if (this.GetCasterPlus() == this.GetParentPlus()) {
        for (const [_, ability] of ipairs(this.abilities)) {
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    ...
    if (egg.IsDoubleNova) {
        egg.IsDoubleNova = undefined;
    }
    if (egg.NovaCaster) {
        egg.NovaCaster = undefined;
    }
    caster.RemoveNoDraw();
    if (caster.ally && !caster.HasDoubleEgg) {
    ...
    if (egg == killer) {
        ...
        if (caster.ally && !caster.HasDoubleEgg && caster.ally.IsAlive()) {
        ...
        if (!caster.HasTalent("special_bonus_imba_phoenix_5")) {
            ...
            if (caster.ally && caster.ally.IsAlive()) {
                caster.ally.SetHealth(caster.ally.GetMaxHealth() * caster.GetTalentValue("special_bonus_imba_phoenix_5", "reborn_pct") / 100);
                let egg_buff2 = caster.ally.FindModifierByNameAndCaster("modifier_imba_phoenix_supernova_caster_dummy", caster);
                if (egg_buff2) {
                    egg_buff2.Destroy();
                }
            }
...
CC_OnTakeDamage(keys:ModifierInstanceEvent):void {
    ...
    if (this.GetCasterPlus().GetHealth() <= 1) {
        ...
        caster.AddNoDraw();
        let egg = BaseNpc_Plus.CreateUnitByName("npc_dota_phoenix_sun", location, false, caster, caster.GetOwnerPlus(), caster.GetTeamNumber());
        egg.AddNewModifier(caster, ability, "modifier_kill", {
            duration: egg_duration + extend_duration
        });
        egg.AddNewModifier(caster, ability, "modifier_imba_phoenix_supernova_egg_thinker", {

...
OnSpellStart( /** params */  /** , reduce_pct */  /** , target_loc */  /** , ix */ ):void {
    if (IsServer()) {
        let caster = this.GetCasterPlus();
        let target;
        if (params) {
        ...
        if (ix) {
            ...
            this[index] = 0;
            this[proj_index] = {}
            table.insert(this[proj_index], target);
        }
        if (reduce_pct) {
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, ExtraData:any):boolean|void {
    if (IsServer()) {
        ...
        if (target) {
            if (GameFunc.GetCount(this[proj_index]) == 1) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            ...
            for (const [_, stored_target] of ipairs(this[proj_index])) {
        ...
        if (target) {
            for (let i = 1; i <= target_missiles; i += 1) {
                ...
                table.insert(this[proj_index], valid_targets[i]);
            }
        }
        if (this[ExtraData.index] == GameFunc.GetCount(this[proj_index])) {
            this[ExtraData.index] = undefined;
            this[proj_index] = undefined;
        }
    }
}
OnOwnerSpawned():void {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let primary_distance = this.GetCastRange(caster_loc, caster) + GetCastRangeIncrease(caster);
...
OnOwnerDied():void {
    if (this.IsTrained() && !this.GetCasterPlus().IsIllusion() && !this.GetCasterPlus().PassivesDisabled()) {
        ...
        let super_illusions = CreateIllusions(this.GetCasterPlus(), this.GetCasterPlus(), {
            ...
            duration: undefined
        }, num_illusions_on_death, this.GetCasterPlus().GetHullRadius(), true, true);
        for (const [_, illusion] of ipairs(super_illusions)) {
            ...
            FindClearSpaceForUnit(illusion, this.GetCasterPlus().GetAbsOrigin() + Vector(RandomInt(0, 1), RandomInt(0, 1), 0) * 108, true);
    ...
    if (keys.unit == this.GetParentPlus() && keys.unit.IsRealHero()) {
        ...
        this.GetCasterPlus().SetContextThink(DoUniqueString(this.GetName()), () => {
            for (const [_, unit] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetCasterPlus().GetAbsOrigin(), undefined, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD, FindOrder.FIND_ANY_ORDER, false))) {
                if (unit.GetName() == this.GetCasterPlus().GetName() && unit != this.GetCasterPlus() && unit.GetOwnerPlus() && unit.GetOwnerPlus().GetAssignedHero() && unit.GetOwnerPlus().GetAssignedHero() == this.GetCasterPlus()) {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        if ((caster_loc - fountain_loc).Length2D() < 1700) {

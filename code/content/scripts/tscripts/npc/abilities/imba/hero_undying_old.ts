...
public debuff_modifier_table : any; 
GetAOERadius():number {
...
OnInventoryContentsChanged():void {
    if (this.GetCasterPlus().HasScepter() && !this.scepter_updated) {
        for (const [_, mod] of ipairs(this.GetCasterPlus().FindAllModifiersByName("modifier_imba_undying_decay_buff"))) {
...
OnSpellStart():void {
    ...
    for (const [_, enemy] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetCursorPosition(), undefined, this.GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NONE, FindOrder.FIND_ANY_ORDER, false))) {
        if (enemy.IsClone() || enemy.IsTempestDouble()) {
            if (enemy.GetPlayerOwner && enemy.GetPlayerOwner().GetAssignedHero && enemy.GetPlayerOwner().GetAssignedHero().entindex()) {
                if (!clone_owner_units[enemy.GetPlayerOwner().GetAssignedHero().entindex()]) {
    ...
    if (GameFunc.GetCount(clone_owner_units) > 0) {
        for (const [tables] of clone_owner_units) {
            enemy.EmitSound("Hero_Undying.Decay.Target");
            this.GetCasterPlus().EmitSound("Hero_Undying.Decay.Transfer");
            selected_unit = EntIndexToHScript(tables[RandomInt(1, GameFunc.GetCount(tables))]);
...
BeCreated(p_0:any,):void {
    ...
    this.GetCasterPlus().Heal(this.strength_gain * 20, this.GetCasterPlus());
}
BeDestroy():void {
    if (!IsServer()) {
        return;
...
BeDestroy():void {
    ...
    if (this.GetAbilityPlus() && this.GetAbilityPlus().debuff_modifier_table) {
        Custom_ArrayRemove(this.GetAbilityPlus().debuff_modifier_table, function (i, j) {
            return this.GetAbilityPlus().debuff_modifier_table[i] != this;
        });
    }
}
OnStackCountChanged(stackCount:number):void {
...
OnSpellStart():void {
    ...
    if (units_ripped >= 1) {
        if (target.GetTeamNumber() != this.GetCasterPlus().GetTeamNumber() && !target.TriggerSpellAbsorb(this)) {
            ...
            target.Heal(this.GetSpecialValueFor("damage_per_unit") * units_ripped, this.GetCasterPlus());
            ...
            target.EmitSound("Hero_Undying.SoulRip.Ally");
            target.Heal(this.GetSpecialValueFor("tombstone_heal"), this.GetCasterPlus());
...
public responses : any; 
public responses_big : any; 
OnSpellStart( /** deathPosition */ ):void {
    ...
    let tombstone = BaseNpc_Plus.CreateUnitByName("npc_dota_unit_tombstone" + this.GetLevel(), position, true, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
...
OnIntervalThink():void {
    ...
    for (const [_, enemy] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetParentPlus().GetAbsOrigin(), undefined, this.radius, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NO_INVIS + DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FindOrder.FIND_ANY_ORDER, false))) {
        if (!enemy.IsCourier() && enemy.GetName() != "npc_dota_unit_undying_zombie") {
            zombie = BaseNpc_Plus.CreateUnitByName(this.zombie_types[RandomInt(1, GameFunc.GetCount(this.zombie_types))], enemy.GetAbsOrigin(), true, this.GetParentPlus(), this.GetParentPlus(), this.GetCasterPlus().GetTeamNumber());
...
BeDestroy():void {
    ...
    for (const [_, ent] of ipairs(Entities.FindAllByName("npc_dota_unit_undying_zombie"))) {
...
BeCreated(keys:any):void {
    ...
    this.target = EntIndexToHScript(keys.target_entindex);
}
}
@registerModifier()
...
OnIntervalThink():void {
    if (!this.GetCasterPlus() || this.GetCasterPlus().IsStunned() || this.GetCasterPlus().IsHexed() || this.GetCasterPlus().IsNightmared() || this.GetCasterPlus().IsOutOfGame() || !this.GetCasterPlus().HasModifier("modifier_imba_undying_flesh_golem") || (this.GetCasterPlus().GetAbsOrigin() - this.position).Length2D() > this.blink_break_range) {
        this.Destroy();
    }
    if (this.GetCasterPlus().GetAggroTarget() != this.GetParentPlus()) {
        this.GetParentPlus().SetAbsOrigin(this.GetCasterPlus().GetAttachmentOrigin(this.GetCasterPlus().ScriptLookupAttachment("attach_attack1")) - Vector(0, 0, 50));
    } else {
        this.GetParentPlus().SetAbsOrigin(this.GetCasterPlus().GetAbsOrigin() + (this.GetCasterPlus().GetForwardVector() * 50));
    }
    this.position = this.GetCasterPlus().GetAbsOrigin();
}
BeDestroy():void {
...
OnSpellStart():void {
    if (this.GetCasterPlus().HasModifier("modifier_imba_undying_flesh_golem_grab") && this.GetCasterPlus().findBuff<modifier_imba_undying_flesh_golem_grab>("modifier_imba_undying_flesh_golem_grab").target) {
        ...
        let knockback_modifier = target.AddNewModifier(this.GetCasterPlus(),this, "modifier_generic_motion_controller", {
            distance: (this.GetCursorPosition() - target.GetAbsOrigin()).Length2D(),
            direction_x: (this.GetCursorPosition() - target.GetAbsOrigin()).Normalized().x,
            direction_y: (this.GetCursorPosition() - target.GetAbsOrigin()).Normalized().y,
            direction_z: (this.GetCursorPosition() - target.GetAbsOrigin()).Normalized().z,
            duration: (this.GetCursorPosition() - target.GetAbsOrigin()).Length2D() / this.GetSpecialValueFor("throw_speed"),
            height: this.GetSpecialValueFor("throw_max_height") * ((this.GetCursorPosition() - target.GetAbsOrigin()).Length2D() / super.GetCastRange(this.GetCursorPosition(), target)),
            bGroundStop: false,
            bDecelerate: false,
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    if (keys.unit == this.GetParentPlus() && !keys.reincarnate) {
        ...
        if (keys.unit.IsRealHero()) {
            this.GetCasterPlus().Heal(this.GetCasterPlus().GetMaxHealth() * this.remnants_max_health_heal_pct_hero * 0.01, this.GetCasterPlus());
            ...
            this.GetCasterPlus().Heal(this.GetCasterPlus().GetMaxHealth() * this.remnants_max_health_heal_pct_non_hero * 0.01, this.GetCasterPlus());

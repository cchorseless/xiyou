...
OnSpellStart():void {
    if (IsServer()) {
        ...
        this.AddTimer(() => {
            ...
            for (const [_, unit] of ipairs(units)) {
                if (unit != previous_unit && unit.damaged_by_fade_bolt != true) {
                    current_target = unit;
                    return;
                }
            }
...
CC_OnModifierAdded(keys:ModifierAddedEvent):void {
    if (this.GetAbilityPlus() && !this.GetCasterPlus().HasModifier("modifier_imba_rubick_arcane_supremacy_flip_aura") && this.GetCasterPlus().GetPlayerID && keys.issuer_player_index == this.GetCasterPlus().GetPlayerID() && keys.unit && keys.unit.GetTeamNumber() != this.GetCasterPlus().GetTeamNumber() && keys.unit.FindAllModifiers) {
        for (const [_, modifier] of ipairs(keys.unit.FindAllModifiers())) {
...
SetLastSpell(hHero, hSpell) {
    ...
    for (const [_, data] of ipairs(imba_rubick_spellsteal.heroesData)) {
...
PrintStatus() {
    ...
    for (const [_, heroData] of ipairs(imba_rubick_spellsteal.heroesData)) {
...
SetStolenSpell(spellData) {
    if (!spellData) {
        return;
    }
    let primarySpell = spellData.primarySpell;
    let secondarySpell = spellData.secondarySpell;
    let linkedTalents = spellData.linkedTalents;
    ...
    for (const [_, talent] of ipairs(linkedTalents)) {
    ...
    if (this.GetCasterPlus().HasAbility("monkey_king_primal_spring_early")) {
        this.GetCasterPlus().findAbliityPlus<monkey_king_primal_spring_early>("monkey_king_primal_spring_early").SetHidden(true);
    }
}
ForgetSpell() {
    if (this.CurrentSpellOwner != undefined) {
        for (let i = 0; i <= this.GetCasterPlus().GetModifierCount() - 1; i += 1) {
            if (string.find(this.GetCasterPlus().GetModifierNameByIndex(i), string.gsub(this.CurrentSpellOwner, "npc_dota_hero_", ""))) {
        ...
        for (let i = 0; i <= this.GetCasterPlus().GetAbilityCount() - 1; i += 1) {
            let talent = this.GetCasterPlus().FindAbilityByName("special_bonus_imba_" + string.gsub(this.CurrentSpellOwner, "npc_dota_hero_", "") + "_" + i);
            if (talent) {
...
CC_OnAbilityExecuted(params:ModifierAbilityEvent):void {
    if (IsServer()) {
        ...
        this.GetAbilityPlus().SetLastSpell(params.unit, params.ability);
    }
}
}
@registerModifier()
...
BeDestroy( /** kv */ ):void {
    ...
    this.GetAbilityPlus().ForgetSpell();
}
/** DeclareFunctions():modifierfunction[] {
    return Object.values({
...
CC_OnAbilityStart(params:ModifierAbilityEvent):void {
    if (IsServer()) {
        if (params.unit == this.GetParentPlus()) {
            if (bit.band(params.ability.GetBehavior(), DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_NORMAL_WHEN_STOLEN) != DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_NORMAL_WHEN_STOLEN) {
                params.ability.SetOverrideCastPoint(0);
            }
            if (params.ability == this.GetAbilityPlus().currentSpell) {
...
BeCreated(kv:any):void {
    if (IsServer()) {
        ...
        this.SetStackCount(this.GetAbilityPlus().animations.GetCurrentReference());
    }
    if (!IsServer()) {
        this.GetAbilityPlus().animations.SetCurrentReferenceIndex(this.GetStackCount());
    }
}
BeRefresh(kv:any):void {
}
...
GetAuraRadius():number {
    if (this.GetCasterPlus().FindTalentValue) {
...
GetAuraEntityReject(target:CDOTA_BaseNPC):boolean {
    return !this.GetCasterPlus().FindTalentValue || this.GetCasterPlus().HasModifier("modifier_imba_rubick_arcane_supremacy_flip_aura");
}
}
@registerModifier()

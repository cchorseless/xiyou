...
OnSpellStart():void {
    ...
    if (RollPercentage(5)) {
        EmitSoundOn(rare_cast_response[math.random(1, GameFunc.GetCount(rare_cast_response))], caster);
    } else if (RollPercentage(75)) {
        EmitSoundOn(cast_response[math.random(1, GameFunc.GetCount(cast_response))], caster);
    }
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, extra_data:any):boolean|void {
    ...
    if (caster.HasTalent("special_bonus_imba_skeleton_king_7") && !bTalent) {
        let direction = (target.GetAbsOrigin() - caster.GetAbsOrigin()).Normalized();
        let distance = (target.GetAbsOrigin() - caster.GetAbsOrigin()).Length2D();
        let summon_point = caster.GetAbsOrigin() + direction * distance - 100;
        let wraith = BaseNpc_Plus.CreateUnitByName("npc_imba_wraith_king_wraith", summon_point, true, caster, caster, caster.GetTeamNumber());
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    ...
    if (this.parent == target) {
        ...
        attacker.Heal(heal_amount, this.caster);
    }
}
}
@registerModifier()
    ...
    if (IsServer()) {
        ...
        if (this.parent == attacker) {
            ...
            if (damage_type == DAMAGE_TYPES.DAMAGE_TYPE_PHYSICAL) {
                ...
                this.parent.Heal(heal_amount, this.caster);
            } else {
        ...
        for (let unit = 0; unit <= skeleton_modifier.GetStackCount() - 1; unit += 1) {
            this.GetCasterPlus().SetContextThink(DoUniqueString(this.GetName()), () => {
                for (let units_per_charge = 1; units_per_charge <= this.skeletons_per_charge; units_per_charge += 1) {
                    skeleton = BaseNpc_Plus.CreateUnitByName("npc_dota_wraith_king_skeleton_warrior", this.caster.GetAbsOrigin() + RandomVector(100), true, this.caster, this.caster, this.caster.GetTeamNumber());
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    ...
    if (this.parent == keys.unit && this.GetParentPlus().fresh && this.parent != keys.attacker) {
        this.remaining_time = math.max(this.parent.findBuff<modifier_kill>("modifier_kill").GetRemainingTime() - this.reincarnate_time, 0);
        this.StartIntervalThink(this.reincarnate_time);
    }
}
OnIntervalThink():void {
    this.skeleton = BaseNpc_Plus.CreateUnitByName("npc_dota_wraith_king_skeleton_warrior", this.parent.GetOrigin(), true, this.caster, this.caster, this.caster.GetTeamNumber());
    ...
    this.skeleton.fresh = false;
    this.skeleton.SetContextThink(DoUniqueString(this.GetName()), () => {
...
CC_GetModifierPreAttack_CriticalStrike(keys:ModifierAttackEvent):number {
    if (IsServer()) {
        ...
        if (attacker == this.caster) {
            ...
            if (GFuncRandom.PRD(this.crit_chance, )) {
...
TheWillOfTheKing(OnDeathKeys, BuffInfo) {
    ...
    if (reincarnate && (!BuffInfo.caster.HasModifier("modifier_item_imba_aegis"))) {
        ...
        if (BuffInfo.caster == unit) {
            ...
            if (USE_MEME_SOUNDS && GameFunc.GetCount(heroes) >= PlayerResource.GetPlayerCount() * 0.35) {
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    if (IsServer()) {
        ...
        if (this.GetParentPlus() == unit) {
            imba_wraith_king_reincarnation.TheWillOfTheKing(keys, );
        }
    }
}
GetAuraRadius():number {
...
CC_OnTakeDamage(keys:ModifierInstanceEvent):void {
    ...
    if (keys.damage_type == DAMAGE_TYPES.DAMAGE_TYPE_PHYSICAL) {
        ...
        let actually_dmg = source_dmg * multiplier;
        this.damage_pool = this.damage_pool + actually_dmg;
    } else if (keys.damage_type != DAMAGE_TYPES.DAMAGE_TYPE_PHYSICAL && keys.damage_type != DAMAGE_TYPES.DAMAGE_TYPE_MAGICAL) {
...
BeDestroy():void {
    if (IsServer()) {
        TrueKill(this.original_killer, this.parent, this.ability_killer);
...
OnSpellStart():void {
    let keys = this.GetCasterPlus().FindModifierByNameAndCaster("modifier_imba_kingdom_come", this.GetCasterPlus());
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    if (IsServer() && !this.GetParentPlus().PassivesDisabled()) {
        ...
        if (this.caster == unit && this.caster.IsRealHero()) {
            let info = this.GetCasterPlus().FindModifierByNameAndCaster("modifier_imba_kingdom_come", this.GetCasterPlus());
...
BeDestroy():void {
    if (IsServer()) {
        let distance = (this.position - this.parent.GetAbsOrigin()).Length2D();
        ...
        if (this.parent.IsRealHero()) {
            ...
            let distance = (this.parent.GetAbsOrigin() - this.caster.GetAbsOrigin()).Length2D();
            let summon_point = this.caster.GetAbsOrigin() + direction * distance - 100;
            let wraith = BaseNpc_Plus.CreateUnitByName("npc_imba_wraith_king_wraith", summon_point, true, this.caster, this.caster, this.caster.GetTeamNumber());

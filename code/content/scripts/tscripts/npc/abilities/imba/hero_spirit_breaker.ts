...
export class modifier_imba_spirit_breaker_charge_of_darkness extends BaseModifierMotionHorizontal_Plus {
    ...
    public bashed_enemies: IBaseNpc_Plus[];
    public trees: CDOTA_MapTree[];
    public darkness_counter: number;
    public attempting_to_board: IBaseNpc_Plus[];
    ...
    UpdateHorizontalMotion(me: CDOTA_BaseNPC, dt: number): void {
        ...
        if (!this.GetAbilityPlus()) {
            this.GetAbilityPlus<imba_spirit_breaker_charge_of_darkness>().charge_cancel_reason = "Ability Does Not Exist";
            this.Destroy();
            return;
        }
        if (!this.target.IsAlive()) {
            ...
            if (GameFunc.GetCount(new_targets) == 0) {
                this.GetAbilityPlus<imba_spirit_breaker_charge_of_darkness>().charge_cancel_reason = "Primary target dead; no other valid targets within 4000 radius";
                this.Destroy();
                return;
            }
            ...
            if (!this.target) {
                this.GetAbilityPlus<imba_spirit_breaker_charge_of_darkness>().charge_cancel_reason = "Primary target dead; only valid target within 4000 radius is being Clotheslined";
                this.Destroy();
                return;
            }
        }
        ...
        for (const enemy of (enemies as IBaseNpc_Plus[])) {
            if (this.GetAbilityPlus().GetAutoCastState() && !this.clothesline_target && enemy != this.target && !enemy.IsRoshan() && enemy.IsRealUnit()) {
                ...
                this.bashed_enemies.push(enemy);
            } else if (greater_bash_ability && greater_bash_ability.IsTrained() && enemy != this.target && !this.bashed_enemies.includes(enemy)) {
        ...
        for (const [_, ally] of GameFunc.iPair(allies)) {
            if (ally != this.GetParentPlus() && (this.attempting_to_board.includes(ally) || (taxi_tracker_modifier && taxi_tracker_modifier.attempting_to_board.includes(ally)))) {
                ...
                }) as modifier_imba_spirit_breaker_charge_of_darkness_taxi;
                ...
                GFuncArray.ArrayRemove(this.attempting_to_board, ally);
                if (taxi_tracker_modifier && taxi_tracker_modifier.attempting_to_board.includes(ally)) {
                    GFuncArray.ArrayRemove(taxi_tracker_modifier.attempting_to_board, ally);
                }
            }
        }
        ...
        let ability = this.GetAbilityPlus<imba_spirit_breaker_charge_of_darkness>();
        if ((this.target.GetOrigin() - me.GetOrigin() as Vector).Length2D() <= 128) {
            ...
            if (greater_bash_ability && greater_bash_ability.IsTrained()) {
                ...
                if (GameServiceConfig.USE_MEME_SOUNDS && ability) {
                    if (RollPercentage(10)) {
                        this.GetCasterPlus().EmitSound("Hero_Spirit_Breaker.FryingPan");
                        ability.frying_pan = true;
                        ability.timer = GameRules.GetDOTATime(true, true);
                    } else {
                        ability.frying_pan = false;
                        ability.timer = undefined;
                    }
                }
            }
            ...
            if (this.target.IsAlive()) {
                ability.charge_cancel_reason = "Charge connected with target and they were still alive after impact";
                me.SetAggroTarget(this.target);
                this.Destroy();
            }
            return;
        } else if (me.IsStunned() || me.IsOutOfGame() || me.IsHexed() || me.IsRooted()) {
            ability.charge_cancel_reason = "Caster was disabled mid-charge";
            this.Destroy();
            return;
        }
        me.FaceTowards(this.target.GetOrigin());
        let distance = (GetGroundPosition(this.target.GetOrigin(), undefined) - GetGroundPosition(me.GetOrigin(), undefined) as Vector).Normalized();
        me.SetOrigin(me.GetOrigin() + distance * me.GetIdealSpeed() * dt as Vector);
    ...
    OnHorizontalMotionInterrupted(): void {
        this.GetAbilityPlus<imba_spirit_breaker_charge_of_darkness>().charge_cancel_reason = "Horizontal Motion Interrupted";
        this.Destroy();
    }
    BeDestroy(): void {
        if (!IsServer()) {
    ...
    CC_OnOrder(keys: ModifierAbilityEvent): void {
        ...
        if (keys.unit == this.GetParentPlus()) {
            ...
            if (cancel_commands.includes(keys.order_type)) {
                this.GetAbilityPlus<imba_spirit_breaker_charge_of_darkness>().charge_cancel_reason = "Cancel Order Issued: " + keys.order_type;
                this.Destroy();
            }
            ...
            if (keys.order_type == dotaunitorder_t.DOTA_UNIT_ORDER_MOVE_TO_TARGET && keys.target == this.GetParentPlus() && !keys.unit.HasModifier("modifier_imba_spirit_breaker_charge_of_darkness_taxi")) {
                this.attempting_to_board.push(keys.unit);
            } else if (this.attempting_to_board.includes(keys.unit)) {
                GFuncArray.ArrayRemove(this.attempting_to_board, keys.unit)
            }
        }
    }
}
...
export class modifier_imba_spirit_breaker_charge_of_darkness_clothesline extends BaseModifierMotionHorizontal_Plus {
    ...
    UpdateHorizontalMotion(me: CDOTA_BaseNPC, dt: number): void {
        ...
        me.SetOrigin(this.GetCasterPlus().GetOrigin() + (this.GetCasterPlus().GetForwardVector() * 128) as Vector);
    }
    OnHorizontalMotionInterrupted(): void {
        this.Destroy();
    }
...
export class modifier_imba_spirit_breaker_charge_of_darkness_taxi extends BaseModifierMotionHorizontal_Plus {
    ...
    UpdateHorizontalMotion(me: CDOTA_BaseNPC, dt: number): void {
        ...
        me.SetOrigin(this.GetCasterPlus().GetOrigin() + (this.GetCasterPlus().GetForwardVector() * (-1) * (this.passenger_num * this.taxi_distance)) as Vector);
    }
    OnHorizontalMotionInterrupted(): void {
        this.Destroy();
    }
    BeDestroy(): void {
        ...
        this.GetParentPlus().RemoveHorizontalMotionController(this);
        let charge_modifier = this.GetCasterPlus().FindModifierByNameAndCaster("modifier_imba_spirit_breaker_charge_of_darkness", this.GetCasterPlus()) as modifier_imba_spirit_breaker_charge_of_darkness;
    ...
    CC_OnOrder(keys: ModifierUnitEvent): void {
        ...
        if (keys.unit == this.GetParentPlus()) {
            let cancel_commands = [dotaunitorder_t.DOTA_UNIT_ORDER_HOLD_POSITION, dotaunitorder_t.DOTA_UNIT_ORDER_STOP];
            if (cancel_commands.includes(keys.order_type)) {
                this.Destroy();
            }
        }
...
export class imba_spirit_breaker_greater_bash extends BaseAbility_Plus {
    ...
    Bash(target: IBaseNpc_Plus, parent: IBaseNpc_Plus, bUltimate = false) {
        if (!IsServer()) {
            return;
        }
...
export class imba_spirit_breaker_nether_strike extends BaseAbility_Plus {
    ...
    OnAbilityPhaseStart(): boolean {
        ...
        if (!this.GetCasterPlus().FindModifierByNameAndCaster("modifier_imba_spirit_breaker_nether_strike_planeswalker", this.GetCasterPlus())) {
            ...
            if (GameServiceConfig.USE_MEME_SOUNDS) {
    ...
    OnAbilityPhaseInterrupted(): void {
        ...
        if (GameServiceConfig.USE_MEME_SOUNDS && this.randy) {
...
export class modifier_imba_spirit_breaker_nether_strike_planeswalker extends BaseModifier_Plus {
    ...
    BeDestroy(): void {
        ...
        if (!this.GetAbilityPlus<imba_spirit_breaker_nether_strike>().wtf_mode) {

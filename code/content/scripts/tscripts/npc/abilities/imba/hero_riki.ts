...
OnIntervalThink():void {
    ...
    if (caster.HasTalent("special_bonus_imba_riki_5")) {
        ...
        for (const [_, ally] of ipairs(allies)) {
            if (ally == caster) {
                let smoke_screen_invi = caster.AddNewModifier(caster, ability, "modifier_imba_smoke_screen_invi_indicator", {});
    ...
    for (const [_, unit] of ipairs(targets)) {
        if (!parent.afflicted[unit.entindex()]) {
            ...
            for (const [_, modifier] of ipairs(duplicateMods)) {
                if (!modifier.IsNull()) {
                    if (modifier != parent.afflicted[unit.entindex()]) {
                        if (modifier.GetStackCount() >= stacks) {
                            if (!parent.afflicted[unit.entindex()].IsNull()) {
            ...
            if (isStrongest && !parent.afflicted[unit.entindex()].IsNull()) {
                parent.afflicted[unit.entindex()].SetStackCount(stacks);
            }
        }
    }
    for (const [index, modifier] of ipairs(parent.afflicted)) {
        let unit = EntIndexToHScript(index);
        let distance = CalcDistanceBetweenEntityOBB(parent, unit);
        if (!modifier.IsNull()) {
            if (distance > aoe) {
...
BeDestroy():void {
    if (IsServer()) {
        for (const [_, modifier] of ipairs(this.GetParentPlus().afflicted)) {
            if (!modifier.IsNull()) {
                modifier.Destroy();
            }
        }
...
OnIntervalThink():void {
    if (this.thinker) {
        this.distance = (Vector(this.pos_x, this.pos_y, 0) - this.caster.GetAbsOrigin()).Length2D();
    }
    ...
    if (this.counter >= this.fade_delay) {
        this.caster.AddNewModifier(caster, ability, "modifier_imba_smoke_screen_invi", {});
    }
}
}
@registerModifier()
...
public hTarget : any; 
public thinker : any; 
public tMarkedTargets : any; 
public tStoredTargets : any; 
public index : any; 
...
OnAbilityPhaseStart():boolean {
    if (IsServer()) {
        ...
        ParticleManager.SetParticleControl(this.trail_pfx, 0, last_position + Vector(0, 0, 35));
        this.AddTimer(FrameTime(), () => {
            if (this.trail_pfx) {
                if ((index == this.index && !current_target.IsNull())) {
                    ...
                    let distance = (last_position - target_loc).Length2D();
                    let direction = (target_loc - last_position).Normalized();
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        this.AddTimer(jump_duration, () => {
            ...
            let final_pos = hTarget.GetAbsOrigin() - target_loc_forward_vector * 100;
...
DoJumpAttack(hTarget, hNextTarget) {
    ...
    let direction = (target_loc - next_target_loc).Normalized();
    if ((hTarget.GetTeamNumber() == this.hCaster.GetTeamNumber())) {
        ...
        for (let i = 1; i <= (this.jump_interval_frames - 1); i += 1) {
            this.AddTimer(FrameTime() * i, () => {
                let location = (start_loc - direction * distance * i);
        ...
        this.hCaster.SetForwardVector(direction);
        let location = target_loc - (hTarget.GetForwardVector() * 100);
        let particle = ResHelper.CreateParticleEx("particles/units/heroes/hero_riki/riki_blink_strike.vpcf", ParticleAttachment_t.PATTACH_ABSORIGIN, this.hCaster);
    ...
    if (IsServer()) {
        ...
        if (parent == attacker) {
            ...
            if (!parent.HasModifier("modifier_imba_riki_tricks_of_the_trade_primary") && !target.IsBuilding() && !parent.PassivesDisabled()) {
                ...
                let origin_difference = target.GetAbsOrigin() - attacker.GetAbsOrigin();
                ...
                if (GFuncRandom.PRD(back_chance, )) {
                    back_chance_success = true;
                }
                if (parent.HasTalent("special_bonus_imba_riki_8") && back_chance_success && target != this.GetParentPlus()) {
                    blink_strike_ability = parent.findAbliityPlus<imba_riki_blink_strike>("imba_riki_blink_strike");
                    ...
                    let blink_point = target.GetAbsOrigin() + direction * distance;
                    ...
                    if (parent.HasModifier("modifier_imba_riki_invisibility")) {
                        ...
                        ParticleManager.SetParticleControlEnt(sucker_particle, 1, target, ParticleAttachment_t.PATTACH_POINT_FOLLOW, "attach_hitloc", target.GetAbsOrigin() + Vector(0, 0, -100), true);
...
OnSpellStart():void {
    ...
    let cast_particle = ResHelper.CreateParticleEx(cast_particle, ParticleAttachment_t.PATTACH_WORLDORIGIN, this.GetCasterPlus());
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        let martyrs_mark_targets = undefined;
...
ProcTricks(caster:IBaseNpc_Plus, ability:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus, target:IBaseNpc_Plus, backstab_ability, backstab_particle, backstab_sound, talent_duration) {
    if (caster.HasTalent("special_bonus_imba_riki_2")) {
        let martyrs_mark_mod = target.findBuff<modifier_imba_martyrs_mark>("modifier_imba_martyrs_mark");
        if (martyrs_mark_mod) {

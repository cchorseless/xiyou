...
OnInventoryContentsChanged():void {
    ...
    if (!caster.HasScepter()) {
        if (caster.HasModifier("modifier_winter_wyvern_arctic_burn_flight") && caster.findBuff<modifier_winter_wyvern_arctic_burn_flight>("modifier_winter_wyvern_arctic_burn_flight").GetDuration() == -1) {
            caster.RemoveModifierByName("modifier_winter_wyvern_arctic_burn_flight");
        }
...
CreateTrackingProjectile(keys) {
    ...
    this.AddTimer(() => {
        ...
        if ((target_location - projectile).Length2D() < speed * FrameTime()) {
    ...
    if (IsServer()) {
        ...
        if (target != undefined) {
            ...
            if (caster.HasTalent("special_bonus_imba_winter_wyvern_4")) {
                ...
                let cold_embrace_modifier = caster.AddNewModifier(caster, ability, "modifier_imba_winter_wyvern_cold_embrace_resistance", {
                    duration: duration
                });
            ...
            target.AddNewModifier(caster, ability, "modifier_winter_wyvern_cold_embrace", {
                duration: duration
            });
            target.AddNewModifier(caster, ability, "modifier_imba_winter_wyvern_cold_embrace", {
                duration: duration,
    ...
    if (IsServer()) {
        ...
        if (keys.target == parent && !keys.attacker.IsTower()) {
            ...
            if (this.damage_taken >= this.damage_treshold && !this.activated) {
                ...
                parent.Heal(this.damage_treshold, parent);

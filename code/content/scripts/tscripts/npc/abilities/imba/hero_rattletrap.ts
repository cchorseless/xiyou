...
OnIntervalThink():void {
    ...
    if (GameFunc.GetCount(enemies) >= 1) {
        ...
        ParticleManager.SetParticleControl(particle2, 1, this.GetParentPlus().GetAbsOrigin() + RandomVector(RandomInt(0, 128)));
    }
    ParticleManager.ReleaseParticleIndex(particle);
    if (this.GetAbilityPlus() && this.GetAbilityPlus().GetAutoCastState()) {
        this.StartIntervalThink(this.interval / this.fragmentation_mult);
...
OnSpellStart():void {
    ...
    let square_dist = 160;
    let cog_vector = GetGroundPosition(caster_pos + Vector(0, cogs_radius, 0), undefined);
    let second_cog_vector = GetGroundPosition(caster_pos + Vector(0, cogs_radius * 2, 0), undefined);
    this.GetCasterPlus().StartGesture(GameActivity_t.ACT_DOTA_RATTLETRAP_POWERCOGS);
    for (let cog = 1; cog <= num_of_cogs; cog += 1) {
        let cog = BaseNpc_Plus.CreateUnitByName("npc_dota_rattletrap_cog", cog_vector, false, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
        ...
        if (this.GetCasterPlus().HasTalent("special_bonus_imba_rattletrap_second_gear")) {
            let second_cog = BaseNpc_Plus.CreateUnitByName("npc_dota_rattletrap_cog", second_cog_vector, false, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
    ...
    for (const [_, unit] of ipairs(units)) {
        if ((unit.GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Length2D() <= this.GetSpecialValueFor("cogs_radius")) {
            if (unit.GetTeamNumber() == this.GetCasterPlus().GetTeamNumber()) {
                FindClearSpaceForUnit(unit, unit.GetAbsOrigin(), false);
            } else {
                FindClearSpaceForUnit(unit, this.GetCasterPlus().GetAbsOrigin() + RandomVector(this.GetSpecialValueFor("extra_pull_buffer")), false);
            }
        } else {
            FindClearSpaceForUnit(unit, unit.GetAbsOrigin(), false);
        }
...
OnIntervalThink():void {
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        if (this.powered && !enemy.HasModifier("modifier_imba_rattletrap_cog_push") && math.abs(AngleDiff(VectorToAngles(this.GetParentPlus().GetForwardVector()).y, VectorToAngles(enemy.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).y)) <= 90) {
...
UpdateHorizontalMotion(me:CDOTA_BaseNPC, dt:number):void {
    ...
    let distance = (me.GetOrigin() - this.position).Normalized();
    me.SetOrigin(me.GetOrigin() + distance * this.knockback_speed * dt);
}
...
UpdateHorizontalMotion(me:CDOTA_BaseNPC, dt:number):void {
    ...
    me.SetForwardVector(me.GetAbsOrigin() - this.center_loc);
}
OnHorizontalMotionInterrupted():void {
    this.Destroy();
}
...
OnSpellStart():void {
    ...
    if (this.GetCasterPlus().GetName() == "npc_dota_hero_rattletrap") {
        ...
        if (this.GetCasterPlus().GetTogglableWearable(DOTASlotType_t.DOTA_LOADOUT_TYPE_MISC)) {
            this.GetCasterPlus().GetTogglableWearable(DOTASlotType_t.DOTA_LOADOUT_TYPE_MISC).AddEffects(EntityEffects.EF_NODRAW);
        }
    }
    if (!this.GetAutoCastState()) {
        ...
        if ((this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin()).Length2D() > 200) {
        ...
        for (let instance = 0; instance <= this.GetSpecialValueFor("carpet_fire_rockets") - 1; instance += 1) {
            ...
            this.AddTimer(this.GetSpecialValueFor("carpet_fire_delay") * instance, () => {
                if (this) {
                    let random_position = cursor_position + RandomVector(RandomInt(0, this.GetSpecialValueFor("radius") * this.GetSpecialValueFor("carpet_fire_spread")));
                    let rocket_target = CreateModifierThinker(this.GetCasterPlus(),this, undefined, {
                        duration: FrameTime()
...
OnProjectileHit_ExtraData(hTarget:CDOTA_BaseNPC|undefined, vLocation:Vector, ExtraData:any):boolean|void {
    ...
    if (this.GetCasterPlus().GetName() == "npc_dota_hero_rattletrap" && this.GetCasterPlus().GetTogglableWearable(DOTASlotType_t.DOTA_LOADOUT_TYPE_MISC)) {
        this.GetCasterPlus().GetTogglableWearable(DOTASlotType_t.DOTA_LOADOUT_TYPE_MISC).RemoveEffects(EntityEffects.EF_NODRAW);
    }
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        ...
        let travel_distance = (enemy.GetAbsOrigin() - cast_position).Length2D();
        let target_distance = (enemy.GetAbsOrigin() - vLocation).Length2D();
...
OnSpellStart():void {
    ...
    this.direction = (this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin()).Normalized();
    this.direction.z = 0;
    let hookshot_duration = ((this.GetSpecialValueFor("tooltip_range") + GetCastRangeIncrease(this.GetCasterPlus())) / this.GetSpecialValueFor("speed")) * 2;
    ...
    ParticleManager.SetParticleControl(hookshot_particle, 1, this.GetCasterPlus().GetAbsOrigin() + this.direction * (this.GetSpecialValueFor("tooltip_range") + GetCastRangeIncrease(this.GetCasterPlus())));
    ...
    let linear_projectile = {
        ...
        fDistance: this.GetSpecialValueFor("tooltip_range") + GetCastRangeIncrease(this.GetCasterPlus()),
    ...
    if (this.GetCasterPlus().GetName() == "npc_dota_hero_rattletrap" && this.GetCasterPlus().GetTogglableWearable(DOTASlotType_t.DOTA_LOADOUT_TYPE_WEAPON)) {
    ...
    this.AddTimer(hookshot_duration, () => {
        if (caster.GetName() == "npc_dota_hero_rattletrap" && caster.GetTogglableWearable(DOTASlotType_t.DOTA_LOADOUT_TYPE_WEAPON)) {
            caster.GetTogglableWearable(DOTASlotType_t.DOTA_LOADOUT_TYPE_WEAPON).RemoveEffects(EntityEffects.EF_NODRAW);
        }
    });
}
OnProjectileThink_ExtraData(vLocation:Vector, ExtraData:any):void {
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        let distance_vector = enemy.GetAbsOrigin() - vLocation;
    ...
    if (hTarget) {
        if (hTarget != this.GetCasterPlus()) {
            if (!ExtraData.autocast) {
                ...
                if ((this.GetCasterPlus().GetAbsOrigin() - hTarget.GetAbsOrigin()).Length2D() > this.GetSpecialValueFor("latch_radius")) {
        ...
        if (ExtraData.autocast && GameFunc.GetCount(this.shish_kabob) > 0) {
            if ((this.GetCasterPlus().GetAbsOrigin() - EntIndexToHScript(this.shish_kabob[GameFunc.GetCount(this.shish_kabob)]).GetAbsOrigin()).Length2D() > this.GetSpecialValueFor("latch_radius")) {
                this.GetCasterPlus().EmitSound("Hero_Rattletrap.Hookshot.Retract");
            }
            this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(),this, "modifier_imba_rattletrap_hookshot", {
                duration: (this.GetSpecialValueFor("tooltip_range") + this.GetCasterPlus().GetCastRangeBonus()) / this.GetSpecialValueFor("speed"),
...
BeCreated(params:any):void {
    ...
    this.distance = (this.target.GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Normalized();
    this.enemies_hit = {}
    if (this.ApplyHorizontalMotionController() == false || (this.GetCasterPlus().GetAbsOrigin() - this.target.GetAbsOrigin()).Length2D() <= this.latch_radius) {
        this.Destroy();
        return;
...
UpdateHorizontalMotion(me:CDOTA_BaseNPC, dt:number):void {
    ...
    this.distance = (this.target.GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Normalized();
    me.SetOrigin(me.GetOrigin() + this.distance * this.speed * dt);
    if ((this.GetCasterPlus().GetAbsOrigin() - this.target.GetAbsOrigin()).Length2D() <= this.latch_radius) {
        FindClearSpaceForUnit(this.GetParentPlus(), this.target.GetAbsOrigin() - this.distance * (this.GetParentPlus().GetHullRadius() + this.target.GetHullRadius()), true);
        this.Destroy();
    }
}
OnHorizontalMotionInterrupted():void {
...
BeDestroy():void {
    ...
    if (this.GetCasterPlus().GetName() == "npc_dota_hero_rattletrap" && (this.GetCasterPlus().GetAbsOrigin() - this.target.GetAbsOrigin()).Length2D() <= this.latch_radius && RollPercentage(15)) {

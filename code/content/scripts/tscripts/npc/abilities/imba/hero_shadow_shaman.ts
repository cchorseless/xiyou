...
OnSpellStart():void {
    ...
    let enemies = FindUnitsInBicycleChain(this.GetCasterPlus().GetTeamNumber(), target.GetAbsOrigin(), this.GetCasterPlus().GetAbsOrigin(), this.GetCasterPlus().GetAbsOrigin() + ((target.GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Normalized() * (this.GetSpecialValueFor("end_distance") + GetCastRangeIncrease(this.GetCasterPlus()))), this.GetSpecialValueFor("start_radius"), this.GetSpecialValueFor("end_radius"), undefined, this.GetAbilityTargetTeam(), this.GetAbilityTargetType(), this.GetAbilityTargetFlags(), FindOrder.FIND_CLOSEST, false);
    let enemies_hit = 0;
    let attachment;
    let dramatic_passive_modifier = this.GetCasterPlus().FindModifierByNameAndCaster("modifier_imba_shadow_shaman_ether_shock_handler", this.GetCasterPlus());
...
OnSpellStart():void {
    ...
    if (target.GetTeamNumber() != this.GetCasterPlus().GetTeamNumber()) {
        if (!target.TriggerSpellAbsorb(this)) {
            if (target.IsIllusion() && !Custom_bIsStrongIllusion(this.parent)) {
                target.Kill(target, this.GetCasterPlus());
            } else {
                if (this.GetCasterPlus().GetName() == "npc_dota_hero_shadow_shaman" && RollPercentage(75)) {
...
OnOwnerSpawned():void {
    ...
    if (this.GetCasterPlus().HasTalent("special_bonus_imba_shadow_shaman_hex_cooldown") && !this.GetCasterPlus().HasModifier("modifier_special_bonus_imba_shadow_shaman_hex_cooldown")) {
        this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(), this.GetCasterPlus().findAbliityPlus("special_bonus_imba_shadow_shaman_hex_cooldown"), "modifier_special_bonus_imba_shadow_shaman_hex_cooldown", {});
    }
}
OnOwnerSpawned():void {
    if (!IsServer()) {
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    ...
    if (keys.attacker == this.GetParentPlus() && this.GetCasterPlus().HasTalent("special_bonus_imba_shadow_shaman_hex_parlor_tricks") && !this.GetParentPlus().IsIllusion() && this.GetAbilityPlus() && this.GetAbilityPlus().IsTrained() && this == this.GetParentPlus().FindAllModifiersByName("modifier_imba_shadow_shaman_voodoo_handler")[1] && !keys.target.IsOther() && !keys.target.IsBuilding() && keys.target.GetTeamNumber() != this.GetParentPlus().GetTeamNumber()) {
        if (GFuncRandom.PRD(this.GetCasterPlus().GetTalentValue("special_bonus_imba_shadow_shaman_hex_parlor_tricks"), )) {
...
GetAuraEntityReject(hTarget:CDOTA_BaseNPC):boolean {
    ...
    return hTarget.IsRoshan() || hTarget == this.GetParentPlus() || (math.abs(AngleDiff(VectorToAngles(hTarget.GetForwardVector()).y, VectorToAngles(this.GetParentPlus().GetAbsOrigin() - hTarget.GetAbsOrigin()).y)) > (this.GetSpecialValueFor("deprecation_angle") || 45)) || this.targets[hTarget.GetEntityIndex()] || !hTarget.CanEntityBeSeenByMyTeam(this.GetParentPlus());
}
}
@registerModifier()
...
CC_OnTakeDamage(keys:ModifierInstanceEvent):void {
    ...
    if (keys.unit == this.GetParentPlus() && keys.damage >= this.GetParentPlus().GetMaxHealth() * this.deprecation_threshold * 0.01) {
        ...
        if (voodoo_modifier && voodoo_modifier.targets) {
            voodoo_modifier.targets[this.GetParentPlus().GetEntityIndex()] = true;
            this.Destroy();
        }
    }
}
...
OnIntervalThink():void {
    ...
    if (!this.GetAbilityPlus().IsChanneling()) {
        ...
        if (this.GetParentPlus().IsRealHero() && this.GetParentPlus().GetPlayerID() && this.GetParentPlus().ModifyGold && this.GetCasterPlus().ModifyGold) {
            ...
            SendOverheadEventMessage(this.GetCasterPlus(), DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_XP, this.GetCasterPlus(), actual_gold_to_steal, undefined);
        }
    }
}
CheckState():Partial<Record<modifierstate,boolean>> {
...
OnIntervalThink():void {
    ...
    if (!this.GetAbilityPlus().IsChanneling() || (this.GetParentPlus().GetAbsOrigin() - this.current_position).Length2D() > this.chariot_break_distance) {
        ...
        this.current_position = this.GetParentPlus().GetAbsOrigin();
        if ((this.GetParentPlus().GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Length2D() > this.chariot_max_length) {
            FindClearSpaceForUnit(this.GetCasterPlus(), this.GetParentPlus().GetAbsOrigin() + this.vector.Normalized() * this.chariot_max_length, false);
        }
    }
}
BeDestroy():void {
...
OnSpellStart():void {
    ...
    for (let i = 1; i <= ward_count; i += 1) {
        this.SummonWard(target_point + formation_vectors[i]);
    }
}
SummonWard(position, bChild, elapsedTime) {
    let new_hp;
    let duration;
    if (!bChild) {
    ...
    let ward = BaseNpc_Plus.CreateUnitByName("npc_dota_shadow_shaman_ward_" + math.min(this.GetLevel(), 3), position, true, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    ...
    if (keys.attacker == this.GetParentPlus() && keys.attacker != keys.unit && this.GetAbilityPlus() && !keys.unit.IsOther() && keys.unit.GetName() != "npc_dota_unit_undying_zombie") {
        if (!keys.unit.IsRealHero() && !keys.unit.IsBuilding()) {
            ...
            SendOverheadEventMessage(this.GetParentPlus(), DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_HEAL, this.GetParentPlus(), this.snake_charmer_creep_count, undefined);
        } else {
            ...
            SendOverheadEventMessage(this.GetParentPlus(), DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_HEAL, this.GetParentPlus(), this.snake_charmer_hero_count, undefined);
        }

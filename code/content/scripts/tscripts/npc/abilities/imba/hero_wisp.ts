...
OnSpellStart():void {
    let ability = this.GetCasterPlus().findAbliityPlus<imba_wisp_tether>("imba_wisp_tether");
    let caster = this.GetCasterPlus();
    let destroy_tree_radius = ability.GetSpecialValueFor("destroy_tree_radius");
    let movespeed = ability.GetSpecialValueFor("movespeed");
    let regen_bonus = ability.GetSpecialValueFor("tether_bonus_regen");
    let latch_distance = this.GetSpecialValueFor("latch_distance");
    if (caster.HasTalent("special_bonus_imba_wisp_5")) {
        ability.bonus_range = caster.GetTalentValue("special_bonus_imba_wisp_5");
    ...
    let tether_modifier = this.GetCursorTarget().AddNewModifier(this.GetCasterPlus(), ability, "modifier_imba_wisp_tether_ally", {});
    ...
    if (caster.HasTalent("special_bonus_imba_wisp_6")) {
        this.target.AddNewModifier(this.GetCasterPlus(), ability, "modifier_imba_wisp_tether_ally_attack", {});
    }
    if (!this.GetAutoCastState()) {
        let distToAlly = (this.GetCursorTarget().GetAbsOrigin() - caster.GetAbsOrigin()).Length2D();
...
OnIntervalThink():void {
    ...
    if (!this.GetAbilityPlus() || !this.GetAbilityPlus().tether_ally) {
        this.Destroy();
        return;
    }
    ...
    if ((this.GetAbilityPlus().tether_ally.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D() <= this.radius) {
        return;
    }
    this.GetParentPlus().RemoveModifierByName("modifier_imba_wisp_tether");
}
...
CC_OnManaGained(keys:ModifierUnitEvent):void {
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        let velocity = this.GetParentPlus().GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin();
        let vVelocity = velocity / FrameTime();
        vVelocity.z = 0;
        let projectile = {
            ...
            ExtraData: {
                slow_duration: this.GetAbilityPlus().slow_duration,
                slow: this.GetAbilityPlus().slow
            }
        }
...
OnRemoved():void {
    if (IsServer()) {
        ...
        if (this.GetAbilityPlus()) {
            this.GetAbilityPlus().target = undefined;
        }
...
BeCreated(params:any):void {
    if (IsServer()) {
        this.target = this.GetAbilityPlus().target;
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        let casterDir = this.GetCasterPlus().GetAbsOrigin() - this.target.GetAbsOrigin();
        ...
        if (distToAlly > this.final_latch_distance) {
            ...
            let pos = this.target.GetAbsOrigin() + casterDir * distToAlly;
...
OnSpellStart():void {
    if (!this.GetCasterPlus().HasAbility("imba_wisp_tether")) {
        ...
        stolenAbility.SetLevel(min((this.GetCasterPlus().GetLevel() / 2) - 1, 4));
...
BeCreated(p_0:any,):void {
    ...
    this.GetParentPlus().SetAbsOrigin(this.GetCasterPlus().GetAbsOrigin() + (this.GetCasterPlus().GetForwardVector() * this.backpack_distance * (-1)));
    this.StartIntervalThink(FrameTime());
}
OnIntervalThink():void {
...
public spirits_num_spirits : any; 
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        this.ability = this.caster.findAbliityPlus<imba_wisp_spirits>("imba_wisp_spirits");
        this.start_time = GameRules.GetGameTime();
        this.spirits_num_spirits = 0;
        let spirit_min_radius = this.ability.GetSpecialValueFor("min_range");
        let spirit_max_radius = this.ability.GetSpecialValueFor("max_range");
        let spirit_movement_rate = this.ability.GetSpecialValueFor("spirit_movement_rate");
        let creep_damage = this.ability.GetSpecialValueFor("creep_damage");
        let explosion_damage = this.ability.GetTalentSpecialValueFor("explosion_damage");
        let slow = this.ability.GetSpecialValueFor("slow");
        let spirit_duration = this.ability.GetSpecialValueFor("spirit_duration");
        let spirit_summon_interval = this.ability.GetSpecialValueFor("spirit_summon_interval");
        let max_spirits = this.ability.GetSpecialValueFor("num_spirits");
        let collision_radius = this.ability.GetSpecialValueFor("collision_radius");
        let explosion_radius = this.ability.GetSpecialValueFor("explode_radius");
        let spirit_turn_rate = this.ability.GetSpecialValueFor("spirit_turn_rate");
        let vision_radius = this.ability.GetSpecialValueFor("explode_radius");
        let vision_duration = this.ability.GetSpecialValueFor("vision_duration");
        let slow_duration = this.ability.GetSpecialValueFor("slow_duration");
        let damage_interval = this.ability.GetSpecialValueFor("damage_interval");
        ...
        this.ability.spirits_spiritsSpawned = {}
        ...
        this.caster.AddNewModifier(this.caster, this.ability, "modifier_imba_wisp_spirits", {
        ...
        this.caster.AddNewModifier(this.caster, this.ability, "modifier_imba_wisp_spirit_damage_handler", {
            duration: -1,
...
BeCreated(params:any):void {
    if (IsServer()) {
        ...
        this.GetAbilityPlus().update_timer = 0;
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        if (ability.spirits_num_spirits < idealNumSpiritsSpawned) {
            let newSpirit = BaseNpc_Plus.CreateUnitByName("npc_dota_wisp_spirit", caster_position, false, caster, caster, caster.GetTeam());
            ...
            if (caster.HasTalent("special_bonus_imba_wisp_3")) {
                ...
                let true_sight_aura = newSpirit.AddNewModifier(caster,this, "modifier_imba_wisp_spirits_true_sight", {});
...
Explode(caster:IBaseNpc_Plus, spirit, explosion_radius, explosion_damage, ability:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus) {
    if (IsServer()) {
        ...
        if (spirit_pfx_silence != undefined) {
            ParticleManager.DestroyParticle(spirit.spirit_pfx_silence, true);
            ParticleManager.ReleaseParticleIndex(spirit.spirit_pfx_silence);
        }
        ability.spirits_spiritsSpawned[spirit.spirit_index] = undefined;
    }
}
OnRemoved():void {
    if (IsServer()) {
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        spirit.hit_table = this.ability.hit_table;
        if (nearby_enemy_units != undefined && GameFunc.GetCount(nearby_enemy_units) > 0) {
            modifier_imba_wisp_spirit_handler.OnHit(this.caster, spirit, nearby_enemy_units, this.creep_damage, this.ability, this.slow_duration, this.slow);
        }
    }
}
OnHit(caster:IBaseNpc_Plus, spirit, enemies_hit, creep_damage, ability:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus:IBaseAbility_Plus, slow_duration, slow) {
    let hit_hero = false;
    let damage_table = {}
    damage_table.attacker = caster;
    damage_table.ability = ability;
    damage_table.damage_type = ability.GetAbilityDamageType();
    for (const [_, enemy] of ipairs(enemies_hit)) {
        if (enemy.IsAlive() && !spirit.IsNull()) {
            ...
            if (enemy.IsConsideredHero() && !enemy.IsIllusion()) {
                ...
                if (spirit.hit_table[enemy] == undefined) {
...
OnRemoved():void {
    if (IsServer()) {
        let spirit = this.GetParentPlus();
        let ability = this.GetAbilityPlus();
        modifier_imba_wisp_spirits.Explode(this.caster, spirit, this.explosion_radius, this.explosion_damage, ability);
        if (spirit.spirit_pfx_silence != undefined) {
            ParticleManager.DestroyParticle(spirit.spirit_pfx_silence, true);
        }
        if (spirit.spirit_pfx_disarm != undefined) {
        ...
        ability.CreateVisibilityNode(spirit.GetAbsOrigin(), this.vision_radius, this.vision_duration);
        ...
        if (ability.spirits_spiritsSpawned) {
            for (const [k, spirit] of ipairs(ability.spirits_spiritsSpawned)) {
                if (!spirit.IsNull()) {
                    ...
                    if (silence) {
                        spirit.spirit_pfx_silence = this.particles[k];
                    } else if (disarm) {
                        spirit.spirit_pfx_disarm = this.particles[k];
                    }
                }
            }
        }
...
AddOvercharge(caster:IBaseNpc_Plus, target:IBaseNpc_Plus, efficiency, overcharge_duration) {
...
BeCreated(params:any):void {
    if (IsServer()) {
        ...
        let net_table = CustomNetTables.GetTableValue("player_table", tostring(this.GetCasterPlus().GetPlayerOwnerID())) || {}
    ...
    if (IsServer()) {
        ...
        this.AddTimer({
            ...
            callback: () => {
                ...
                if (!imba_wisp_relocate.InterruptRelocate(this.GetCasterPlus(),this, tether_ability)) {
...
InterruptRelocate(caster:IBaseNpc_Plus, ability:IBaseAbility_Plus:IBaseAbility_Plus, tether_ability) {
...
public timer_buff : number; 
public relocate_timerPfx : number; 
IsDebuff():boolean {
    return false;
}
IsHidden():boolean {
...
GetEffectName():string {
    return this.GetParentPlus().overcharge_effect;
}
BeCreated(p_0:any,):void {
    this.ability = this.GetAbilityPlus();
    this.caster = this.GetCasterPlus();
...
CC_GetModifierPercentageCasttime(keys:ModifierAbilityEvent):number {
    if (this.GetParentPlus().HasAbility && this.GetParentPlus().HasAbility("furion_teleportation") && this.GetParentPlus().findAbliityPlus<furion_teleportation>("furion_teleportation").IsInAbilityPhase()) {
        return 0;
    } else {
        return this.bonus_cast_speed;
    }

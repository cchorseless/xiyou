...
OnSpellStart():void {
    ...
    for (let link = 1; link <= max_targets; link += 1) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            if (!linked_units[enemy.GetEntityIndex()]) {
    ...
    for (const [_, modifiers] of ipairs(modifier_table)) {
...
BeDestroy():void {
    ...
    for (const [_, enemy] of ipairs(this.bond_table)) {
        if (enemy != this.GetParentPlus()) {
            let bond_modifiers = enemy.FindAllModifiersByName("modifier_imba_fatal_bonds");
            for (const [_, modifier] of ipairs(bond_modifiers)) {
                for (let num = GameFunc.GetCount((modifier.bond_table)); num >= 1; num += -1) {
...
CC_OnTakeDamage(keys:ModifierInstanceEvent):void {
    if (IsServer() && bit.band(keys.damage_flags, DOTADamageFlag_t.DOTA_DAMAGE_FLAG_REFLECTION) != DOTADamageFlag_t.DOTA_DAMAGE_FLAG_REFLECTION) {
        ...
        if (unit == this.GetParentPlus() && this.bond_table) {
            for (const [_, bonded_enemy] of ipairs(this.bond_table)) {
            ...
            if ((keys.unit.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D() <= this.golem_link_radius) {
                let damageTable = {
...
OnIntervalThink():void {
    if (this.good_guy) {
        ...
        this.parent.Heal(heal, this.caster);
...
OnSpellStart():void {
    ...
    if (!caster.HasTalent("special_bonus_imba_warlock_6")) {
        ...
        let demon = BaseNpc_Plus.CreateUnitByName("npc_imba_warlock_upheaval_demon", target_point, true, caster, caster, caster.GetTeamNumber());
...
OnSpellStart():void {
    ...
    if (this.GetCasterPlus().HasScepter()) {
        for (let golems = 0; golems <= this.GetSpecialValueFor("number_of_golems_scepter") - 1; golems += 1) {
...
SummonGolem(target_point:IBaseNpc_Plus, bScepter, bDeath) {
    ...
    this.AddTimer(effect_delay, () => {
        ...
        let golem = undefined;
        if (!bScepter || bDeath) {
            golem = BaseNpc_Plus.CreateUnitByName("npc_dota_warlock_golem_" + this.GetLevel(), target_point, true, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
        } else {
            golem = BaseNpc_Plus.CreateUnitByName("npc_dota_warlock_golem_scepter_" + this.GetLevel(), target_point, true, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
        }
...
public particle_link_damage : number; 
    ...
    if (IsServer()) {
        this.AddTimer(FrameTime(), () => {
            if (GameFunc.GetCount(this.demon_table) < this.scepter_demon_count) {
                ...
                for (let i = 1; i <= this.scepter_demon_count; i += 1) {
...
OnIntervalThink():void {
    for (let i = 1; i <= GameFunc.GetCount(this.demon_table); i += 1) {
        if (!this.parent.IsNull() && this.parent.GetAbsOrigin && !this.demon_table[i].IsNull() && this.demon_table[i].GetAbsOrigin) {
            let direction = (this.parent.GetAbsOrigin() - this.demon_table[i].GetAbsOrigin()).Normalized();
    ...
    if (IsServer()) {
        ...
        if (this.caster == attacker) {
            ...
            if (this.caster.HasModifier(this.modifier_upheaval)) {
                let modifier_upheaval_handler = this.caster.FindModifierByName(this.modifier_upheaval);
...
GetAuraEntityReject(target:CDOTA_BaseNPC):boolean {
    ...
    if ((this.caster.GetAbsOrigin() - target.GetAbsOrigin()).Length2D() > this.radius) {
        return true;
    }
    return false;
}

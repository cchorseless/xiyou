...
OnUpgrade():void {
    ...
    if (!this.GetCasterPlus().successful_hooks) {
        this.GetCasterPlus().successful_hooks = 0;
    }
...
CastFilterResultLocation(vLocation:Vector):UnitFilterResult {
    ...
    if (IsNearFountain(vLocation, 1700)) {
...
OnSpellStart():void {
    ...
    let target_position = GetGroundPosition(this.GetCursorPosition() + vHookOffset, this.GetCasterPlus());
    if (target_position == this.GetCasterPlus().GetAbsOrigin()) {
        target_position = target_position + this.GetCasterPlus().GetForwardVector();
    }
    ...
    let vKillswitch = Vector((((this.GetCastRange() + GetCastRangeIncrease(this.GetCasterPlus())) / hook_speed) * 2) + 10, 0, 0);
    ...
    ParticleManager.SetParticleControlEnt(hook_particle, 0, this.GetCasterPlus(), ParticleAttachment_t.PATTACH_POINT_FOLLOW, "attach_weapon_chain_rt", this.GetCasterPlus().GetAbsOrigin() + vHookOffset, true);
    ParticleManager.SetParticleControl(hook_particle, 2, Vector(hook_speed, this.GetCastRange() + GetCastRangeIncrease(this.GetCasterPlus()), hook_width));
    ...
    let projectile_info = {
        ...
        fDistance: this.GetCastRange() + GetCastRangeIncrease(this.GetCasterPlus()),
        ...
        fExpireTime: GameRules.GetGameTime() + ((this.GetCastRange() + GetCastRangeIncrease(this.GetCasterPlus()) / hook_speed)),
        vVelocity: (target_position - this.GetCasterPlus().GetAbsOrigin()).Normalized() * hook_speed * Vector(1, 1, 0),
    ...
    if (this.GetCasterPlus() && this.GetCasterPlus().IsHero()) {
        let hHook = this.GetCasterPlus().hook_wearable;
...
OnProjectileThink_ExtraData(vLocation:Vector, ExtraData:any):void {
        ...
        for (const [_, ent] of ipairs(Entities.FindAllInSphere(location, radius))) {
            for (const [_, model] of ipairs(runes)) {
                for (const [_, rune] of ipairs(Entities.FindAllByModel(model))) {
                    if ((location - rune.GetAbsOrigin()).Length2D() < radius) {
    ...
    if (ExtraData.goorback != "back") {
        ...
        if (EntIndexToHScript(ExtraData.rune)) {
            ...
            ParticleManager.SetParticleControlEnt(ExtraData.pfx_index, 1, rune, ParticleAttachment_t.PATTACH_POINT_FOLLOW, "attach_hitloc", rune.GetAbsOrigin() + Vector(0, 0, 96), true);
            rune.SetAbsOrigin(GetGroundPosition(vLocation, this.GetCasterPlus()));
        } else {
            let target = EntIndexToHScript(ExtraData.hooked_target);
            if (!target || !target.IsNull || target.IsNull()) {
                return;
            }
            let location = vLocation + (this.GetCasterPlus().GetAbsOrigin() - target.GetAbsOrigin()).Normalized() * (ExtraData.hook_spd / (1 / FrameTime()));
            ...
            if (this.GetCasterPlus().GetTeamNumber() != target.GetTeamNumber() && !target.IsRune() && this.GetCasterPlus().HasTalent("special_bonus_imba_pudge_7")) {
                ...
                let distance_diff = (hooked_loc - target.GetAbsOrigin()).Length2D();
                if (distance_diff < damage_cap) {
                    let move_damage = distance_diff * rupture_damage;
                    if (move_damage > 0) {
                        if (!target.is_ruptured) {
...
OnProjectileHit_ExtraData(hTarget:CDOTA_BaseNPC|undefined, vLocation:Vector, ExtraData:any):boolean|void {
    if (hTarget) {
        let buff1 = hTarget.findBuff<modifier_imba_hook_target_enemy>("modifier_imba_hook_target_enemy");
        let buff2 = hTarget.findBuff<modifier_imba_hook_target_ally>("modifier_imba_hook_target_ally");
    }
    if (hTarget && this.GetCasterPlus().GetTeamNumber() != hTarget.GetTeamNumber() && (IsNearFountain(hTarget.GetAbsOrigin(), 1700) || IsNearFountain(this.GetCasterPlus().GetAbsOrigin(), 1700))) {
        return false;
    }
    if (ExtraData.goorback == "go") {
        ...
        let bVision = false;
        if (!target) {
            target = BaseNpc_Plus.CreateUnitByName("npc_dummy_unit", vLocation, false, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
        }
        ParticleManager.SetParticleControlEnt(ExtraData.pfx_index, 1, target, ParticleAttachment_t.PATTACH_POINT_FOLLOW, "attach_hitloc", target.GetAbsOrigin() + Vector(0, 0, 96), true);
        if (hTarget) {
            EmitSoundOnLocationWithCaster(hTarget.GetAbsOrigin(), "Hero_Pudge.AttackHookImpact", hTarget, this.GetCasterPlus());
            ...
            ParticleManager.SetParticleControlEnt(nFXIndex, 0, hTarget, ParticleAttachment_t.PATTACH_POINT_FOLLOW, "attach_hitloc", hTarget.GetAbsOrigin() + Vector(0, 0, 96), true);
            ...
            if (hTarget.GetTeamNumber() != this.GetCasterPlus().GetTeamNumber()) {
                ...
                if (this.GetCasterPlus().HasModifier("modifier_pudge_arcana")) {
                    if (hTarget.IsRealHero()) {
                        this.GetCasterPlus().successful_hooks = this.GetCasterPlus().successful_hooks + 1;
                    } else {
                        this.GetCasterPlus().successful_hooks = 0;
                    }
                    if (this.GetCasterPlus().successful_hooks >= 2) {
                        ...
                        if (this.GetCasterPlus().HasModifier("modifier_pudge_arcana") == 1) {
                        ...
                        let stack_10 = math.floor(this.GetCasterPlus().successful_hooks / 10);
                        ParticleManager.SetParticleControl(hook_counter, 2, Vector(stack_10, this.GetCasterPlus().successful_hooks - stack_10 * 10, this.GetCasterPlus().successful_hooks));
...
CC_GetActivityTranslationModifiers():string {
    if (this.GetCasterPlus().successful_hooks == undefined) {
        this.GetCasterPlus().successful_hooks = 0;
    }
    if (this.GetCasterPlus().successful_hooks >= 1 && this.GetCasterPlus().successful_hooks < 3) {
        return "hook_streak_small";
    } else if (this.GetCasterPlus().successful_hooks >= 3 && this.GetCasterPlus().successful_hooks < 5) {
        return "hook_streak_medium";
    } else if (this.GetCasterPlus().successful_hooks >= 5) {
...
BeDestroy():void {
    ...
    for (const [_, item] of ipairs(this.disable)) {
        if (item) {
...
CC_OnDeath(params:ModifierInstanceEvent):void {
    ...
    if (this.GetCasterPlus().IsRealHero() && target.IsRealHero() && caster.GetTeamNumber() != target.GetTeamNumber() && (!params.unit.IsReincarnating || !params.unit.IsReincarnating())) {
        ...
        if ((this.GetAbilityPlus().GetCasterPlus().GetAbsOrigin() - target.GetAbsOrigin()).Length2D() <= flesh_heap_range) {
            this.SetStackCount(this.GetStackCount() + 1);
...
OnChannelFinish(bInterrupted:boolean):void {
    if (this.target) {
        let target_buff = this.target.FindModifierByNameAndCaster("modifier_imba_dismember", this.GetCasterPlus());
        if (bInterrupted) {
...
UpdateHorizontalMotion(me:CDOTA_BaseNPC, dt:number):void {
    ...
    let distance = this.caster.GetOrigin() - me.GetOrigin();
    if (distance.Length2D() > this.pull_distance_limit && this.parent.HasModifier("modifier_imba_dismember")) {
        me.SetOrigin(me.GetOrigin() + distance.Normalized() * this.pull_units_per_second * dt);
    } else {
        this.Destroy();
    }
}
...
BeCreated(p_0:any,):void {
    ...
    this.pfx = ParticleManager.CreateParticleForTeam("particles/units/heroes/hero_pudge/pudge_swallow.vpcf", ParticleAttachment_t.PATTACH_OVERHEAD_FOLLOW, this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber(), this.GetCasterPlus());

    ...
    for (const [_, unit] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetCasterPlus().GetAbsOrigin(), undefined, this.GetSpecialValueFor("fake_lance_distance"), DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED, FindOrder.FIND_ANY_ORDER, false))) {
        if (unit.GetPlayerOwnerID() == this.GetCasterPlus().GetPlayerOwnerID() && unit != this.GetCasterPlus() && unit.IsIllusion()) {
            ProjectileManager.CreateTrackingProjectile({
                ...
                EffectName: "particles/units/heroes/hero_phantom_lancer/phantomlancer_spiritlance_projectile.vpcf",
                iMoveSpeed: lance_speed * ((unit.GetAbsOrigin() - target.GetAbsOrigin()).Length2D() / (this.GetCasterPlus().GetAbsOrigin() - target.GetAbsOrigin()).Length2D()),
                bDodgeable: true,
                bVisibleToEnemies: true,
                bReplaceExisting: false,
                bProvidesVision: false
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, extradata:any):boolean|void {
    ...
    if (target.GetTeamNumber() != this.GetCasterPlus().GetTeamNumber()) {
        if (!this.GetCasterPlus().IsIllusion()) {
            ...
            let illusions = CreateIllusions(this.GetCasterPlus(), this.GetCasterPlus(), {
            ...
            }, 1, this.GetCasterPlus().GetHullRadius(), false, true);
        ...
        if (!this.GetCasterPlus().IsIllusion()) {
            ...
            let illusions = CreateIllusions(this.GetCasterPlus(), illusion_type, {
                outgoing_damage: this.GetSpecialValueFor("illusion_damage_out_pct"),
                incoming_damage: this.GetSpecialValueFor("illusion_damage_in_pct"),
                bounty_base: 5,
                bounty_growth: undefined,
...
public new_pos : any; 
public illusion_1 : any; 
public illusion_2 : any; 
public illusion_3 : any; 
...
OnSpellStart():void {
    ...
    if (!this.illusion_1 || this.illusion_1.IsNull() || !this.illusion_1.IsAlive()) {
        this.illusion_1 = CreateIllusions(this.GetCasterPlus(), this.GetCasterPlus(), {
        ...
        }, 1, this.GetCasterPlus().GetHullRadius(), false, true)[1];
        ...
        this.illusion_1.findBuff<modifier_illusion>("modifier_illusion").SetDuration(this.GetSpecialValueFor("illusion_duration") + this.GetSpecialValueFor("delay"), true);
    }
    table.insert(affected_units, this.illusion_1);
    if (!this.illusion_2 || this.illusion_2.IsNull() || !this.illusion_2.IsAlive()) {
        this.illusion_2 = CreateIllusions(this.GetCasterPlus(), this.GetCasterPlus(), {
        ...
        this.illusion_2.findBuff<modifier_illusion>("modifier_illusion").SetDuration(this.GetSpecialValueFor("illusion_duration") + this.GetSpecialValueFor("delay"), true);
    }
    ...
    for (const [_, unit] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetCursorPosition(), undefined, this.GetSpecialValueFor("search_radius"), DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED, FindOrder.FIND_CLOSEST, false))) {
        if (unit.GetName() != this.GetCasterPlus().GetName() && (unit.IsRealHero() || unit.IsClone() || unit.IsTempestDouble() || unit.IsIllusion())) {
            this.illusion_3 = CreateIllusions(this.GetCasterPlus(), unit, {
                outgoing_damage: -100,
                incoming_damage: 0,
                bounty_base: 5,
                bounty_growth: undefined,
...
BeCreated(keys:any):void {
    ...
    if (this.GetParentPlus().IsIllusion() && this.GetParentPlus().HasModifier("modifier_illusion")) {
        this.GetParentPlus().findBuff<modifier_illusion>("modifier_illusion").SetDuration(this.GetParentPlus().FindModifierByName("modifier_illusion").GetRemainingTime() + this.GetSpecialValueFor("illusion_extended_duration"), true);
    }
}
BeDestroy():void {
    if (!IsServer()) {
...
OnIntervalThink():void {
    if (this.bRushChecking && this.GetParentPlus().GetAggroTarget() == this.target && ((this.target.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D() <= this.GetAbilityPlus().GetTalentSpecialValueFor("max_distance") || this.target.HasModifier("modifier_imba_phantom_lancer_sun_catcher_aura")) && ((this.target.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D() >= this.GetAbilityPlus().GetTalentSpecialValueFor("min_distance") || this.GetCasterPlus().HasScepter())) {
...
CC_OnOrder(keys:ModifierUnitEvent):void {
    if (keys.unit == this.GetParentPlus() && this.GetAbilityPlus() && !this.GetAbilityPlus().GetToggleState() && (this.GetAbilityPlus().IsCooldownReady() || this.GetCasterPlus().HasScepter()) && !this.GetParentPlus().PassivesDisabled() && keys.target) {
        if (keys.order_type == dotaunitorder_t.DOTA_UNIT_ORDER_ATTACK_TARGET) {
            if (((keys.target.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D() <= this.GetAbilityPlus().GetTalentSpecialValueFor("max_distance") || keys.target.HasModifier("modifier_imba_phantom_lancer_sun_catcher_aura")) && ((keys.target.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D() >= this.GetAbilityPlus().GetTalentSpecialValueFor("min_distance") || this.GetCasterPlus().HasScepter())) {
                this.GetParentPlus().EmitSound("Hero_PhantomLancer.PhantomEdge");
...
OnIntervalThink():void {
    this.target = EntIndexToHScript(-this.GetStackCount());
...
OnSpellStart():void {
    ...
    ParticleManager.SetParticleControl(linear_particle, 0, this.GetCasterPlus().GetAbsOrigin() + Vector(0, 0, 700));
    ParticleManager.SetParticleControl(linear_particle, 1, (this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin()).Normalized() * this.GetSpecialValueFor("speed") * Vector(1, 1, 0));
...
BeCreated(p_0:any,):void {
    ...
    } else if (!this.GetParentPlus().IsRealHero() && this.GetParentPlus().GetOwnerPlus() && this.GetParentPlus().GetOwnerPlus().GetAssignedHero()) {
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    if (keys.attacker == this.GetParentPlus() && !this.GetParentPlus().PassivesDisabled() && !keys.target.IsBuilding() && this.owner && this.owner.juxtapose_table && !this.owner.PassivesDisabled()) {
        ...
        if (GameFunc.GetCount((this.owner.juxtapose_table)) < this.GetAbilityPlus().GetTalentSpecialValueFor("max_illusions") && this.duration > 0 && this.owner.juxtapose_table) {
            let illusions = CreateIllusions(this.owner, this.GetParentPlus(), {
                outgoing_damage: this.GetSpecialValueFor("illusion_damage_out_pct"),
                incoming_damage: this.GetSpecialValueFor("illusion_damage_in_pct"),
                bounty_base: 5,
                bounty_growth: undefined,
            ...
            for (const [_, illusion] of ipairs(illusions)) {
                ...
                table.insert(this.owner.juxtapose_table, illusion.entindex());
            }
        ...
        if (this.duration > 0 && (GameFunc.GetCount((this.owner.juxtapose_table)) >= this.GetAbilityPlus().GetTalentSpecialValueFor("max_illusions"))) {
            for (const [_, unit] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetParentPlus().GetAbsOrigin(), undefined, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED, FindOrder.FIND_ANY_ORDER, false))) {
                if (unit.GetPlayerOwnerID() == this.GetCasterPlus().GetPlayerOwnerID()) {
                    let assault_modifier = unit.AddNewModifier(this.GetParentPlus().GetOwnerPlus().GetAssignedHero(), this.GetAbilityPlus(), "modifier_imba_phantom_lancer_juxtapose_assault", {
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    if (keys.unit == this.GetParentPlus() && this.owner && !this.owner.IsNull() && this.owner.juxtapose_table) {
        Custom_ArrayRemove(this.owner.juxtapose_table, function (i, j) {
            return this.owner.juxtapose_table[i] != this.GetParentPlus().entindex();
        });
    }
}
}

        ...
        if (target.IsAlive()) {
            ...
            if ((!target.IsSilenced() || !target.findBuff<modifier_imba_silencer_global_silence>("modifier_imba_silencer_global_silence") || !target.HasModifier("modifier_imba_silencer_global_silence_v2")) || this.aghs_upgraded) {
                ...
                if (this.talent_learned) {
                    ...
                    this.caster.Heal(heal_amount, this.caster);
...
public modifier_int_damage : number; 
public modifier_hit_counter : number; 
    ...
    if (IsServer()) {
        ...
        if (this.caster == attacker) {
            ...
            if (glaive_attack) {
                ...
                SetGlaiveAttackProjectile(this.caster, true);
            } else {
                this.glaive_attack = false;
                SetGlaiveAttackProjectile(this.caster, false);
            }
        }
    }
}
            ...
            if (target.IsAlive() && this.glaive_attack) {
                if (keys.target.IsRealHero && (keys.target.IsRealHero() || keys.target.IsTempestDouble()) && !keys.target.IsClone()) {
                    ...
                    if (!modifier_buff) {
                        ...
                        }).SetStackCount(this.ability.GetSpecialValueFor("int_steal"));
                    } else {
...
@registerAbility()
export class imba_silencer_last_word_silence_aura extends BaseAbility_Plus {
public silence_duration : number; 
    ...
    if (IsServer()) {
        ...
        if (this.caster.GetUnitName() == "npc_dota_hero_silencer" && this.caster.IsRealHero()) {
            if (params.unit.IsRealHero() && params.unit != this.caster && params.unit.GetTeam() != this.caster.GetTeam() && !params.reincarnate) {
                ...
                let distance = (this.caster.GetAbsOrigin() - params.unit.GetAbsOrigin()).Length2D();
...
OnUpgrade():void {
    if (IsServer()) {
        if (this.IsStolen() && this.GetLevel() == 1) {
            ...
            let origCaster = caster.spellStealTarget;
...
OnSpellStart():void {
    ...
    if (IsServer()) {
        ...
        if (USE_MEME_SOUNDS && RollPercentage(30)) {

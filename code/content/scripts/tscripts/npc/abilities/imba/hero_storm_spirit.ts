...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let remnant_pos = {}
        remnant_pos[1] = Vector(0, far_distance, 0);
        remnant_pos[2] = Vector(-far_distance, -far_distance, 0);
        remnant_pos[3] = Vector(far_distance, -far_distance, 0);
        if (remnant_count > 1) {
            for (let i = 1; i <= remnant_count; i += 1) {
                let dummy = BaseNpc_Plus.CreateUnitByName("npc_imba_dota_stormspirit_remnant", caster.GetAbsOrigin() + remnant_pos[i], false, caster, undefined, caster.GetTeamNumber());
                dummy.AddNewModifier(caster,this, "modifier_imba_static_remnant", {
                    duration: remnant_duration
                });
            }
        } else {
            let dummy = BaseNpc_Plus.CreateUnitByName("npc_imba_dota_stormspirit_remnant", caster.GetAbsOrigin(), false, caster, undefined, caster.GetTeamNumber());
            dummy.AddNewModifier(caster,this, "modifier_imba_static_remnant", {
                duration: remnant_duration
            });
        }
        ...
        for (const [_, enemy] of ipairs(nearby_enemies)) {
            ...
            if (enemy.IsAlive()) {
                ...
                if (pull_duration != 0 && !this.ballLightning) {
                    enemy.AddNewModifier(this.GetCasterPlus(),this, "modifier_imba_vortex_root", {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        if (!caster.HasScepter()) {
            ...
            let direction = (caster.GetAbsOrigin() - target.GetAbsOrigin()).Normalized();
            ...
            let enemies = FindUnitsInRadius(caster.GetTeamNumber(), caster_loc, undefined, this.GetCastRange(), this.GetAbilityTargetTeam(), this.GetAbilityTargetType(), this.GetAbilityTargetFlags(), FindOrder.FIND_ANY_ORDER, false);
            for (const [_, enemy] of ipairs(enemies)) {
                let direction = (caster.GetAbsOrigin() - enemy.GetAbsOrigin()).Normalized();
...
@registerModifier()
export class modifier_imba_vortex_pull extends BaseModifier_Plus {
...
GetMotionControllerPriority() {
    return DOTA_MOTION_CONTROLLER_PRIORITY.DOTA_MOTION_CONTROLLER_PRIORITY_LOW;
}
BeCreated(params:any):void {
    if (IsServer()) {
...
HorizontalMotion(unit:IBaseNpc_Plus, time) {
    if (IsServer()) {
        let set_point = unit.GetAbsOrigin() + (this.GetCasterPlus().GetAbsOrigin() - unit.GetAbsOrigin()).Normalized() * this.speed;
...
@registerModifier()
export class modifier_imba_vortex_root extends BaseModifier_Plus {
public vortex_loc : any; 
public vortex_particle_fx : any; 
public speed : number; 
public electric_vortex_pull_distance : number; 
...
HorizontalMotion(unit:IBaseNpc_Plus, time) {
    if (IsServer()) {
        let set_point = unit.GetAbsOrigin() + (this.vortex_loc - unit.GetAbsOrigin()).Normalized() * this.speed;
        unit.SetAbsOrigin(Vector(set_point.x, set_point.y, GetGroundPosition(set_point, unit).z));
    }
}
/** DeclareFunctions():modifierfunction[] {
...
CC_GetActivityTranslationModifiers():string {
    if (this.GetParentPlus().GetName() == "npc_dota_hero_storm_spirit") {
        return "overload";
    }
    return 0;
}
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        if ((target_loc - caster_loc).Length2D() > 130) {
...
OnProjectileThink_ExtraData(location:Vector, ExtraData:any):void {
    ...
    if ((this.traveled + ExtraData.speed < this.distance) && caster.IsAlive() && (caster.GetMana() > ExtraData.total_mana_cost * 0.01)) {
        ...
        if (caster.GetName() == "npc_dota_hero_storm_spirit") {
            caster.EmitCasterSound("npc_dota_hero_storm_spirit", responses, 100, ResHelper.EDOTA_CAST_SOUND.FLAG_BOTH_TEAMS, undefined, undefined);
        }
    ...
    if (IsServer()) {
        if (target) {
            ...
            if (!target.IsAlive()) {
                ...
                if (caster.GetName() == "npc_dota_hero_storm_spirit" && RollPercentage(10)) {
                    caster.EmitCasterSound("npc_dota_hero_storm_spirit", responses, 100, ResHelper.EDOTA_CAST_SOUND.FLAG_BOTH_TEAMS, undefined, undefined);
                }
            }
        }

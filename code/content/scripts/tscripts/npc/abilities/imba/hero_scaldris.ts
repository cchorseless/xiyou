...
ScepterSwap(ability:IBaseAbility_Plus:IBaseAbility_Plus1, ability2) {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let wave_distance = this.GetSpecialValueFor("wave_distance") + GetCastRangeIncrease(caster);
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let direction = (target_loc - caster_loc).Normalized();
        let distance = (target_loc - caster_loc).Length2D();
...
OnProjectileThink_ExtraData(location:Vector, extra_data:any):void {
    if (IsServer()) {
        if ((location - Vector(extra_data.x, extra_data.y, location.z)).Length2D() <= 16) {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let rush_distance = this.GetSpecialValueFor("rush_distance") + GetCastRangeIncrease(caster);
...
@registerModifier()
export class modifier_imba_jet_blaze_rush extends BaseModifier_Plus {
...
OnProjectileThink_ExtraData(location:Vector, extra_data:any):void {
    if (IsServer()) {
        if ((location - Vector(extra_data.x, extra_data.y, location.z)).Length2D() <= 16 && this.hit_check == false) {
            this.hit_check = true;
            let caster = this.GetCasterPlus();

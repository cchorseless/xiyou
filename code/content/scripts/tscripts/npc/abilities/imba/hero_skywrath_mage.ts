...
OnSpellStart():void {
    ...
    if (RollPercentage(5)) {
        ...
        EmitSoundOn(cast_responses[math.random(1, GameFunc.GetCount(cast_responses))], caster);
    }
...
OnSpellStart():void {
    ...
    if (GameFunc.GetCount(enemies) >= 1) {
        ...
        ParticleManager.SetParticleControlEnt(cast_particle, 2, enemies[1], ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", enemies[1].GetAbsOrigin(), true);
        ParticleManager.ReleaseParticleIndex(cast_particle);
        if ((enemies[1].GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Length2D() <= this.GetSpecialValueFor("search_radius")) {
        ...
        if (GameFunc.GetCount(basic_units) >= 1) {
            ...
            ParticleManager.ReleaseParticleIndex(cast_particle);
            if ((basic_units[1].GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Length2D() <= this.GetSpecialValueFor("search_radius")) {
    ...
    if (scepter) {
        for (const [_, enemy_hero] of ipairs(enemies)) {
            if (enemy_hero != enemies[1] && !enemy_hero.IsMagicImmune()) {
                if ((enemy_hero.GetAbsOrigin() - caster.GetAbsOrigin()).Length2D() <= ability.GetSpecialValueFor("scepter_search_radius")) {
        ...
        for (const [_, enemy_creep] of ipairs(basic_units)) {
            if (enemy_creep != basic_units[1] && !enemy_creep.IsMagicImmune()) {
                if ((enemy_creep.GetAbsOrigin() - caster.GetAbsOrigin()).Length2D() <= ability.GetSpecialValueFor("scepter_search_radius")) {
...
OnSpellStart():void {
    ...
    if (scepter) {
        ...
        for (const [_, enemy_hero] of ipairs(enemy_heroes)) {
            let distance = (enemy_hero.GetAbsOrigin() - target_point).Length2D();
        ...
        for (const [_, enemy_creep] of ipairs(enemy_creeps)) {
            let distance = (enemy_creep.GetAbsOrigin() - target_point).Length2D();

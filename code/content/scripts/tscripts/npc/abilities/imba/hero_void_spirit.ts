...
BeCreated(p_0:any,):void {
    ...
    for (let outer_portals = 1; outer_portals <= this.portals_per_ring; outer_portals += 1) {
        portal_position = GetGroundPosition(RotatePosition(this.GetCasterPlus().GetAbsOrigin(), QAngle(0, this.angle_per_ring_portal * (outer_portals - 1), 0), this.GetCasterPlus().GetAbsOrigin() + (this.GetCasterPlus().GetForwardVector() * (this.first_ring_distance_offset))), undefined);
...
CC_OnOrder(keys:ModifierUnitEvent):void {
    if (keys.unit == this.GetParentPlus() && this.portal_selection_orders[keys.order_type] && this.portals && GameRules.GetGameTime() >= this.GetCreationTime() + 0.11) {
        ...
        for (const [particle, position] of ipairs(this.portals)) {
            ParticleManager.SetParticleControl(particle, 2, Vector(0, 0, 0));
            if (!this.shortest_distance || (this.selected_pos - position).Length2D() < this.shortest_distance) {
                this.shortest_distance = (this.selected_pos - position).Length2D();
...
OnIntervalThink():void {
    ...
    for (const [_, enemy] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.center, undefined, this.ring_size, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NONE, FindOrder.FIND_ANY_ORDER, false))) {
        let distance = CalculateDistance(enemy, this.center);
        if (!this.hit_enemies[enemy.entindex()] && distance < this.ring_size + this.thickness && distance > this.ring_size - this.thickness) {
            ...
            if (enemy.IsRealHero() || enemy.IsClone() || enemy.IsTempestDouble()) {
                ...
                ProjectileManager.CreateTrackingProjectile({
                    ...
                    Source: enemy.GetAbsOrigin(),
                    vSourceLoc: enemy.GetAbsOrigin(),
                    Target: EntIndexToHScript(this.thinker_entindex),
                    iMoveSpeed: this.return_projectile_speed,
                    bDodgeable: false,
                    bIsAttack: false,
                    bReplaceExisting: false,
...
OnSpellStart():void {
    if (this.GetCasterPlus().HasAbility("imba_void_spirit_astral_step") && this.GetCasterPlus().findAbliityPlus<imba_void_spirit_astral_step>("imba_void_spirit_astral_step").original_vector) {
        this.GetCasterPlus().findAbliityPlus<imba_void_spirit_astral_step>("imba_void_spirit_astral_step").OnSpellStart(this.GetCasterPlus().FindAbilityByName("imba_void_spirit_astral_step").original_vector * (-1));
    }
}
}
@registerModifier()
...
OnAbilityPhaseStart():boolean {
    if (this.charge_modifier && this.charge_modifier.GetStackCount() > 0) {
        ...
        DisplayError(this.GetCasterPlus().GetPlayerOwnerID(), "#dota_hud_error_astral_step_no_charges");
        return false;
    }
}
...
OnChannelFinish(bInterrupted:boolean):void {
    ...
    if (this.GetCasterPlus().HasAbility("imba_void_spirit_astral_step")) {
        this.GetCasterPlus().findAbliityPlus<imba_void_spirit_astral_step>("imba_void_spirit_astral_step").OnSpellStart(undefined, (this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin()).Normalized() * (this.GetCasterPlus().FindAbilityByName("imba_void_spirit_astral_step").GetSpecialValueFor("max_travel_distance") * (1 + (this.GetSpecialValueFor("bonus_range_pct") * 0.01)) + this.GetCasterPlus().GetCastRangeBonus()) * Vector(1, 1, 0), bInterrupted);
    }
}
}
@registerModifier()
...
OnIntervalThink():void {
    Custom_ArrayRemove(this.stack_table, function (i, j) {
        return GameRules.GetDOTATime(true, true) - this.stack_table[i] <= this.swiftness_duration;
    });
...
OnSpellStart( /** recastVector */  /** , warpVector */  /** , bInterrupted */ ):void {
    if (this.GetCursorPosition() == this.GetCasterPlus().GetAbsOrigin()) {
        this.GetCasterPlus().SetCursorPosition(this.GetCursorPosition() + this.GetCasterPlus().GetForwardVector());
    }
    ...
    let final_position = this.GetCasterPlus().GetAbsOrigin() + ((this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin()).Normalized() * math.max(math.min(((this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin()) * Vector(1, 1, 0)).Length2D(), this.GetSpecialValueFor("max_travel_distance") + this.GetCasterPlus().GetCastRangeBonus()), this.GetSpecialValueFor("min_travel_distance")));
    if (recastVector) {
        final_position = this.GetCasterPlus().GetAbsOrigin() + recastVector;
    }
    if (warpVector) {
        final_position = GetGroundPosition(this.GetCasterPlus().GetAbsOrigin() + warpVector, undefined);
    }
    this.original_vector = (final_position - this.GetCasterPlus().GetAbsOrigin()).Normalized() * (this.GetSpecialValueFor("max_travel_distance") + this.GetCasterPlus().GetCastRangeBonus());
    ...
    for (const [_, enemy] of ipairs(FindUnitsInLine(this.GetCasterPlus().GetTeamNumber(), this.GetCasterPlus().GetAbsOrigin(), final_position, undefined, this.GetSpecialValueFor("radius"), DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES))) {
        ...
        this.GetCasterPlus().SetAbsOrigin(enemy.GetAbsOrigin() - this.GetCasterPlus().GetForwardVector());
    ...
    if (this.GetCasterPlus().HasAbility("imba_void_spirit_astral_step_helper_2")) {
        if (this.GetCasterPlus().findAbliityPlus<imba_void_spirit_astral_step_helper_2>("imba_void_spirit_astral_step_helper_2").charge_modifier && ((!warpVector && this.GetCasterPlus().FindAbilityByName("imba_void_spirit_astral_step_helper_2").charge_modifier.GetStackCount() <= 1) || (warpVector && this.GetCasterPlus().FindAbilityByName("imba_void_spirit_astral_step_helper_2").charge_modifier.GetStackCount() <= 0))) {
            this.GetCasterPlus().findAbliityPlus<imba_void_spirit_astral_step_helper_2>("imba_void_spirit_astral_step_helper_2").StartCooldown(this.GetCasterPlus().FindAbilityByName("imba_void_spirit_astral_step_helper_2").charge_modifier.GetRemainingTime());
        }
    }

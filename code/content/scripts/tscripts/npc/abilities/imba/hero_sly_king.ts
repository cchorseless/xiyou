...
OnSpellStart():void {
    if (IsServer()) {
        ...
        for (let i = 1; i <= projectile_count; i += 1) {
            ...
            travel_distance = this.GetSpecialValueFor("cast_range") + GetCastRangeIncrease(caster);
            let projectile = {
                Ability: this,
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            let distance = (enemy.GetAbsOrigin() - this.pos).Length2D();
            let direction = (enemy.GetAbsOrigin() - this.pos).Normalized();

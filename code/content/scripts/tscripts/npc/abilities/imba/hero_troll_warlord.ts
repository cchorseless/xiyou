...
OnToggle():void {
    if (IsServer()) {
        ...
        if (RollPercentage(25) && (caster.GetName() == "npc_dota_hero_troll_warlord") && !caster.beserk_sound) {
            caster.EmitSound("troll_warlord_troll_beserker_0" + math.random(1, 4));
            caster.beserk_sound = true;
            this.AddTimer(10, () => {
                caster.beserk_sound = undefined;
            });
        }
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let axe_range = this.GetSpecialValueFor("axe_range") + GetCastRangeIncrease(caster);
        ...
        for (let i = 1; i <= axe_count; i += 1) {
            ...
            let velocity = RotateVector2D(direction, angle, true) * axe_speed;
            let projectile = {
                Ability: this,
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, ExtraData:any):boolean|void {
    ...
    if (target) {
        ...
        if (GFuncRandom.PRD(ExtraData.on_hit_pct, )) {
        ...
        if (this[ExtraData.index]["count"] == ExtraData.axe_count) {
...
DoAxeStuff(index, range, caster_loc:IBaseNpc_Plus) {
    let caster = this.GetCasterPlus();
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        let was_hit = false;
        for (const [_, stored_target] of ipairs(this[index])) {
...
CC_OnAttackLanded(params:ModifierAttackEvent):void {
    ...
    if (((params.attacker == parent) || ((params.attacker.GetTeamNumber() == parent.GetTeamNumber()) && params.attacker.HasModifier("modifier_imba_battle_trance") && parent.HasScepter())) && (params.attacker.IsRealHero() || params.attacker.IsClone()) && params.original_damage > 0) {
        let modifier = params.attacker.FindModifierByNameAndCaster("modifier_imba_fervor_stacks", parent);
        if (modifier) {
            ...
            modifier = params.attacker.AddNewModifier(parent, this.GetAbilityPlus(), "modifier_imba_fervor_stacks", {});
            modifier.last_target = params.target;
        }
    }
}
...
BeCreated(p_0:any,):void {
    ...
    this.bonus_bat = min(ability.GetTalentSpecialValueFor("bonus_bat"), parent.GetBaseAttackTime());
...
BeCreated(p_0:any,):void {
    ...
    this.bonus_bat = min(this.ability.GetTalentSpecialValueFor("bonus_bat"), this.parent.GetBaseAttackTime());
    if (!IsServer()) {
        return;
    }
...
OnIntervalThink():void {
    ...
    if (this.target && this.target.IsAlive() && !this.target.IsAttackImmune() && !this.target.IsInvulnerable() && this.caster.CanEntityBeSeenByMyTeam(this.target)) {
        ...
        if (!this.target.HasModifier("modifier_imba_battle_trance_vision_720") && (this.target.GetAbsOrigin() - this.caster.GetAbsOrigin()).Length2D() <= this.range) {
            this.target.AddNewModifier(this.caster, this.ability, "modifier_imba_battle_trance_vision_720", {});
        } else if (this.target.HasModifier("modifier_imba_battle_trance_vision_720") && (this.target.GetAbsOrigin() - this.caster.GetAbsOrigin()).Length2D() > this.range) {
    ...
    if (GameFunc.GetCount(non_hero_enemies) > 0) {
        for (let enemy = 1; enemy <= GameFunc.GetCount(non_hero_enemies); enemy += 1) {
            if (this.caster.CanEntityBeSeenByMyTeam(non_hero_enemies[enemy]) && !non_hero_enemies[enemy].IsRoshan()) {

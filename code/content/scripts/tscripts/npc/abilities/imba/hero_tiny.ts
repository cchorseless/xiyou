...
CC_OnDeath(params:ModifierInstanceEvent):void {
    ...
    let pfx_name = string.gsub("particles/units/heroes/hero_tiny/tiny01_death.vpcf", "lvl1", "lvl" + grow_level);
...
BeCreated(p_0:any,):void {
    if (IsServer()) {
        ...
        if (caster.tree != undefined) {
            caster.tree.AddEffects(EntityEffects.EF_NODRAW);
            UTIL_Remove(caster.tree);
            caster.tree = undefined;
        }
        this.tree = SpawnEntityFromTableSynchronous("prop_dynamic", {
            model: caster.tree_model
        });
...
GetAbilityTextureName():string {
    ...
    if (!this.GetCasterPlus().arcana_style) {
...
CC_OnAttackStart(keys:ModifierAttackEvent):void {
    if (keys.attacker == this.GetCasterPlus()) {
        if (keys.target != undefined && keys.target.IsBuilding()) {
            let caster = this.GetCasterPlus();
            let building_damage_modifier = caster.AddNewModifier(caster,this, "imba_tiny_tree_building_modifier", {});
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    ...
    if (IsServer()) {
        if (caster == keys.attacker && !keys.no_attack_cooldown) {
            if (caster.HasModifier("imba_tiny_tree_building_modifier")) {
                caster.RemoveModifierByName("imba_tiny_tree_building_modifier");
            }
            if (keys.target != undefined) {
                let splash_distance = caster.GetForwardVector() * this.GetSpecialValueFor("splash_distance");
                ...
                let enemies = FindUnitsInRadius(caster.GetTeam(), caster.GetAbsOrigin() + splash_distance, undefined, splash_radius, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_BASIC, 0, 0, false);
                for (const [_, enemy] of ipairs(enemies)) {
                    if (enemy != keys.target) {
                        damage_table.victim = enemy;
                        ApplyDamage(damage_table);
                        EmitSoundOn("Hero_Tiny.Tree.Target", caster);
                        let nfx = ResHelper.CreateParticleEx(caster.tree_cleave_effect, ParticleAttachment_t.PATTACH_POINT, caster);
...
OnRemoved():void {
    if (IsServer()) {
        ...
        if (this.GetAbilityPlus().tree != undefined) {
            this.GetAbilityPlus().tree.Destroy();
        }
...
@registerAbility()
export class imba_tiny_tree_building_modifier extends BaseAbility_Plus {
IsHidden():boolean {
    return true;
}
IsBuff() {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let distance = (target_point - caster_position).Length2D();
        let direction = (target_point - caster_position).Normalized();
        let velocity = direction * travel_speed;
        if (target == undefined) {
            let projectile = {
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, ExtraData:any):boolean|void {
    if (IsServer()) {
        if (!target) {
            EmitSoundOnLocationWithCaster(location, this.GetCasterPlus().tree_throw_target_sound, this.GetCasterPlus());
            return undefined;
        }
...
KnockBack(caster:IBaseNpc_Plus, target:IBaseNpc_Plus, knockback_center) {
    if (caster.HasTalent("special_bonus_imba_tiny_5")) {
        let knockback_direction = (target.GetAbsOrigin() - knockback_center).Normalized();
...
GetCooldown(nLevel:number):number {
    ...
    OnSpellStart():void {
        ...
        let distance = (vPos - casterPos).Length2D();
        this.direction = (vPos - casterPos).Normalized();
        let velocity = distance / delay * this.direction;
        ...
        let info = {
            EffectName: this.GetCasterPlus().avalance_projectile_effect,
            Ability: this,
    ...
    imba_tiny_avalanche.OnProjectileHit_ExtraData = function(hTarget, vLocation, extradata) {
        let caster = this.GetCasterPlus();
        ...
        let avalanche = ResHelper.CreateParticleEx(this.GetCasterPlus().avalanche_effect || "particles/units/heroes/hero_tiny/tiny_avalanche.vpcf", ParticleAttachment_t.PATTACH_CUSTOMORIGIN, undefined);
...
CC_OnAttackLanded(params:ModifierAttackEvent):void {
    if (IsServer()) {
        ...
        if (params.attacker == this.GetParentPlus() && !this.GetParentPlus().IsIllusion()) {
            if (GFuncRandom.PRD(this.chance, )) {
                let vPos = params.target.GetAbsOrigin();
                let caster = this.GetCasterPlus();
                let delay = this.GetSpecialValueFor("projectile_duration");
                let casterPos = caster.GetAbsOrigin();
                let distance = (vPos - casterPos).Length2D();
                let direction = (vPos - casterPos).Normalized();
                let velocity = distance / delay * direction;
                velocity.z = 0;
...
@registerModifier()
export class modifier_tiny_toss_movement extends BaseModifier_Plus {
...
HorizontalMotion(me:IBaseNpc_Plus, dt) {
    if (IsServer()) {
...
VerticalMotion(me:IBaseNpc_Plus, dt) {
    if (IsServer()) {
    ...
    if (IsServer()) {
        if (this.time < this.bounce_duration) {
            ...
            if (this.bounce_duration / 2 > this.time) {
                ...
                this.parent.SetAbsOrigin(GetGroundPosition(this.parent.GetAbsOrigin(), this.parent) + Vector(0, 0, this.toss_z));
        ...
        if (params.target == this.GetParentPlus()) {
            ...
            if (GFuncRandom.PRD(this.chance, )) {
                if (this.GetParentPlus().HasTalent("special_bonus_imba_tiny_4")) {
                    ...
                    let avalanche = ResHelper.CreateParticleEx(caster.avalanche_effect || "particles/units/heroes/hero_tiny/tiny_avalanche.vpcf", ParticleAttachment_t.PATTACH_CUSTOMORIGIN, params.attacker);
...
OnUpgrade():void {
    ...
    let grow_pfx_name = string.gsub(this.GetCasterPlus().grow_effect, "lvl1", "lvl" + level);
    let grow = ResHelper.CreateParticleEx(this.GetCasterPlus().grow_effect, ParticleAttachment_t.PATTACH_POINT_FOLLOW, this.GetCasterPlus());
    ParticleManager.SetParticleControl(grow, 0, this.GetCasterPlus().GetAbsOrigin());
    ParticleManager.ReleaseParticleIndex(grow);
    if (this.GetCasterPlus().ambient_pfx) {
        ParticleManager.DestroyParticle(this.GetCasterPlus().ambient_pfx, true);
        ParticleManager.ReleaseParticleIndex(this.GetCasterPlus().ambient_pfx);
    }
    let pfx_name = string.gsub(this.GetCasterPlus().ambient_pfx_effect, "lvl1", "lvl" + level);
    this.GetCasterPlus().ambient_pfx = ResHelper.CreateParticleEx(pfx_name, ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW, this.GetCasterPlus());
}
SetupModel(level) {

...
OnSpellStart():void {
    ...
    let self_haunt_modifier = this.GetCasterPlus().FindModifierByNameAndCaster("modifier_imba_spectre_haunt_self", this.GetCasterPlus());
    if (self_haunt_modifier) {
        for (const [_, enemy] of ipairs(enemies)) {
            if (enemy.FindModifierByNameAndCaster("modifier_imba_spectre_haunt", this.GetCasterPlus()) && enemy != self_haunt_modifier.current_target) {
                FindClearSpaceForUnit(this.GetCasterPlus(), enemy.GetAbsOrigin() + RandomVector(256), false);
...
OnIntervalThink():void {
    ...
    if ((this.GetParentPlus().GetAbsOrigin() - this.location).Length2D() <= this.travel_speed) {
        this.location = this.GetParentPlus().GetAbsOrigin();
        if (!this.GetParentPlus().IsInvisible() && !this.GetParentPlus().IsInvulnerable() && !this.GetParentPlus().IsOutOfGame() && !this.GetParentPlus().IsAttackImmune() && this.GetCasterPlus().IsAlive() && (this.self_haunt_modifier && this.self_haunt_modifier.current_target != this.GetParentPlus()) && !IsNearFountain(this.GetParentPlus().GetAbsOrigin(), 1200)) {
        ...
        this.location = this.location + (this.GetParentPlus().GetAbsOrigin() - this.location).Normalized() * this.travel_speed;
    }

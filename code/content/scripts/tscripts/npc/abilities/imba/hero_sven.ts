...
OnSpellStart():void {
    ...
    if (this.GetCasterPlus().GetName() == "npc_dota_hero_sven") {
...
CC_GetActivityTranslationModifiers():string {
    if (this.GetParentPlus().GetName() == "npc_dota_hero_sven") {
        return "sven_warcry";
    }
}
}
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        if ((math.random(1, 100) <= 50) && (caster.GetName() == "npc_dota_hero_sven")) {
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, ExtraData:any):boolean|void {
    if (IsServer()) {
        ...
        if (target || location) {
            ...
            let blink_pos = target_pos + (caster_pos - target_pos).Normalized() * 100;
            FindClearSpaceForUnit(caster, blink_pos, true);
            if (((target_pos - caster_pos).Length2D() > 600) && (RandomInt(1, 100) <= 20) && (caster.GetName() == "npc_dota_hero_sven")) {
...
CC_GetModifierPreAttack_CriticalStrike(params:ModifierAttackEvent):number {
    if (this.GetAbilityPlus().target == params.target) {
...
BeCreated(p_0:any,):void {
    ...
    if (IsServer()) {
        ...
        ParticleManager.SetParticleControl(this.buff2_fx, 0, this.GetParentPlus().GetAbsOrigin() + Vector(0, 0, 0));
...
CC_GetModifierTotal_ConstantBlock(keys:ModifierAttackEvent):number {
    ...
    SendOverheadEventMessage(this.GetParentPlus(), DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_BLOCK, this.GetParentPlus(), math.min(this.hp_shield, keys.damage), this.GetParentPlus());
    this.SetStackCount(this.hp_shield);
    return keys.damage;
}
}
...
OnAbilityPhaseStart():boolean {
    ...
    ParticleManager.SetParticleControlEnt(roar_pfx, 1, caster, ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW, undefined, caster.GetAbsOrigin(), true);
    ParticleManager.ReleaseParticleIndex(roar_pfx);
    if (caster.GetName() == "npc_dota_hero_sven") {
        if ((math.random(1, 100) <= 25)) {

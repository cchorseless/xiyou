...
BeCreated(p_0:any,):void {
    ...
    if (this.GetCasterPlus().time_spawned) {
        this.SetStackCount(math.floor(GameRules.GetGameTime() - this.GetCasterPlus().time_spawned) * (-1));
    }
...
BeCreated(p_0:any,):void {
    ...
    if (this.GetParentPlus().time_spawned) {
        this.SetStackCount(math.floor(GameRules.GetGameTime() - this.GetParentPlus().time_spawned) * (-1));
    }
...
OnUpgrade():void {
    ...
    if (this.GetLevel() >= 1 && this.GetCasterPlus().FindModifierByNameAndCaster(this.GetIntrinsicModifierName(), this.GetCasterPlus()) && !this.GetCasterPlus().FindModifierByNameAndCaster(this.GetIntrinsicModifierName(), this.GetCasterPlus()).particle) {
        this.GetCasterPlus().FindModifierByNameAndCaster(this.GetIntrinsicModifierName(), this.GetCasterPlus()).particle = ResHelper.CreateParticleEx("particles/units/heroes/hero_visage/visage_soul_overhead.vpcf", ParticleAttachment_t.PATTACH_OVERHEAD_FOLLOW, this.GetCasterPlus());
        this.GetCasterPlus().FindModifierByNameAndCaster(this.GetIntrinsicModifierName(), this.GetCasterPlus()).AddParticle(this.GetCasterPlus().FindModifierByNameAndCaster(this.GetIntrinsicModifierName(), this.GetCasterPlus()).particle, false, false, -1, false, false);
    }
}
OnSpellStart():void {
    let target = this.GetCursorTarget();
    ...
    if (this.GetCasterPlus().GetName() == "npc_dota_hero_visage") {
        if (RollPercentage(10)) {
            ...
            this.GetCasterPlus().EmitSound(this.responses_rare[RandomInt(1, GameFunc.GetCount(this.responses_rare))]);
        } else {
            if (!this.responses) {
                this.responses = {
    ...
    if (assumption_counter_modifier) {
        ...
        for (const [_, mod] of ipairs(assumption_stack_modifiers)) {
            mod.Destroy();
        }
    ...
    if (target_counter < this.GetTalentSpecialValueFor("targets")) {
        ...
        for (const [_, enemy] of ipairs(enemy_creeps)) {
            if (target_counter >= this.GetTalentSpecialValueFor("targets")) {
                return;
            }
            if (enemy != target) {
...
BeCreated(params:any):void {
    ...
    let assumption_modifier = this.GetParentPlus().FindModifierByNameAndCaster("modifier_imba_visage_soul_assumption", this.GetCasterPlus());
    ...
    if (assumption_modifier && assumption_modifier.particle && assumption_counter_modifier) {
        ...
        for (let bar = 1; bar <= this.stack_limit; bar += 1) {
...
OnUpgrade():void {
    let cloak_modifier = this.GetCasterPlus().FindModifierByNameAndCaster("modifier_imba_visage_gravekeepers_cloak", this.GetCasterPlus());
    if (cloak_modifier) {
...
OnIntervalThink():void {
    if (this.summon_familiars_ability && this.summon_familiars_ability.familiar_table) {
        ...
        for (let num = 1; num <= GameFunc.GetCount(this.summon_familiars_ability.familiar_table); num += 1) {
...
OnSpellStart():void {
    ...
    if (this.familiar_table) {
        for (let num = 1; num <= GameFunc.GetCount(this.familiar_table); num += 1) {
            if (this.familiar_table[num] && EntIndexToHScript(this.familiar_table[num]) && EntIndexToHScript(this.familiar_table[num]).IsNull && !EntIndexToHScript(this.familiar_table[num]).IsNull() && EntIndexToHScript(this.familiar_table[num]).IsAlive && EntIndexToHScript(this.familiar_table[num]).IsAlive() && EntIndexToHScript(this.familiar_table[num]).ForceKill) {
                EntIndexToHScript(this.familiar_table[num]).ForceKill(false);
            }
        }
    }
    this.familiar_table = {}
    let familiar = undefined;
    let spawn_location = undefined;
    let stone_form_ability = undefined;
    let summon_particle = undefined;
    for (let num = 1; num <= unit_count; num += 1) {
        spawn_location = this.GetCasterPlus().GetAbsOrigin() + (this.GetCasterPlus().GetForwardVector() * 200) + (this.GetCasterPlus().GetRightVector() * ((math.max(unit_count - 1, 0) * 120) * (-0.5 + ((math.max(num - 1, 0)) / (unit_count - 1)))));
        familiar = BaseNpc_Plus.CreateUnitByName("npc_dota_visage_familiar" + math.min(this.GetLevel(), 3), spawn_location, true, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
...
OnIntervalThink():void {
    ...
    for (const [_, ally] of ipairs(allies)) {
        if (ally != this.GetParentPlus() && string.find(ally.GetDebugName(), "npc_dota_visage_familiar") && !ally.IsMoving()) {
            ally.SetAbsOrigin(GetGroundPosition(ally.GetAbsOrigin() + (ally.GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()), ally));
        }
    }
}

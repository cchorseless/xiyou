...
UpdateCustomIndicator(loc) {
    let origin = this.GetCasterPlus().GetAbsOrigin();
    let point_blank = this.GetSpecialValueFor("point_blank_range");
    let direction = loc - origin;
    direction.z = 0;
    direction = direction.Normalized();
    ParticleManager.SetParticleControl(this.effect_cast, 0, origin);
    ParticleManager.SetParticleControl(this.effect_cast, 1, origin + direction * (this.GetCastRange(loc, undefined) + 200));
    ParticleManager.SetParticleControl(this.effect_cast, 6, origin + direction * point_blank);
}
...
OnSpellStart():void {
    ...
    let projectile_direction = point - origin;
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, extraData:any):boolean|void {
    ...
    let length = (location - origin).Length2D();
...
PlayEffects(target:IBaseNpc_Plus, point_blank) {
    ...
    if (point_blank) {
        ...
        ParticleManager.ReleaseParticleIndex(effect_cast);
        let effect_cast = ResHelper.CreateParticleEx(particle_cast3, ParticleAttachment_t.PATTACH_POINT_FOLLOW, target);
...
OnProjectileHit(target:CDOTA_BaseNPC|undefined, location:Vector):boolean|void {
    ...
    if (this.GetCasterPlus().HasTalent("special_bonus_unique_snapfire_5")) {
        if (target.GetTeam() == this.GetCasterPlus().GetTeam()) {
            target.Heal(this.GetCasterPlus().GetTalentValue("special_bonus_unique_snapfire_5"), this.GetCasterPlus());
...
PlayEffects2(target:IBaseNpc_Plus) {
    ...
    let effect_cast = ResHelper.CreateParticleEx(particle_cast, ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW, target);
    ParticleManager.ReleaseParticleIndex(effect_cast);
    let effect_cast = ResHelper.CreateParticleEx(particle_cast2, ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW, target);
    EmitSoundOn(sound_target, target);
    return effect_cast;
}
PlayEffects3(target:IBaseNpc_Plus, radius) {
...
CC_GetModifierOverrideAttackDamage( /** keys */ ):number {
    if (this.GetStackCount() <= 0) {
        return;
    }
    if (!IsServer()) {
    ...
    let fury_shredder_handle = target.findBuff<modifier_imba_snapfire_lil_shredder_debuff>("modifier_imba_snapfire_lil_shredder_debuff");
...
OnProjectileHit(target:CDOTA_BaseNPC|undefined, location:Vector):boolean|void {
    ...
    if (target.mega_blob) {
        ...
        for (let i = 1; i <= blob_rings_count; i += 1) {
            ...
            let vec = (location - target_pos).Length2D();
            let info = {
                Target: thinker,
                Source: target,
                Ability: this,
...
PlayEffects(loc) {
    ...
    ParticleManager.ReleaseParticleIndex(effect_cast);
    let effect_cast = ResHelper.CreateParticleEx(particle_cast2, ParticleAttachment_t.PATTACH_WORLDORIGIN, this.GetCasterPlus());
...
SetValidTarget(location) {
    let origin = this.GetParentPlus().GetOrigin();
    let vec = location - origin;
    let direction = vec;
    direction.z = 0;
    direction = direction.Normalized();
    if (vec.Length2D() < this.min_range) {
        vec = direction * this.min_range;
    } else if (vec.Length2D() > this.max_range) {
        vec = direction * this.max_range;
    }
    this.target = GetGroundPosition(origin + vec, undefined);
    this.vector = vec;
...
BeCreated(kv:any):void {
    ...
    let distance = (this.GetCasterPlus().GetAbsOrigin() - this.GetParentPlus().GetAbsOrigin()).Length2D();

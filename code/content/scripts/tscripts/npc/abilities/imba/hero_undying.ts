...
OnSpellStart():void {
    ...
    if (caster.GetName() == "npc_dota_hero_undying" && RollPercentage(50)) {
        if (caster.HasModifier(flesh_golem_modifier)) {
            EmitSoundOnClient(responses_big[RandomInt(1, GameFunc.GetCount(responses_big))], caster.GetPlayerOwner());
        } else {
            EmitSoundOnClient(responses[RandomInt(1, GameFunc.GetCount(responses))], caster.GetPlayerOwner());
        }
    }
...
OnStackCountChanged(prev_stacks:number):void {
    ...
    if (stacks > prev_stacks) {
        ...
        this.GetCasterPlus().Heal(this.strength_gain * this.hp_gain_per_str, this.GetCasterPlus());
        this.ForceRefresh();
...
SpawnTombstone(target_point:IBaseNpc_Plus) {
    let caster = this.GetCasterPlus();
    let ability = this;
    ...
    let tombstone = BaseNpc_Plus.CreateUnitByName("npc_dota_undying_imba_tombstone", target_point, true, caster, caster, caster.GetTeamNumber());
...
OnIntervalThink():void {
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        if (!enemy.IsCourier() && !IsUndyingZombie(enemy)) {
            let zombie = BaseNpc_Plus.CreateUnitByName(GenerateZombieType(), enemy.GetAbsOrigin(), true, this.owner, this.owner, this.caster.GetTeamNumber());
            ...
            let deathlust_ability = zombie.FindAbilityByName(this.deathlust_ability);
...
CC_OnDeath(keys:ModifierInstanceEvent):void {
    ...
    if (IsUndyingTombstone(unit)) {
        ...
        let zombies = Entities.FindAllByClassname("npc_dota_unit_undying_zombie");
        for (const [_, zombie] of ipairs(zombies)) {

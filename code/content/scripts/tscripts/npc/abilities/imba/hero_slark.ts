...
public damage_table_self : number; 
...
BeCreated(params:any):void {
    ...
    let visual_unit = BaseNpc_Plus.CreateUnitByName("npc_dota_slark_visual", this.GetParentPlus().GetAbsOrigin(), true, this.GetParentPlus(), this.GetParentPlus(), this.GetParentPlus().GetTeamNumber());
    ...
    if (this.premature_modifier) {
        if (this.premature_modifier.GetStackCount() < this.premature_stack_activation) {
            ...
            let spawn_unit = BaseNpc_Plus.CreateUnitByName("npc_dota_slark_spawn", this.GetParentPlus().GetAbsOrigin() + RandomVector(128), true, this.GetParentPlus(), this.GetParentPlus(), this.GetParentPlus().GetTeamNumber());
...
OnSpellStart():void {
    ...
    if (!this.GetCasterPlus().HasModifier("modifier_imba_slark_pounce")) {
        ...
        if (this.GetCasterPlus().findBuff<modifier_imba_slark_pounce>("modifier_imba_slark_pounce").redirect_pos) {
            this.GetCasterPlus().findBuff<modifier_imba_slark_pounce>("modifier_imba_slark_pounce").direction = (this.GetCasterPlus().FindModifierByName("modifier_imba_slark_pounce").redirect_pos - this.GetCasterPlus().GetAbsOrigin()).Normalized();
        } else {
...
BeDestroy():void {
    ...
    this.GetParentPlus().RemoveVerticalMotionController();
    if (this.GetCasterPlus().GetName() == "npc_dota_hero_slark") {
...
UpdateHorizontalMotion(me:CDOTA_BaseNPC, dt:number):void {
    ...
    me.SetAbsOrigin(me.GetAbsOrigin() + this.pounce_speed * this.direction * dt);
}
...
UpdateVerticalMotion(me:CDOTA_BaseNPC, dt:number):void {
    ...
    me.SetAbsOrigin(me.GetAbsOrigin() + Vector(0, 0, this.vertical_velocity) * dt);
...
OnIntervalThink():void {
    ...
    this.limit = ((this.leash_radius - (this.GetParentPlus().GetAbsOrigin() - this.leash_position).Length2D()) / this.leash_radius) * this.move_speed;
    if (this.limit == 0) {
        this.limit = -0.01;
    }
}
...
CC_GetModifierMoveSpeed_Limit():number {
    ...
    if ((this.GetParentPlus().GetAbsOrigin() - this.leash_position).Length2D() >= this.begin_slow_radius && math.abs(AngleDiff(VectorToAngles(this.GetParentPlus().GetAbsOrigin() - this.leash_position).y, VectorToAngles(this.GetParentPlus().GetForwardVector()).y)) <= 85) {
        return this.limit;
    }
}
}
...
@registerEvent(Enum_MODIFIER_EVENT.ON_DEATH)
CC_OnDeath(keys:ModifierInstanceEvent):void {
    if (keys.unit == this.GetParentPlus() && (keys.unit == this.GetParentPlus() || keys.unit.GetTeamNumber() != keys.attacker.GetTeamNumber()) && this.GetCasterPlus().HasModifier("modifier_imba_slark_essence_shift") && ((keys.unit.GetAbsOrigin() - this.GetCasterPlus().GetAbsOrigin()).Length2D() <= 300 || keys.attacker == this.GetCasterPlus()) && !this.GetCasterPlus().HasModifier("modifier_morphling_replicate")) {
...
OnIntervalThink():void {
    ...
    this.bVisible = false;
    for (const [_, enemy] of ipairs(FindUnitsInRadius(this.GetParentPlus().GetTeamNumber(), this.GetParentPlus().GetAbsOrigin(), undefined, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD, FindOrder.FIND_FARTHEST, false))) {
...
BeCreated(params:any):void {
    ...
    if ((!params || params && params.bAutoCast != 1)) {
        ...
        let visual_unit = BaseNpc_Plus.CreateUnitByName("npc_dota_slark_visual", this.GetCasterPlus().GetAbsOrigin(), true, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
        ...
        for (let arr = 1; arr <= 20; arr += 1) {
            ...
            ParticleManager.SetParticleControl(this.shadow_dummy_particle, 1, GetGroundPosition(this.GetParentPlus().GetAbsOrigin() + RotatePosition(Vector(0, 0, 0), QAngle(0, (360 / 20) * arr, 0), Vector(800, 0, 0)), undefined));
            this.AddParticle(this.shadow_dummy_particle, false, false, -1, false, false);
        }
        for (let arr = 1; arr <= 10; arr += 1) {
            this.shadow_dummy_particle = ResHelper.CreateParticleEx("particles/units/heroes/hero_slark/slark_shadow_dance_dummy_sceptor.vpcf", ParticleAttachment_t.PATTACH_WORLDORIGIN, this.GetParentPlus());
            ParticleManager.SetParticleControl(this.shadow_dummy_particle, 1, GetGroundPosition(this.GetParentPlus().GetAbsOrigin() + RotatePosition(Vector(0, 0, 0), QAngle(0, (360 / 10) * arr, 0), Vector(300, 0, 0)), undefined));
            this.AddParticle(this.shadow_dummy_particle, false, false, -1, false, false);
        }
    }
}

...
OnIntervalThink():void {
    ...
    if (!this.IsHidden() && !this.GetCasterPlus().HasModifier("modifier_bloodseeker_thirst")) {
        this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(),this, "modifier_bloodseeker_thirst", {});
...
@registerModifier()
export class modifier_imba_rip_current_movement extends BaseModifier_Plus {
public caster : IBaseNpc_Plus; 
public ability : IBaseAbility_Plus; 
...
OnIntervalThink():void {
    ...
    this.HorizontalMotion(this.caster, this.frametime);
}
CheckState():Partial<Record<modifierstate,boolean>> {
    let state = {
...
HorizontalMotion(me:IBaseNpc_Plus, dt) {
    if (IsServer()) {
        if (this.distance_travelled < this.distance) {
            this.caster.SetAbsOrigin(this.caster.GetAbsOrigin() + this.direction * this.velocity * dt);
...
CC_GetModifierProcAttack_BonusDamage_Physical(keys:ModifierAttackEvent):number {
    if (IsServer()) {
        ...
        if (caster == attacker) {
            ...
            if (target.IsStunned()) {
                ...
                if (GameFunc.GetCount(modifiers) > 0) {
                    for (const [_, modifier] of ipairs(modifiers)) {
                        if (!modifier.extended_by_deep_bash) {
                ...
                if (continue_looking_lua_stuns) {
                    ...
                    for (const [_, modifier] of ipairs(modifiers)) {
                        if (modifier.CheckState) {
                            if (modifier.CheckState()[modifierstate.MODIFIER_STATE_STUNNED] && !modifier.extended_by_deep_bash) {
                                if (!modifier.GetAbilityPlus().IsPassive()) {
                                    modifier.SetDuration(modifier.GetRemainingTime() + extend_duration, true);
                                    modifier.extended_by_deep_bash = true;
                                    smack_target = true;
            ...
            if (!smack_target) {
                if (GFuncRandom.PRD(bash_chance_pct, )) {
...
OnInventoryContentsChanged():void {
    if (IsServer()) {
        ...
        if (caster.HasAbility(rain_ability)) {
            let rain_ability_handler = caster.FindAbilityByName(rain_ability);
...
OnSpellStart():void {
    ...
    if (target.GetUnitName() == "npc_dota_hero_riki") {
        ...
        if (RollPercentage(15)) {
            ...
            let cast_response = cast_response_string + "0" + cast_response_table[RandomInt(1, 6)];
            EmitSoundOn(cast_response, caster);
        }
    }
    EmitSoundOn(sound_cast, caster);
    ...
    if (caster.HasTalent("special_bonus_imba_slardar_7")) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            if (enemy != target && !enemy.HasModifier(modifier_debuff)) {
                ...
                particle_haze_fx = ResHelper.CreateParticleEx(particle_haze, ParticleAttachment_t.PATTACH_OVERHEAD_FOLLOW, enemy);
...
CC_GetModifierTotalDamageOutgoing_Percentage(keys:ModifierAttackEvent):number {
    ...
    let slip_up_chance_pct = slip_up_chance_pct + caster.GetTalentValue("special_bonus_imba_slardar_4");
...
BeCreated(p_0:any,):void {
    if (IsServer()) {
        ...
        ability.dummy = BaseNpc_Plus.CreateUnitByName("npc_dummy_unit_perma", caster.GetAbsOrigin(), false, caster, undefined, caster.GetTeamNumber());
...
UpdateHorizontalMotion(me:CDOTA_BaseNPC, dt:number):void {
    if (IsServer()) {
        ...
        let distance = (caster.GetAbsOrigin() - dummy.GetAbsOrigin()).Length2D();
        ...
        let direction = (caster.GetAbsOrigin() - dummy.GetAbsOrigin()).Normalized();

...
OnSpellStart():void {
    if (IsServer()) {
        ...
        this["split_" + index] = this.GetSpecialValueFor("split_amount");
        this[index] = 1;
    ...
    if (hTarget) {
        if (hTarget.IsRealHero() || hTarget.IsConsideredHero() || hTarget.IsRoshan()) {
            if (hTarget.GetTeamNumber() != this.GetCasterPlus().GetTeamNumber()) {
                ...
                hTarget.Heal(heal, this.GetCasterPlus());
        ...
        this.AddTimer(ExtraData.bounce_delay, () => {
            ...
            if (hTarget.GetTeamNumber() != this.GetCasterPlus().GetTeamNumber()) {
                for (const [_, unit] of ipairs(enemies)) {
                    if (hTarget) {
                        if ((unit != hTarget) && (!unit.IsOther()) && ((this["split_" + ExtraData.index] >= 1) || GameFunc.GetCount(tJumpTargets) == 0)) {
            ...
            if (GameFunc.GetCount(tJumpTargets) == 0) {
                ...
                if (this[ExtraData.index] == 1) {
                    ...
                    this["split_" + ExtraData.index] = undefined;
                } else {
                    this[ExtraData.index] = this[ExtraData.index] - 1;
                }
                return undefined;
            } else if (GameFunc.GetCount(tJumpTargets) >= 2) {
                this["split_" + ExtraData.index] = this["split_" + ExtraData.index] - 1;
            }
...
OnToggle():void {
    if (this.GetToggleState()) {
        ...
        this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(),this, "modifier_imba_voodoo_restoration", {});
        if ((!_G.VOODOO) && (this.GetCasterPlus().GetName() == "npc_dota_hero_witch_doctor")) {
            _G.VOODOO = true;
...
OnIntervalThink():void {
    ...
    hParent.Heal(heal, this.GetCasterPlus());
    SendOverheadEventMessage(hParent, DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_HEAL, hParent, heal, hParent);
}
}
@registerAbility()
...
CC_OnDeath(params:ModifierInstanceEvent):void {
    if ((!_G.MALEDICT_KILL) && (this.GetParentPlus() == params.unit) && this.GetCasterPlus().GetName() == "npc_dota_hero_witch_doctor") {
        _G.MALEDICT_KILL = true;
        this.GetCasterPlus().EmitSound("witchdoctor_wdoc_ability_maledict_0" + math.random(1, 4));
        this.AddTimer(30, () => {
            _G.MALEDICT_KILL = undefined;
        });
    }
}
DealHPBurstDamage(hTarget) {
    ...
    if ((!_G.MALEDICT_POP) && (maxHP_pct < 0.2) && (this.soundcount == 2) && (this.GetCasterPlus().GetName() == "npc_dota_hero_witch_doctor")) {
        _G.MALEDICT_POP = true;
        this.GetCasterPlus().EmitSound("witchdoctor_wdoc_killspecial_0" + math.random(1, 3));
        this.AddTimer(30, () => {
            _G.MALEDICT_POP = undefined;
        });
    }
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        if (this.GetCasterPlus().HasTalent("special_bonus_imba_witch_doctor_8")) {
            ...
            let spawn_line_direction = RotateVector2D((vPosition - this.GetCasterPlus().GetAbsOrigin()).Normalized(), 90, true);
            let talent_ward = this.CreateWard(vPosition - ((distance / 2) * spawn_line_direction));
        ...
        if (USE_MEME_SOUNDS == true && RollPercentage(MEME_SOUNDS_CHANCE)) {
            this.dance = true;
...
CreateWard(vPosition, bIsMiniWard) {
    ...
    let death_ward = BaseNpc_Plus.CreateUnitByName("imba_witch_doctor_death_ward", vPosition, true, this.GetCasterPlus(), undefined, this.GetCasterPlus().GetTeam());
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, vLocation:Vector, ExtraData:any):boolean|void {
    if (this[ExtraData.index]) {
        if (!this[ExtraData.index].IsNull()) {
            if (this.GetCasterPlus().HasScepter()) {
                this[ExtraData.index].PerformAttack(target, false, true, true, true, false, false, true);
            } else {
                this[ExtraData.index].PerformAttack(target, false, true, true, true, false, false, false);
            }
...
CreateBounceAttack(originalTarget, extraData) {
...
BeDestroy():void {
    if (IsServer()) {
        this.GetAbilityPlus()[this.GetParentPlus().index] = undefined;
    }
}
OnIntervalThink():void {
    if (IsServer()) {
        ...
        if (this.attack_target) {
            if (!((((this.attack_target.GetAbsOrigin() - hParent.GetAbsOrigin()).Length2D()) <= range) && UnitFilter(this.attack_target, hAbility.GetAbilityTargetTeam(), hAbility.GetAbilityTargetType(), hAbility.GetAbilityTargetFlags(), this.GetCasterPlus().GetTeamNumber()) == 0)) {
        ...
        if (this.GetCasterPlus().HasTalent("special_bonus_imba_witch_doctor_5")) {
            if (!params.unit.IsAlive()) {
                if (params.unit.IsRealHero()) {
                    let talent_ward = this.GetAbilityPlus().CreateWard(params.unit.GetAbsOrigin(), true);

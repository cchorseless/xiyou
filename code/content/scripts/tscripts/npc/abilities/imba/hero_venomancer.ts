...
OnIntervalThink():void {
    if (!this.GetParentPlus().HasModifier("modifier_imba_poison_sting_debuff")) {
        ...
        SendOverheadEventMessage(this.GetCasterPlus(), DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_BONUS_SPELL_DAMAGE, this.GetParentPlus(), this.damage, undefined);
    }
}
...
OnSpellStart( /** talent_spawn_location */ ):void {
    ...
    let plague_ward = BaseNpc_Plus.CreateUnitByName("npc_dota_venomancer_plague_ward_" + math.min(this.GetLevel(), 4), talent_spawn_location || this.GetCursorPosition(), true, this.GetCasterPlus(), this.GetCasterPlus(), this.GetCasterPlus().GetTeamNumber());
    plague_ward.EmitSound("Hero_Venomancer.Plague_Ward");
    if (this.GetCasterPlus().HasAbility("imba_venomancer_venomous_gale")) {
        plague_ward.AddRangeIndicator(this.GetCasterPlus(), this.GetCasterPlus().findAbliityPlus<imba_venomancer_venomous_gale>("imba_venomancer_venomous_gale"), "ward_range", undefined, 183, 247, 33, false, false, true);
    }
    ...
    if (!talent_spawn_location && this.GetCasterPlus().GetName() == "npc_dota_hero_venomancer" && RollPercentage(25)) {
        ...
        for (const [response, timer] of ipairs(this.responses)) {
...
OnSpellStart():void {
    ...
    let ward_range = this.GetSpecialValueFor("ward_range") + GetCastRangeIncrease(caster);
    if ((this.GetCasterPlus().GetAbsOrigin() - this.GetCursorPosition()).Length2D() > this.GetSpecialValueFor("cast_range") + this.GetCasterPlus().GetCastRangeBonus()) {
        for (const [_, ally] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetCursorPosition(), undefined, this.GetSpecialValueFor("ward_range") + this.GetCasterPlus().GetCastRangeBonus(), DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_OTHER, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NONE, FindOrder.FIND_CLOSEST, false))) {
            if (ally.GetName() == "npc_dota_venomancer_plagueward" && ally.GetOwnerPlus() == this.GetCasterPlus() && (ally.GetAbsOrigin() - this.GetCursorPosition()).Length2D() < (this.GetCasterPlus().GetAbsOrigin() - this.GetCursorPosition()).Length2D()) {
    ...
    if (this.bWardCaster) {
        ...
        this.bWardCaster.SetForwardVector((target_loc - caster_loc).Normalized());
    } else {
    ...
    for (let i = 1; i <= projectile_count; i += 1) {
        ...
        let velocity = RotateVector2D(direction, angle, true);
        let projectile;
        if (this.bWardCaster) {
            travel_distance = this.GetSpecialValueFor("ward_range") + GetCastRangeIncrease(caster);
            projectile = {
                Ability: this,
            ...
            travel_distance = super.GetCastRange(target_loc, undefined) + GetCastRangeIncrease(caster);
            projectile = {
                Ability: this,
                EffectName: "particles/units/heroes/hero_venomancer/venomancer_venomous_gale.vpcf",
...
OnProjectileHit_ExtraData(target:CDOTA_BaseNPC|undefined, location:Vector, ExtraData:any):boolean|void {
    ...
    if (target) {
        let was_hit = false;
        for (const [_, stored_target] of ipairs(this[ExtraData.index])) {
        ...
        if (was_hit) {
            ...
            table.insert(this[ExtraData.index], target);
        }
        ApplyDamage({
            victim: target,
            attacker: caster,
        ...
        if ((target.IsRealHero() || target.IsClone()) && this.GetCasterPlus().HasTalent("special_bonus_imba_venomancer_venomous_gale_plague_wards") && this.GetCasterPlus().HasAbility("imba_venomancer_plague_ward_v2") && this.GetCasterPlus().findAbliityPlus<imba_venomancer_plague_ward_v2>("imba_venomancer_plague_ward_v2").IsTrained()) {
            let starting_position = target.GetAbsOrigin() + RandomVector(100);
        ...
        this[ExtraData.index]["count"] = this[ExtraData.index]["count"] || 0;
        this[ExtraData.index]["count"] = this[ExtraData.index]["count"] + 1;
        if (this[ExtraData.index]["count"] == ExtraData.projectile_count) {
            if ((GameFunc.GetCount(this[ExtraData.index]) > 0) && (caster.GetName() == "npc_dota_hero_venomancer")) {
                caster.EmitSound("venomancer_venm_cast_0" + math.random(1, 2));
            }
            this[ExtraData.index] = undefined;
        }
    }
}
OnOwnerSpawned():void {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let scourge_ward = BaseNpc_Plus.CreateUnitByName("npc_imba_venomancer_scourge_ward", target_loc, true, caster, caster, caster.GetTeamNumber());
        ...
        if (ability_gale) {
            scourge_ward.AddRangeIndicator(caster, ability_gale, "ward_range", undefined, 183, 247, 33, false, false, true);
        }
        ...
        if (plague_count >= 1) {
            ...
            for (let i = 1; i <= plague_count; i += 1) {
                let plague_loc = target_loc + RotateVector2D(direction, start_angle + (angle * i), true) * plague_radius;
                let plague_ward = BaseNpc_Plus.CreateUnitByName("npc_imba_venomancer_plague_ward", plague_loc, true, caster, caster, caster.GetTeamNumber());
    ...
    if (!bFirst) {
        ...
        for (const [_, enemy] of ipairs(enemies)) {
            if (!(enemy == parent)) {
                ...
                let nova_poison = enemy.FindAllModifiersByName("modifier_imba_poison_nova");

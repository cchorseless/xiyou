...
OnSpellStart():void {
    ...
    for (let i = 1; i <= mini_blast_count; i += 1) {
        ...
        let direction = (target_point - caster.GetAbsOrigin()).Normalized();
        let spawn_point = target_point + direction * mini_blast_distance;
        ...
        this.AddTimer(this.blast_delay, () => {
            ...
            for (const [_, unit] of ipairs(units)) {
                if (unit.GetTeamNumber() == this.caster.GetTeamNumber()) {
                    unit.Heal(heal, this.caster);
                    SendOverheadEventMessage(unit, DOTA_OVERHEAD_ALERT.OVERHEAD_ALERT_HEAL, unit, heal, unit);
                } else {
                    ApplyDamage({
                        victim: unit,
                        damage: damage,
...
OnSpellStart():void {
    ...
    for (let i = 1; i <= 1 + caster.GetTalentValue("special_bonus_imba_pugna_8"); i += 1) {
        ...
        if (i != 1 || !this.GetCursorTarget() || this.GetCursorTarget() != this.GetCasterPlus()) {
            nether_ward = BaseNpc_Plus.CreateUnitByName("npc_imba_pugna_nether_ward_" + (ability_level), point[i], true, caster, caster, caster.GetTeam());
        } else {
            nether_ward = BaseNpc_Plus.CreateUnitByName("npc_imba_pugna_nether_ward_" + (ability_level), this.GetCasterPlus().GetAbsOrigin() + (this.GetCasterPlus().GetForwardVector() * 150), true, caster, caster, caster.GetTeam());
        }
...
CC_OnSpentMana(keys:ModifierAbilityEvent):void {
    if (IsServer()) {
        ...
        let caster = ward.GetOwnerEntity();
        ...
        for (const [_, forbidden_ability] of ipairs(forbidden_abilities)) {
        ...
        if (cast_ability_name == "spirit_breaker_nether_strike") {
            ...
            let ability_bash = ward.findAbliityPlus<spirit_breaker_greater_bash>("spirit_breaker_greater_bash");
        ...
        if (cast_ability_name == "imba_nevermore_requiem" && !ward.HasModifier("modifier_imba_necromastery_souls") && target.HasAbility("imba_nevermore_necromastery")) {
            ...
            if (target.HasModifier("modifier_imba_necromastery_souls")) {
                ...
                if (ward.HasModifier("modifier_imba_necromastery_souls")) {
                    let modifier_souls_handler = ward.findBuff<modifier_imba_necromastery_souls>("modifier_imba_necromastery_souls");
        ...
        if (cast_ability_name == "imba_sven_storm_bolt") {
            ...
            if (GameFunc.GetCount(enemies) > 0) {
                if (enemies[1].findAbliityPlus<imba_sven_storm_bolt>("imba_sven_storm_bolt")) {
        ...
        if (cast_ability_name == "luna_eclipse") {
            if (!ward.findAbliityPlus<luna_lucent_beam>("luna_lucent_beam")) {
                ward.AddAbility("luna_lucent_beam");
            }
            let ability_lucent = ward.findAbliityPlus<luna_lucent_beam>("luna_lucent_beam");
            ability_lucent.SetLevel(4);
        }
        let ability_behavior = ability.GetBehavior();
        ...
        if (ability_behavior == DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_NONE) {
            ...
            if (ability_target_team == DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_FRIENDLY) {
                ...
                if (ability_range > 0 && (target_point - ward_position).Length2D() > ability_range) {
            ...
            } else if ((target_point - ward_position).Length2D() <= ability_range) {
                ExecuteOrderFromTable({
                    UnitIndex: ward.GetEntityIndex(),
                    OrderType: dotaunitorder_t.DOTA_UNIT_ORDER_CAST_TARGET,
...
OnIntervalThink():void {
    if (IsServer()) {
        if (this.parent.IsIllusion() && this.parent.GetTeamNumber() != this.caster.GetTeamNumber() && !Custom_bIsStrongIllusion(this.parent)) {
        ...
        if (this.is_ally) {
            ...
            this.parent.Heal(actual_damage, this.caster);
            ...
            this.caster.Heal(actual_damage, this.caster);
            if (missing_health < actual_damage) {
                let recover_mana = actual_damage - missing_health;

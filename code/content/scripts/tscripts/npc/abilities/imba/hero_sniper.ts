...
OnSpellStart():void {
    ...
    if (GameFunc.GetCount(enemies) >= 4) {
        if (RollPercentage(5)) {
            ...
            EmitSoundOn(group_cast_response[math.random(1, GameFunc.GetCount(group_cast_response))], caster);
        }
    } else if (RollPercentage(75)) {
    ...
    let distance = (target_point - caster.GetAbsOrigin()).Length2D();
    let direction = (target_point - caster.GetAbsOrigin()).Normalized();
    let launch_position = caster.GetAbsOrigin() + direction * (distance / 2);
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        this.distance_from_target = (this.caster.GetAbsOrigin() - this.current_target.GetAbsOrigin()).Length2D();
    }
}
...
CC_OnAttackRecord(keys:ModifierAttackEvent):void {
    if (!this.GetParentPlus().PassivesDisabled() && !this.GetParentPlus().IsIllusion() && keys.target && !keys.target.IsBuilding() && !keys.target.IsOther() && keys.target.GetTeamNumber() != this.GetParentPlus().GetTeamNumber()) {
        if (this.GetStackCount() < this.GetSpecialValueFor("perfectshot_attacks") && GFuncRandom.PRD(this.GetSpecialValueFor("proc_chance"), )) {
...
CC_OnAttackLanded(keys:ModifierAttackEvent):void {
    if (keys.attacker == this.GetParentPlus() && !this.GetParentPlus().IsIllusion() && keys.target && !keys.target.IsBuilding() && !keys.target.IsOther() && keys.target.GetTeamNumber() != this.GetParentPlus().GetTeamNumber() && !keys.target.IsMagicImmune()) {
        if (this.headshot_records[keys.record]) {
            if (keys.target.Custom_IsUnderForcedMovement && !keys.target.Custom_IsUnderForcedMovement()) {
                keys.target.AddNewModifier(this.GetCasterPlus(), this.GetAbilityPlus(), "modifier_knockback", {
                    center_x: this.GetParentPlus().GetAbsOrigin()[1] + 1,
                    center_y: this.GetParentPlus().GetAbsOrigin()[2] + 1,
                    center_z: this.GetParentPlus().GetAbsOrigin()[3],
...
GetCastPoint():number {
    ...
    if (this.GetCasterPlus().HasTalent("special_bonus_imba_sniper_6") && this.GetCasterPlus().findBuffStack("modifier_imba_sniper_assassinate_handler", this.GetCasterPlus()) == 1) {
        ...
        if (IsServer() == true && this.GetCasterPlus().HasModifier("modifier_imba_wisp_overcharge_721_aura") || this.GetCasterPlus().HasModifier("modifier_imba_wisp_overcharge_721") && this.GetCasterPlus().FindModifierByName) {
            let overcharge_modifier = this.GetCasterPlus().findBuff<modifier_imba_wisp_overcharge_721>("modifier_imba_wisp_overcharge_721") || this.GetCasterPlus().FindModifierByName("modifier_imba_wisp_overcharge_721_aura");
...
OnAbilityPhaseStart():boolean {
    ...
    if (caster.HasTalent("special_bonus_imba_sniper_6") && this.GetCasterPlus().findBuffStack("modifier_imba_sniper_assassinate_handler", this.GetCasterPlus()) == 1) {
        ...
        this.timer = this.AddTimer(1.75 * scepter_speed_mult, () => {
    ...
    let targets = {}
    targets[1] = this.GetCursorTarget();
    EmitSoundOn(cast_response[math.random(1, GameFunc.GetCount(cast_response))], caster);
...
FireAssassinateProjectile(targets:IBaseNpc_Plus, projectile_num, total_projectiles, bAutoCast) {
    if (IsServer()) {
        let caster = this.GetCasterPlus();
        let ability = this;
    ...
    this.AddTimer(0.1, () => {
        if (!this.enemy_died) {
            ...
            if (hp_pct <= 10 && target.IsAlive()) {
                EmitSoundOn(almost_kill_responses[math.random(1, GameFunc.GetCount(almost_kill_responses))], caster);
            }
        }
    });
}
AssassinateHit(target:IBaseNpc_Plus, projectile_num) {
    let caster = this.GetCasterPlus();
    let ability = this;
    let kill_responses = {
    ...
    if (target.primary_assassination_target) {
    ...
    this.AddTimer(FrameTime(), () => {
        if (!target.IsAlive() && !this.enemy_died) {
            ...
            if (RollPercentage(50)) {
                EmitSoundOn(kill_responses[math.random(1, GameFunc.GetCount(kill_responses))], caster);
            }
        }
    });
}
OnProjectileThink_ExtraData(location:Vector, extradata:any):void {
    ...
    for (const [_, enemy] of ipairs(enemies)) {
        if (!enemy.primary_assassination_target) {

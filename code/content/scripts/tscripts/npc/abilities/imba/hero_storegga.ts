...
OnAbilityPhaseStart():boolean {
    if (IsServer()) {
        ...
        let kv = {}
        kv["duration"] = this.animation_time;
        kv["initial_delay"] = this.initial_delay;
        this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(),this, "modifier_storegga_arm_slam", kv);
    }
    return true;
}
...
OnAbilityPhaseStart():boolean {
    if (IsServer()) {
        if (this.GetCasterPlus().findBuff<modifier_storegga_grabbed_buff>("modifier_storegga_grabbed_buff") != undefined) {
            return;
        }
        this.animation_time = this.GetSpecialValueFor("animation_time");
        this.initial_delay = this.GetSpecialValueFor("initial_delay");
        ...
        kv["duration"] = this.animation_time;
        kv["initial_delay"] = this.initial_delay;
        let hBuff = this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(),this, "modifier_storegga_grab", kv);
        if (hBuff != undefined) {
...
public hBuff : any; 
public hThrowTarget : any; 
public hThrowBuff : any; 
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let info = {
            ...
            vVelocity: this.vDirection * this.throw_speed,
        ...
        if (this.hThrowTarget != undefined) {
            ...
            if (this.hThrowTarget.IsRealHero()) {
                ...
                if (this.hThrowTarget.IsAlive() == false) {
                    ...
                    ParticleManager.SetParticleControlForward(nFXIndex, 1, -this.GetCasterPlus().GetForwardVector());
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        if (this.hAvalancheTarget != undefined && this.hAvalancheTarget.IsAlive()) {
            vNewAvalancheDir2 = this.hAvalancheTarget.GetOrigin() - this.GetCasterPlus().GetOrigin();
        } else {
            let enemies = FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.GetParentPlus().GetOrigin(), undefined, FIND_UNITS_EVERYWHERE, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES, FindOrder.FIND_CLOSEST, false);
            this.hAvalancheTarget = enemies[RandomInt(1, GameFunc.GetCount(enemies))];
...
OnIntervalThink():void {
    if (IsServer()) {
        ...
        let flDist = (this.hTarget.GetOrigin() - this.GetParentPlus().GetOrigin()).Length2D();
        if (flDist > 700) {
            return;
        }
        let hBuff = this.GetCasterPlus().AddNewModifier(this.GetCasterPlus(), this.GetAbilityPlus(), "modifier_storegga_grabbed_buff", {});
        if (hBuff != undefined) {
            this.GetCasterPlus().flThrowTimer = GameRules.GetGameTime() + RandomFloat(this.min_hold_time, this.max_hold_time);

...
OnSpellStart():void {
    ...
    this.FireOrb(this.GetCasterPlus().GetAbsOrigin() - this.GetCursorPosition());
    this.FireOrb(this.GetCursorPosition() - this.GetCasterPlus().GetAbsOrigin());
...
FireOrb(position) {
    ...
    let projectile_info = {
        ...
        fDistance: (this.GetSpecialValueFor("max_distance") * math.max(this.GetCasterPlus().GetTalentValue("special_bonus_imba_puck_illusory_orb_speed"), 1)) + GetCastRangeIncrease(this.GetCasterPlus()) + this.talent_cast_range_increases,
        fStartRadius: this.GetSpecialValueFor("radius"),
        fEndRadius: this.GetSpecialValueFor("radius"),
        vVelocity: position.Normalized() * this.GetSpecialValueFor("orb_speed") * math.max(this.GetCasterPlus().GetTalentValue("special_bonus_imba_puck_illusory_orb_speed"), 1),
...
GetBehavior():DOTA_ABILITY_BEHAVIOR|Uint64 {
    return super.GetBehavior() + DOTA_ABILITY_BEHAVIOR.DOTA_ABILITY_BEHAVIOR_UNIT_TARGET;
}
...
OnSpellStart( /** refreshDuration */ ):void {
...
OnIntervalThink():void {
    ...
    if (this.GetCasterPlus().IsAlive() && this.counter >= this.rapid_fire_interval && this.GetAbilityPlus()) {
        this.counter = 0;
        let direction = (this.GetParentPlus().GetAbsOrigin() - this.coil_thinker_location).Normalized();
        if ((this.GetCasterPlus().GetAbsOrigin() - this.coil_thinker_location).Length2D() <= this.rapid_fire_max_distance) {
    ...
    if ((this.GetParentPlus().GetAbsOrigin() - this.coil_thinker_location).Length2D() >= this.coil_break_radius) {
        ...
        if (this.GetAbilityPlus()) {
            ...
            this.GetAbilityPlus().OnSpellStart(this.GetRemainingTime() + (stun_duration * (1 - this.GetParentPlus().GetStatusResistance())));
        }

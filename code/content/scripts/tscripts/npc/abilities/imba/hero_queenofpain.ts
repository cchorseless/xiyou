...
OnIntervalThink():void {
    ...
    if (IsServer()) {
        let parent = this.GetParentPlus();
        let ability = this.GetAbilityPlus();
        if (math.mod(this.counter, this.damage_interval) == 0) {
            ApplyDamage({
                victim: parent,
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let blink_range = this.GetSpecialValueFor("blink_range") + GetCastRangeIncrease(caster);
        let scream_damage_pct = this.GetSpecialValueFor("scream_damage_pct");
        let distance = (target_pos - caster_pos);
...
OnSpellStart( /** scream_damage_pct */  /** , source_unit */  /** , is_talent */ ):void {
    if (IsServer()) {
        let caster = this.GetCasterPlus();
        let scream_source;
        if (source_unit) {
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        let travel_distance = this.GetSpecialValueFor("travel_distance") + GetCastRangeIncrease(caster);
        let projectile_speed = this.GetSpecialValueFor("projectile_speed");
        let direction;
        ...
        if (USE_MEME_SOUNDS && RollPercentage(20)) {
            if (!this.meme_index) {
                this.meme_index = 1;
            }

...
BeCreated(keys:any):void {
    ...
    if (this.starting_unit_entindex && EntIndexToHScript(this.starting_unit_entindex)) {
        ...
        this.units_affected[this.current_unit] = 1;
...
OnIntervalThink():void {
    ...
    for (const [_, enemy] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.current_unit.GetAbsOrigin(), undefined, this.radius, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NO_INVIS, FindOrder.FIND_CLOSEST, false))) {
        if (!this.units_affected[enemy] || (this.GetCasterPlus().HasTalent("special_bonus_imba_zuus_8") && this.units_affected[enemy] < this.GetCasterPlus().GetTalentValue("special_bonus_imba_zuus_8", "additional_hits")) && enemy != this.current_unit && enemy != this.previous_unit) {
            ...
            if (this.units_affected[this.current_unit]) {
                this.units_affected[this.current_unit] = this.units_affected[this.current_unit] + 1;
            } else {
                this.units_affected[this.current_unit] = 1;
            }
            this.zapped = true;
    ...
    if ((this.unit_counter >= this.jump_count && this.jump_count > 0) || !this.zapped) {
        for (const [_, enemy] of ipairs(FindUnitsInRadius(this.GetCasterPlus().GetTeamNumber(), this.current_unit.GetAbsOrigin(), undefined, this.radius * this.static_chain_mult, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_TYPE.DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAGS.DOTA_UNIT_TARGET_FLAG_NO_INVIS, FindOrder.FIND_CLOSEST, false))) {
            if ((!this.units_affected[enemy] || (this.GetCasterPlus().HasTalent("special_bonus_imba_zuus_8") && this.units_affected[enemy] < this.GetCasterPlus().GetTalentValue("special_bonus_imba_zuus_8", "additional_hits"))) && enemy != this.current_unit && enemy != this.previous_unit && enemy.HasModifier("modifier_imba_zuus_static_charge")) {
                enemy.EmitSound("Hero_Zuus.ArcLightning.Target");
                this.lightning_particle = ResHelper.CreateParticleEx("particles/units/heroes/hero_zuus/zuus_arc_lightning_.vpcf", ParticleAttachment_t.PATTACH_ABSORIGIN_FOLLOW, this.current_unit);
                ParticleManager.SetParticleControlEnt(this.lightning_particle, 0, this.current_unit, ParticleAttachment_t.PATTACH_POINT_FOLLOW, "attach_hitloc", this.current_unit.GetAbsOrigin(), true);
                ParticleManager.SetParticleControlEnt(this.lightning_particle, 1, enemy, ParticleAttachment_t.PATTACH_POINT_FOLLOW, "attach_hitloc", enemy.GetAbsOrigin(), true);
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        CustomNetTables.SetTableValue("player_table", tostring(this.GetCasterPlus().GetPlayerOwnerID()), {
...
CastLightningBolt(caster:IBaseNpc_Plus, ability:IBaseAbility_Plus:IBaseAbility_Plus, target:IBaseNpc_Plus, target_point:IBaseNpc_Plus, nimbus) {
    if (IsServer()) {
        ...
        let dummy_unit = BaseNpc_Plus.CreateUnitByName("npc_dummy_unit", Vector(target_point.x, target_point.y, 0), false, undefined, undefined, caster.GetTeam());
...
Apply(target:IBaseNpc_Plus) {
    ...
    damage_table.ability = ability;
    damage_table.damage_type = ability.GetAbilityDamageType();
    damage_table.damage_flag = DOTADamageFlag_t.DOTA_DAMAGE_FLAG_HPLOSS;
    damage_table.damage = (current_health / 100) * damage_health_pct;
    damage_table.victim = target;
    ApplyDamage(damage_table);
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        this.zuus_nimbus_unit = BaseNpc_Plus.CreateUnitByName("npc_dota_zeus_cloud", Vector(this.target_point.x, this.target_point.y, 450), false, caster, undefined, caster.GetTeam());
...
OnSpellStart():void {
    if (IsServer()) {
        ...
        for (const [_, nimbus] of ipairs(Entities.FindAllByName("npc_dota_zeus_cloud"))) {
            if (nimbus.IsAlive() && (target_point - nimbus.GetAbsOrigin()).Length2D() < distance) {
        ...
        this.traveled = 0;
        this.distance = (target_loc - caster_loc).Length2D();
        if (this.distance == 0) {
            return;
        }
        this.direction = (target_loc - caster_loc).Normalized();
        ...
        let projectile = {
            ...
            bDeleteOnHit: false,
            vVelocity: this.direction * speed * Vector(1, 1, 0),
    ...
    if (IsServer()) {
        ...
        for (const [_, hero] of ipairs(HeroList.GetAllHeroes())) {
            if (hero.IsAlive() && hero.GetTeam() != caster.GetTeam() && (!hero.IsIllusion()) && !hero.IsClone()) {
                ...
                if (caster.HasAbility("imba_zuus_static_field") && caster.findAbliityPlus<imba_zuus_static_field>("imba_zuus_static_field").IsTrained() && (caster.GetAbsOrigin() - hero.GetAbsOrigin()).Length2D() <= 2500) {
        ...
        if (thundergods_focus_stacks >= this.GetSpecialValueFor("stacks_to_awaken")) {
            ...
            caster.AddNewModifier(caster,this, "modifier_imba_zuus_thundergods_awakening", {
                duration: min(thundergods_focus_stacks * 3, 42)
            });
        }
    }
}

...
import { TeamShenFenItem_UI } from "./TeamShenFenItem_UI";
export class TeamShenFenItem extends TeamShenFenItem_UI {
	// 初始化数据
	componentDidMount() {
	...
	init = () => {
		...
		if (canSelect) {
			this.panel_1.current!.SetPanelEvent("onmouseactivate", this.onbtn_select)
		}
	...
	onbtn_select = () => {
		// 更新父节点
		let node = TeamPanel.GetInstance()
		if (node) {
			// 成员满了
			if (node.checkIsFull() && !this.hasSelect) {
				TipsHelper.showTips('成员满员',this)
			}
			else {
				if (!this.hasSelect) {
				...
				this.updateSelf();
				node.updateTeamCount(this.props.playerID, this.hasSelect)
			}
		}

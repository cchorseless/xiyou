...
export function AbilityPanel({ overrideentityindex = -1 as ItemEntityIndex, overridedisplaykeybind = DOTAKeybindCommand_t.DOTA_KEYBIND_NONE, draggable = false, dragstartcallback, dragdropcallback, dragendcallback, dragtype = "InventorySlot", slot = -1, className, ...other }: AbilityPanelAttributes) {
    ...
    return (
        <Panel className={CSSHelper.ClassMaker("AbilityPanel", className, "AbilityMaxLevel" + m_max_level)} {...other} ref={refSelf}>
                                ...
                                if (overrideentityindex != -1 && Entities.IsValidEntity(overrideentityindex)) {
                                    if (Abilities.IsItem(overrideentityindex)) {
                                        if (!$.GetContextPanel().BHasClass("dragging") && overrideentityindex != -1 && Entities.IsValidEntity(overrideentityindex)) {
                                            ...
                                            let pContentsPanel = pContextMenu.GetContentsPanel() as any;
...
export function InventorySlot({ overrideentityindex = -1 as ItemEntityIndex, overridedisplaykeybind = DOTAKeybindCommand_t.DOTA_KEYBIND_NONE, slot = -1, isBackpack = false, ...other }: InventorySlotAttributes) {
    return (
        ...
        }} className={CSSHelper.ClassMaker("InventoryItem", { "BackpackSlot": isBackpack })} {...other} overrideentityindex={overrideentityindex} overridedisplaykeybind={overridedisplaykeybind} slot={slot} />
    );
}
...
export function SecondaryAbilityPanel({ overrideentityindex = -1 as ItemEntityIndex, className, ...other }: InventorySlotAttributes) {
    return (
        <Panel {...other} className={CSSHelper.ClassMaker("DOTASecondaryAbility ShowAbility RequiresCharges HasCharges", className)} onactivate={(self) => {
            if (overrideentityindex != -1 && Entities.IsValidEntity(overrideentityindex)) {
                if (GameUI.IsAltDown()) {
                    Abilities.PingAbility(overrideentityindex);
                    return;

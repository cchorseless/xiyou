...
const all_kv_to_ts = async (singleFile = null) => {
    ...
    // 整體合併
    let kvToInterface = {};
    for (let _kk in kvInterface_client) { kvToInterface[_kk] = kvInterface_client[_kk].FilePath }
    for (let _kk in kvInterface_server) { kvToInterface[_kk] = kvInterface_server[_kk].FilePath }
    let successCount = 0;
    let errorCount = 0;
